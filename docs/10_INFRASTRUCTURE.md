...
**The `rls` and `e2e` jobs apply all 78 migrations to a fresh database on every
...
| Migrations | **78**, latest `0078_portal_guard_and_retention_nudge` |
...
npx supabase db reset   # re-runs all 78 migrations + seed
