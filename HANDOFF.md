...
**2026-09-22, latest (night): the sixth audit brief — the previous audit's conclusions re-verified against `415bfa3` (site `ce47c4f`; both repositories at exactly those commits, nothing newer anywhere; CI green on `415bfa3`): seven of its eight findings FIXED with current file references, one HOLDS (docs record escalation OFF — hosted agrees), one STILL PRESENT — Settings → Lead escalation had no preview of the affected enquiries and the eligible recipients. No new defect confirmed (the flow traced end to end; the checks and the two improvement notes are in DECISIONS `T-audit-2026-09-22-escalation-preview`). HOSTED READ ONLY at 0111 (15:56Z): policy OFF with the 0107 placeholders and NO recipients; two active admins with addresses; `notification_jobs` EMPTY; all three lead crons succeeding, sweep health ok; 8 website leads, 0 open + unanswered + unredacted — so activating the stored policy at that instant would mint nothing (the sweep's WHERE replicated as a SELECT; the minting function was not called). BUILT on branch `feat/audit-2026-09-22-escalation-preview` (worktree `.worktrees/gnk-crm/audit-f`): **0112** — `lead_escalation_candidates` (the sweep's eligibility rule moved into ONE function with a verdict per lead), `raise_lead_escalations` minting `verdict = 'due'` from it (same signature, same events, same cron call — the self-test proves the minted set equals the due set on fixed dates, rolled back), and `preview_lead_escalation` (STABLE SECURITY DEFINER, admin + aal2 + own org, evaluates the form's values AS IF ON and returns policy, recipients with reasons, counts that keep the sweep's jobs apart from the worker's e-mails, a bounded page of enquiries — never the enquirer); a Preview activation button beside Save, the card under the form, stale on any change. LOCAL at 0112; **HOSTED STILL AT 0111 — NOT applied, NOT merged, NOT deployed, escalation OFF everywhere, nothing sent.** Measured: database 19/19 (throwaway organisations), unit +33, browser 10/10 desktop + mobile, tsc 0, eslint 0. **Operator step (row 1i): say "Apply 0112 to hosted and merge" — the ritual and the rollback are the entry's last paragraph; the advisor count is expected to move by one WARN (the 0111 class). Activation stays a separate Save on the page with approved values, which are still placeholders.**
