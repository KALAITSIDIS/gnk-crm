...
| 1f | **Land the 2026-09-21 evening audit branch** (`fix/audit-2026-09-21-key-lifetime-sweep-health-interest`: 0104 key lifetime, 0105 sweep outcomes, 0106 proposal interest) | operator | **AWAITING APPROVAL.** In order, each per §3 (separate calls, verify separately, ledger by hand, advisors): (a) **0104** — additive (a column, a CHECK, three functions replaced with unchanged signatures); not deploy-coupled: the deployed worker never reads `key_attempts`. (b) **0105** — additive (a table, three functions, `enquiry_alerts_sweep()` replaced; the cron command text is unchanged so nothing is re-scheduled); the dashboard card calls `enquiry_alert_sweep_health()` once deployed, so apply BEFORE the merge or the card shows "sweep outcomes unreadable" until it lands. (c) **0106** — additive (one function); the new route answers 503 until it exists, so apply BEFORE the merge. Then merge the PR, confirm the deployment, and check: `select outcome, count(*) from enquiry_alert_sweep_runs group by 1` fills with `ok` within minutes; the admin dashboard's sweeps line stays green; open a real proposal link and see the button (no submission needed). Rollback per DECISIONS `T-audit-2026-09-21-evening`. No live e-mail, no replay and no production enquiry are part of this. |
