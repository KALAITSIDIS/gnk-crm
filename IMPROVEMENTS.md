...
- **Hard delete anywhere** — the append-only hash-chained `events` spine *is* the commission evidence. Retire states (archived / withdrawn / spam / lost) are the correct answer, and the GDPR erasure path already handles the one case that legally requires more.
  ...
  **⚠️ THIS GUIDANCE AND THE PRACTICE NOW DISAGREE — resolve it rather than letting it drift.** Production has been hard-deleted twice: about nineteen properties on **2026-08-22**, and four (PAF0002/0003/0005 plus a viewing with its signed slip and two storage objects) on **2026-08-28**, operator-instructed and after the cost was spelled out. Both were TEST records, not evidence, which is the defensible reading. The measurable residue: **28 `property` events now reference rows that no longer exist**, and the chain still verifies because events themselves were never touched. Either this line should carve out an explicit exception for test records, or the desk should archive instead. What it should not stay is a rule the project quietly breaks.
