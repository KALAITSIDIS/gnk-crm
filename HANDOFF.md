...
| 1c | **Arm the website enquiry alert** | operator | Two variables in the **Vercel** environment for `gnk-crm` (never in the repo — it is public): `RESEND_API_KEY` from a free resend.com account, and `ENQUIRY_ALERT_TO=info@kalaitsidis.com`. To send FROM your own domain rather than Resend's shared sender, verify `kalaitsidis.com` in Resend (a few DNS records) and set `ENQUIRY_ALERT_FROM`. Until both are set the alert logs SKIPPED and enquiries still save normally — **but nobody is told one arrived.** |
