...
- `properties.ts` `updatePropertySection`: `internal_notes`, `amenities_notes`, `constraints_notes`, `encumbrances_notes` (typed; internal and encumbrance notes are where the owner's situation, creditors and numbers get written). The rest is the listing's own data, which contact erasure does not touch and which nothing lets anyone correct in the chain anyway. The marketing copy keeps its values on purpose: it was written for publication, and what was advertised when is evidence in a dispute.
