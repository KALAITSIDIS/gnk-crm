...
| 1g | **Land the 2026-09-22 afternoon audit branch** (`fix/audit-2026-09-22-chain-order-escalation-payload-due-cutoff`: 0109 id under the chain lock, 0110 due-time age cutoff, the escalation's stable provider payload) | operator | **OPEN 2026-09-22.** In this order, each step on the operator's word: (a) CI green on the branch push; (b) 0109 then 0110 on hosted per §3 — each in one `execute_sql` call, verified in a further call (trigger digest `77a96eed1fdb654ea37c7c833ff0cd2d`; `raise_lead_escalations` has `p_org, p_now` and is the only function of its name; cron command unchanged; policy OFF; chains ok; health clean; ledger rows by hand → 110; `get_advisors`); neither is deploy-coupled; 0110 refuses to apply while the policy is ON; (c) merge the PR, confirm the deploy READY, probe feed/sweep. Rollback per DECISIONS: 0109 → 0108's body; 0110 → drop the two-argument function and re-create 0107's; never delete events or re-mint hashes. Escalation stays OFF throughout; nothing here activates it. |
