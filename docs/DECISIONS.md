...
- NOT the SDK's `dataCollection` option: supplying it at all swaps the base from the `sendDefaultPii: false` profile to DEFAULTS (`resolveDataCollectionOptions`: `options.dataCollection != null ? DEFAULTS : …`), where `userInfo`, bodies and headers are all on — a partial object would turn IP collection ON. And it cannot reach `http.target`, `request_path` or the browser. Not `httpIntegration({ maxRequestBodySize: "none" })` either: @sentry/nextjs builds its own Http integration (`disableIncomingRequestSpans: true`), and replacing it is integration surgery for a field the scrub already drops.
- No try/catch around the scrub. The SDK drops an event whose `beforeSend` throws and sends its own "Event processing pipeline threw an error" event instead (measured) — that fails CLOSED; a catch returning the event would fail open. The scrub guards every shape instead (tested).
- A token IN THE PATH (`/p/<token>`, `/api/portals/<portal>/<token>`) is not cut — the brief says keep the path. Its own BACKLOG line.

**Tests.** `lib/services/scrub-event.test.ts` (25): the measured shapes — Next's root span, an onRequestError error event, a sampled transaction, the door's transaction with a bearer secret and a body, a browser event, a navigation breadcrumb and a relative fetch span, odd shapes that must not throw — and one test whose event the installed SDK builds itself: a real `ServerRuntimeClient` with `requestDataIntegration()` and `sendDefaultPii: false`, fed `httpRequestToRequestData()` of an incoming request plus a captured body, `scrubEvent` as both hooks — so an SDK upgrade that puts the URL, a cookie or the body somewhere new fails there, not in someone else's log. Written first and seen failing (14 of 25). Eight one-line mutations of the implementation (each `delete`, the relative cut, each newly redacted header, the contexts pass, the url cut) turn 2–6 tests red each. The real-pipeline harness re-run with the new hooks found none of the search term, cookie, bearer secret, door headers or body on the error event or either transaction, with paths, method and ordinary headers intact. Gates: typecheck, lint, unit 191 files / 2331 tests.
