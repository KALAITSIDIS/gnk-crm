...
Finding PERF-3, migration 0018 `admin_dashboard_stats` (SECURITY INVOKER). The headline € KPIs no longer under-report past the row caps — proven with a rolled-back 2,100-deal probe where the old capped sum was €122,000 light. Top-agent ranking is now exact rather than a 5,000-event sample, and 9 dashboard round trips became 4. **Awaiting hosted migration apply before it can deploy.**
