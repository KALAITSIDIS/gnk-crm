...
| A8 | Quality Score | Computed in app code on every property save; stored in `properties.quality_score`. Weights in §C1. Publishing to visibility `public` blocked below 70 unless admin override (logged as event) — except an EMPTY project/phase, which is refused regardless of score or override (2026-09-02, see §C1). |
...
**Containers, 2026-09-02** (DECISIONS `T-container-aware-listings`, `T-wizard-project-layout`, `T-container-review`). A `project` or `phase` is graded on its units, not on rooms: the dwelling pair (area 10 + bedrooms/planning 5) is replaced by **at least one unit 15**, and **price 10** becomes **units priced** (≥ 1 unit with an asking price — the units carry the prices; the container's own price is a "from" figure). A unit is `kind = unit`, not archived, under the container OR any of its phases — a phase is not a unit (`lib/services/container-units.ts` is the one definition; the gate, the score, the worklist and the pages share it). An otherwise-complete empty development scores 75 — above the threshold on purpose: the score informs, and the publish gate refuses an EMPTY container separately and non-overridably ("Coming soon" is the honest state). The create wizard's step 2 for a development is its layout (floors or villas), generating the units in the same submit through the same writer the units page uses; admin and listing manager only, matching who manages units.
