...
- **Arm the desk-alert sweep at its designed cadence — NEEDS AN OPERATOR DECISION (the same `pg_net` decision as the line above, 0101).** 0101 is ON HOSTED and MERGED (PR #22 → `ba54f18`, 2026-09-21; production READY). Since 0101 a website enquiry's desk e-mail is a `notification_jobs` row written with the lead, sent by the enquiry route's `after()` at once and by the sweep `/api/internal/enquiry-alerts` for whatever that missed. Two operator steps arm the sweep, in order: (1) Vercel → gnk-crm → Production → add `CRON_SECRET` (any long random string) and redeploy — this alone arms the DAILY Vercel cron already in `vercel.json` (06:00 UTC ±59 min, the most Hobby allows), so a missed alert is sent within a day; (2) decide `pg_net`: if yes, create the two Vault secrets and move `supabase/activation/0103_enquiry_alerts_cron.sql` (renumbered: 0102 is the 2026-09-21 review migration — the key window, `released`, the legacy closure in the claim — on hosted and merged the same day, PR #24 → `4fae644`) into `supabase/migrations/` (its header lists the five count pins it moves — it becomes the eleventh cron job), apply per HANDOFF §3 — then a retry lands within two minutes, which is the cadence the eight-attempt schedule was written for. Until (1) the sweep answers 503 and rows wait as "Desk alert queued" on the inbox; the accelerator still sends at once as before. VERIFY: `grep -c enquiry-alerts supabase/migrations/*.sql` — a hit means (2) is done; `npx vercel env ls production` naming `CRON_SECRET` means (1) is.
