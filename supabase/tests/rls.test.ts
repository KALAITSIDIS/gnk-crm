...
describe("RLS matrix — 12 mandatory tests (doc 04)", () => {
  ...
  it("33. task_kinds: readable, not writable, and the FK still refuses an unknown kind", async () => {
    ...
    const shipped = [
      ...
      "retention_expired",
      // 0089 — a won deal whose property still carries a live hold
  ...
  it("50. cron_health() is service_role-only and sees all twelve jobs (0074, 0092, 0098, 0103, 0107)", async () => {
    ...
    expect(jobs, "all twelve scheduled jobs are visible").toHaveLength(12);
    ...
    expect(names, "the chain walkers are among them").toEqual(
      expect.arrayContaining(["verify-events-chain", "verify-events-chain-full", "ensure-events-partitions", "lead-sla", "enquiry-alerts", "lead-escalation"]),
    );
  });
