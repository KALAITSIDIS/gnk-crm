...
| 1e | **Approve, apply and arm the desk-alert outbox** (0101, branch `feat/enquiry-alert-outbox`) | operator | **OPEN 2026-09-21.** (a) Review the branch; push it for the free CI rehearsal. (b) Apply 0101 to hosted per §3 — additive, the deployed route keeps working; read the `release-compat` report first. (c) Merge; confirm the deploy. (d) Vercel, gnk-crm, Production: add `CRON_SECRET` (a long random string), redeploy — the daily Vercel cron in `vercel.json` then sweeps once a day. (e) Decide `pg_net` (the same decision as 1d's third point): yes → Vault secrets `cron_secret` + `crm_url`, move `supabase/activation/0102_enquiry_alerts_cron.sql` into migrations, bump the five cron pins, apply — a retry then lands within two minutes. Until (d) the sweep answers 503 and the inbox shows a missed alert as "Desk alert queued"; the accelerator sends at once regardless. |
