...
export async function uploadPropertyDocument(
  ...
  if (docErr) {
    ...
    await removeObjectsBestEffort(admin.storage, "documents", [path], "property document: cleanup after a rejected row");
  ...
  await logEvent(supabase, {
    ...
    // no `title` — see uploadContactDocument; the row keeps it
...
export async function deletePropertyDocument(
  ...
  const { data: doc } = await supabase
    .from("documents")
    // doc_type and visibility, not the title — see deleteContactDocument
  ...
  const admin = createAdminClient();
  if (doc.storage_path) {
    await removeObjectsBestEffort(admin.storage, "documents", [doc.storage_path], "property document delete");
  }
