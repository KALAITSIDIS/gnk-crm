...
**2026-09-23, latest (afternoon): the property-validation brief — verified against `a321de5` (site `ce47c4f`), ALL FOUR findings CONFIRMED and fixed on branch `fix/property-floor-area-validation` (worktree `.worktrees/gnk-crm/property-validation`; migration 0113; NOT merged, NOT deployed, NOT on hosted).** Floor 9 of 3 and 0 m² covered/plot areas were accepted on edit (and by the importer and any direct write) while the create wizard refused 0 m² — the 2026-09-15 audit's LST-07, never built. One rule definition (`lib/validators/property-measurements.ts`: known area ≥ 0.01 m², unknown = null never 0; `floor_number ≤ total_floors` when both known; ground 0, basements negative) now applied by the wizard, the Details form, the unit forms, the unit-type stamp, the unit writer and the importer; 0113 adds four validated CHECKs after an offender-count abort. **Production was READ-ONLY checked: 17 properties, 0 offenders (PAF0001 floor 2 of 2 is admitted by `≤`), so 0113 applies cleanly.** **Awaiting approval, in this order:** push the branch (CI) → PR → merge → deploy READY → re-run `scripts/maintenance/preflight-0113-floor-area.sql` on hosted → apply 0113 per §3 — code FIRST (the still-deployed editor would meet the CHECKs with a raw 23514). Record: DECISIONS `T-audit-2026-09-23-property-floor-area`.**
