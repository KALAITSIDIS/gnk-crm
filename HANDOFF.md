...
**2026-09-26, latest (task / deal organisation isolation): a task belongs to the organisation of the deal it names, and a deal's reminder supersession stays inside that organisation — BUILT on branch `fix/task-deal-org-isolation` (worktree `.worktrees/gnk-crm/task-deal-org`), NOT merged, NOT applied to hosted. HOSTED IS AT 0118.**
...
- **Hosted, read-only (2026-09-26, before anything was written):** ledger 0118 (118 rows), **0 tasks**, 1 deal, 1 organisation, 0 mismatches, the trigger's md5 `081ca764…` = 0025's text, `tasks_deal_id_fkey` = 0001's; the constraint validates trivially there. **Apply order: ADDITIVE — hosted 0119 first, then merge.** ONE `execute_sql` with the file's exact text (its DO block and probe must run with it; `set local lock_timeout = '5s'` is inside — apply outside 03:00–04:00 UTC, pg_cron's clock (06:00–07:00 Cyprus in summer), and re-run on a timeout), then separately: the FK (`pg_get_constraintdef` = `FOREIGN KEY (org_id, deal_id) REFERENCES deals(org_id, id)`, `convalidated`), the unique key, the index, `tasks_deal_id_fkey` gone, the trigger body md5 `2020a0ee…` = local (`md5(replace(prosrc, chr(13), ''))`), SECURITY DEFINER + `search_path=public`, ACL unchanged (callable by nobody over PostgREST), the trigger still AFTER UPDATE WITH its WHEN clause; the ledger row 0119 by hand; `rls_aal2_coverage` 0, the chain verifies; advisors: security expected UNCHANGED at 2 ERR + 37 WARN (no ACL moves); performance INFO rows will move (+1 unused index for `tasks_org_deal_idx`, and `deals_org_id_id_key`'s; if `tasks_org_deal_fkey` is listed under unindexed_foreign_keys it is the partial predicate, not a missing index) — record the INFO counts before and after. Then merge; the app carries no code change beyond `database.types.ts` (the tasks Relationships entry), so the deployed `aa01ef8`-era application is unaffected either side of the apply.
