...
**2026-09-13: a whole-system audit of this repo (`a8925a3`) and gnk-web (`a342e32`) ran, and a five-day sprint against it shipped the same day — here: sign-up closed (T-signup-closed), the site told after every public write and the feed budget for a proven forwarder (T-site-revalidate), the 24-month enquiry retention sweep, the website lead's real channel and nine indexes (T-enquiry-retention, migration 0092); on the site: Next 16.3.5, fra1, a one-hour stale ceiling, Greek/Cyrillic type, a 3:1 field edge, a nav that wraps, the revalidate door, URL-persisted search. The audit report is a private artifact held by the operator. Still open from it, none urgent: SEC-03 (personal data in immutable events — ids in payloads, notes in a redactable table), CRM-04/CRM-07 (archived rows counted by the dashboard status card and offered by the ⌘K search), CRM-05 (a build year beside a pre-completion status is accepted silently), CRM-06 (phone layout: cards by default, fewer lead-card actions), OPS-01 (the non-atomic create path).**
