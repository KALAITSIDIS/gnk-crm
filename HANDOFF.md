...
| Hosted DB | `yjgirvzgoiywdojnpkpd` — **34 migrations, latest `0034` (applied 2026-08-21)**, `non_filename_versions` = **0**, **79 events**, **3 properties**, 1 of which now names an owner (0034 backfilled it from its active mandate), 5 district + 10 area centroids — all MEASURED 2026-08-21, in a call separate from the one that applied 0034. **`verify_events_chain` returns true, checked both before AND after 0034** — a chain that verifies only afterwards proves nothing about what the migration did to it. `get_advisors` ran after, per §3: every lint is pre-existing, and 0034 created no database objects so it could not have added one. **DB-level 2FA is LIVE** — `require_aal2` on all 29 RLS tables, IMPROVEMENTS C2 |
