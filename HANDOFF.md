...
| Hosted DB | `yjgirvzgoiywdojnpkpd` — **38 migrations, latest `0038` (applied 2026-08-22)**, `non_filename_versions` = **0**, **79 events**, 3 properties, 1 mandate, 2 contacts — MEASURED 2026-08-22 in a call separate from the one that applied it. **`verify_events_chain` checked BEFORE and AFTER every apply**, true both sides; a chain that verifies only afterwards proves nothing about what the migration did. `get_advisors` after each: no new lint. **0037 closed a privilege-escalation path** — `mandates_safe` gave `anon`/`authenticated` `arwd` on an auto-updatable view owned by a `rolbypassrls` role, so writes through it skipped `mandates` RLS; now `authenticated=r`, `anon` nothing, byte-identical to local. **`cyprus_config.default_mandate_terms` (0038) is a PLACEHOLDER** — 3% / open / 6 months / 30 days, `verified_at` NULL and `source_note` says so. It prefills every new mandate, so the operator should set the desk's real standard and stamp `verified_at`. **DB-level 2FA is LIVE** — `require_aal2` on all 29 RLS tables, IMPROVEMENTS C2 |
