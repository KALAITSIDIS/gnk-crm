...
**Landing (2026-09-24 morning).** On the operator's word ("merge now, do the landing"). A first attempt, on "ALL OK HERE ITS MERGED", was held by the session's permission guard — the PR was in fact still open. Main moved three times while it was open (#49, #51, then #51's landing docs; each a DECISIONS append conflict, main's entries kept first). PR #53 merged pinned to `a8d19b9` → main `ed6166c` (merge commit) at 07:02:18Z; `a8d19b9` differs from the CI-green `60863d0` in docs only (`git diff 60863d0 a8d19b9 -- . ':(exclude)docs/**' ':(exclude)HANDOFF.md'` is empty), and CI was green on `a8d19b9` as well (checks, rls, e2e). Before the merge, on the tree merged with #51 (`6020dab`): typecheck and lint clean, unit 196 files / 2497 tests with `--testTimeout=30000`. Vercel production `dpl_8KfND96ttsHQXab7Nxe324SRbLCe` READY 07:03:24Z, built from `ed6166c`, aliased to gnk-crm.vercel.app, fra1. CI on the merge commit: green (run 35967477738: checks, rls, e2e). No runtime errors since the deploy; no Sentry issue first seen after it; CSP nonce on 16 of 16 `/login` scripts.

**Verified in production**, with counts and markers of my own only:
- **The path.** 40 GETs of `/api/portals/bazaraki/cafe5300<56 hex>?probe=gnk53-<i>`: a registered portal and a fake, well-formed token, so the route does one read and answers 404 (40 of 40), writes nothing and is not metered. Every request also runs the proxy, whose `middleware GET` transaction carries `http.target`. The sampled ones (30, extrapolated) store it as `/api/portals/bazaraki/[token]` and nothing else: the marker and the query are gone, the path and the portal stay. `url.full`/`http.url` are not set on those transactions. No route-handler transaction was sampled.
- **The browser.** The live `/login` chunks carry `nxtPtoken`, `[token]` and the `createDsc` hook.
- **Not probed: `/p/<token>`.** Every request there, malformed or unknown, notes a miss against the caller's IP hash, and 20 misses in a 15-minute window refuse that IP (0081). A probe from here would write rows and lock the operator's own IP out of proposal links. The trace-header fix rests on the unit test with the real OpenTelemetry span and listener, and on the bundle.
- **A correction to "Production, counts only" above.** Those counts covered transactions NAMED on the routes. The proxy's `middleware GET` transaction runs on every path and carries the raw `http.target` — a carrier this entry did not name, found by this landing's own control query. Counted after the landing: across ALL transactions in 30 days, 0 carried an un-redacted `http.target` on either route (`http.target:/p/* !http.target:*token*` and `http.target:*/api/portals/* !http.target:*token*`). "Nothing to rotate" still holds.
...
## T-deal-created-title-shape — a converted deal's `created` event no longer carries the buyer's name (2026-09-23; no migration)

**The defect** (found by T-updated-event-shape-only's pre-merge review and recorded in BACKLOG; the operator asked for it next). `convertLead` (`lib/actions/leads.ts`) titles the new deal `<contact display name> — <reference>` and wrote that `title` into the deal's hash-chained `created` event. So the buyer's name entered the chain on EVERY conversion, where neither erasure nor a correction can reach it (SEC-03). T-updated-event-shape-only had just made a RETITLE shape-only, which left the creation as the larger leak.

**Why a payload edit is enough.** Nothing reads it: the timeline's `created` line prints an amount only (`lib/services/events.ts`), the title redaction in `entity-timeline.ts` applies to document events, no SQL reads a deal `created` payload, and `convertLead` is the only path that creates a deal (no other insert into `deals` in `lib` or `app`, none in `supabase/migrations`). The `title` key is gone from the payload. The deal ROW still gets the title, where the pipeline shows it and an edit can correct it. Erasure does not reach it: contact erasure leaves deals alone and keeps a person's identity by design (`identity_retained`), so after an erasure the row still reads `<buyer> — <reference>`. The difference from the chain is that the row CAN be corrected, and the event could not. The payload is now `{ from_lead, stage }`, plus `expected_value` and `expected_value_from` when a website budget band seeded the value. `stage` is the office's pipeline stage name, not a person.

**Existing events cannot be changed, and are not.** Measured on hosted 2026-09-23, count and ids only: two deal `created` events exist, both carrying `title` (ids 14 and 70, 2026-07-15 and 2026-08-04, operator test data; one deal row). They keep it.

**Tests, red first.** `lib/actions/deal-created-event-payload.test.ts` drives the real `convertLead` through `lib/testing/fake-client.ts` (the lead, the first stage, the buyer, the listing, the deal insert, the claim). It checks that the buyer's name is in no logged payload and that the `created` payload has exactly its new shape, with and without a budget band. It also pins that the deal row is still titled `<buyer> — <reference>` and that the lead's `converted` event is still ids only. RED before the fix: 3 of 5, each because the title was in the payload; the two pins passed. GREEN after: 5/5. No e2e or database test asserts the payload's `title`.

**Deploy.** Code only: no migration, no environment variable, nothing to apply on hosted. BACKLOG's titles entry keeps its other writers open (task and document titles, a mandate file's name, the lost reason), and the deal title is struck inside it with a VERIFY that hits the pre-fix code and not the fixed one.

**Review.** A three-lens check of the PR (readers of the payload, other ways the buyer's name reaches the chain at conversion, and test validity; five agents, two refuters per finding) confirmed no defect. The route its refuters split on is recorded in BACKLOG's titles entry: system-built task titles copy a name. The `deal_no_contact` nudge copies the deal's title and `retention_expired` the contact's retained name (0078), so ticking either writes the name through the task's `completed` / `reopened` event, which is already one of the entry's open writers. Two nits are fixed: this entry and the test header used to say erasure reaches the row's title, and the BACKLOG entry called every remaining writer rendered when a task's `created` title is not (so that one is a plain payload edit too).

**Landing (2026-09-24 morning).** On the operator's word ("merge #54", then "merge #54 when it's green"):
- **Before the merge.** Main had moved twice. PR #53's code (Sentry scrubbing only, no overlap) was merged into the branch, and the resolved tree gave typecheck 0, lint 0, unit 2502/2502 across 197 files; branch CI on `a4f5eaf` was green (run 35967892078: checks, rls, e2e). Then #53's landing docs arrived (`de6cc07`, docs only, no non-doc line, a clean merge), and the waiting script correctly refused to merge on a moved main. The PR was then merged by hand, pinned to `a4f5eaf`.
...
**Landing (2026-09-24 morning).** On the operator's word ("merge the rls fix when it's green"), a waiting script merged PR #55 as soon as branch CI on `ceeb208` was green (run 35970527195: checks, rls, e2e) and main had not moved. Main `9d79157`. Production `dpl_FJt9tqbzj5Pb53xfBzaZHk63E3Wi` READY and aliased, built because the ignore step skips only docs; the app code is unchanged. `/login` 200 with the CSP nonce on 16 of 16 scripts. CI on the merge commit green on the first attempt (run 35971794324: checks, rls, e2e), with test 15 in its new form.

**The sweep's result.** 97 agents across three finders: every file under `supabase/tests/` read in full, every spec and helper under `tests/e2e/`, and a pattern search over all 284 test files. Two refuters per candidate, and a candidate was kept only if both confirmed. Result: 22 confirmed (test 15 and 21 more) and 21 refuted, 20 of those because two values turned out to come from one clock or a generous margin. The 21 are one BACKLOG entry, "Clock-dependent tests found by the 2026-09-24 sweep", grouped by shape with the fix for each. Two are urgent: the lead-escalation pair's fixtures are dated 25–28 September 2026.
...
- Never stored, per the query validator's "unknown attribute": `x-vercel-ja3-digest` and every `http.response.header.*` — this setup does not copy response headers. (Corrected at the landing: `x-vercel-proxy-signature` and `x-vercel-sc-headers` were first listed here too, but their queries validated and simply held 0 values outside `[Filtered]` in those 7 days; the landing critic found `x-vercel-proxy-signature` present, and `[redacted]`, after the deploy.)
...
**Landing (2026-09-24 morning).** On the operator's word ("merge when green and land it"). CI was green on `28ed59a`, then #55 landed and the PR conflicted (a DECISIONS append; #55 also changed `supabase/tests/rls.test.ts`), so main was merged in and CI re-run on the merged head `48b10f5`: green (run 35972513023: checks, rls, e2e). PR #56 merged pinned to `48b10f5` → main `75d8635` (merge commit) at 08:12:21Z; CI on the merge commit green (run 35973882993). Vercel production `dpl_6uZZgd9ZZpNFoQNi8gMrb8sVrCiH` READY 08:13:43Z, built from `75d8635`, aliased to gnk-crm.vercel.app, fra1.

**Verified in production** by a read-only marker probe (08:14:04–08:14:20Z: 60 GETs of `/login` and 30 of the portal feed with a fake token, each sending a cookie `gnk_probe56=…`, an unlisted header `x-gnk-probe56-new` and a listed `accept-language: xx-PROBE56`; never `/p/`) and three independent checkers, counts only:
- **Deploy health:** READY and aliased, no runtime errors, no Sentry issue first seen after the deploy, CSP nonce on 16 of 16 `/login` scripts, the live browser chunk carries the list (two literals only `HEADERS_KEPT` holds), `/contacts` 307 to `/login`.
- **The fix:** after the deploy, **0** transactions hold a raw `x-vercel-proxied-for` or `x-vercel-ja4-digest`, against **3,220** and **3,210** in the 24 hours before (the same query, so the zero is not a query artifact). Every transaction carrying them stores `[redacted]` (190 extrapolated from 19 sampled — 13 of the proxy's `middleware GET`, 6 `GET /login`). The unlisted marker header is `[redacted]` on all of them and its value on none. The listed `accept-language` kept `xx-PROBE56` on all of them. The marker cookie never became an attribute ("Unknown attribute" — nor did a bare `cookie` key).
- **The adversarial critic** could not refute it for server transactions: `x-real-ip`, `x-forwarded-for`, `x-vercel-forwarded-for`, `forwarded`, the nine `x-vercel-ip-*` geo/ASN headers, `x-vercel-proxy-signature`, `x-vercel-oidc-token` and `x-vercel-sc-host` all exist after the deploy with 0 non-redacted values. `x-vercel-id` (kept on purpose) is the `region::region::id` request id; its residual risks are a coarse edge region and being the join key into Vercel's own request logs, which hold the client IP — our logs, accepted.
- **Caveat:** every post-deploy transaction so far is the probe's; no signed-in or browser traffic has arrived since. Re-run the two `x_vercel_*` counts and `has:http.request.header.cookie.sb_yjgirvzgoiywdojnpkpd_auth_token` (11,870 in the 14 days before; must be 0 after) once the desk has used the app.

**Found by the critic, confirmed, NOT this change's:** browser INP spans (`span.op:ui.interaction.click`) store an IPv4-shaped caller address in `client.address` and `http.client_ip` — 50 of 60 in 14 days, across five releases. The SDK does not put it there: `browser-utils` sets `client.address: "{{auto}}"` only when `userInfo` is on, and it is off (`sendDefaultPii` unset), so Sentry's ingest infers it from the request. No `beforeSend*` hook can remove what ingest adds. BACKLOG line; the likely fix is the project setting "Prevent Storing of IP Addresses" (operator).

Remote branch deleted; worktrees removed.
...
**Landing (2026-09-24 morning).** On the operator's word ("merge #57"). Main was checked right before the merge: `6b87a4f` already held all of main, with #56 merged into it and the full gates run: typecheck 0, lint 0, unit 2512/2512, branch CI green (run 35974720140). PR #57 → main `af652d3`, pinned to `6b87a4f`. Vercel production `dpl_EWKGsCBh98Uquv6bmYDuQBemXdct` READY and aliased. `/login` 200 with the CSP nonce on 16 of 16 scripts, `/tasks` 307 to login, no runtime errors, and no Sentry issue first seen after the deploy. CI on the merge commit green on the first attempt (run 35975936887: checks, rls, e2e). Nothing on hosted. Remote branch deleted.


## T-escalation-test-dates-far-future — the lead-escalation tests' hand-dated leads move to 2099, out of the live cron's reach (2026-09-24; tests only)

**The time bomb** (the 2026-09-24 clock sweep, BACKLOG "Clock-dependent tests found by the 2026-09-24 sweep"; the operator asked for it next). `supabase/tests/lead-escalation-preview.test.ts` and the age-cutoff block of `supabase/tests/lead-escalation.test.ts` date their leads 25–28 September 2026 (and 24 October for the DST case). They sweep them through `raise_lead_escalations(p_org, p_now)` with the GLOBAL `cyprus_config.lead_escalation` policy switched ON for a short stretch. The live `lead-escalation` cron runs every five minutes on the same stack, for every org, on the REAL clock. The preview file's header relied on the leads being "dated in the future relative to the suite's clock", and that stopped holding on 25 September. From 2026-09-28T06:15Z (and again from 2026-10-26T07:15Z for the DST lead), a cron tick landing inside an ON stretch could mint these leads first. That would fail the preview's `minted = 3` and the age-cutoff block's "0 rows" checks, with the code correct. CI would have gone red at random.

**The fix.** Every hand-dated lead instant in the two places moved from 2026 to 2099, which has exactly 2026's calendar (checked in node: Friday 25 September, Tuesday 1 September, Thursday 1 October, switches on 29 March and 25 October). Only the year changed. The expected due times follow: on the local database, `lead_escalation_due_at` gives 2099-09-28T06:15Z for Friday 22:00 and 2099-10-26T07:15Z across the October switch, the same shape as 2026. This follows the 2099 precedent of `enquiry-contact-suggestions`. Pausing the cron with `cron.alter_job` was rejected, because it is global state a crashed run would leave off. Unchanged: the working-time clock tests and the grants probe. They call `lead_escalation_due_at` on a timestamp and create no lead, so no cron can reach them. So did the minting-sweep block's real-clock leads, which that file already asserts on state ("exactly one row") because the cron may mint them.

**The guard.** Each file now has a test that every lead it dates by hand was received after 2098, which is always beyond the live cron's reach. The preview file checks all of its fixtures; the escalation file checks the age-cutoff block's five, including the two created inside tests. RED on the 2026 dates (11 and 5 within reach), GREEN after.
...
**Refuse, not normalise.** BACKLOG's entry suggested collapsing CR/LF to a space. That was a suggestion in a findings list; no DECISIONS entry adopted it. It would store a value the visitor did not type (`+357 99 Email: x@y` as a phone). A browser's one-line input drops LF and CR (the new e2e shows Chromium turning a pasted newline into a space), so the refusal is met by scripts and, rarely, by a visitor who pasted one of the other separators (VT, FF, NEL, U+2028, U+2029 survive an input, as measured in Chrome during gnk-web's review). Both are told why, in the proposal page's language where that applies. (An earlier draft of this entry said a browser input "cannot hold a break"; that was true of LF and CR only.)
...
**What cannot be detected, recorded.** A break that forged a header the grammar accepts is the same bytes as a genuine one; unit tests pin both identities:
- a name of `Ann` + LF + `Email: x@y` when no e-mail was given;
- a name of `Ann` + LF + `Email: x@y` + a blank line, which pushes the REAL Email and Phone lines into the visitor's words. The review found this second one.

Only the lead's `created` event (`has_email` / `has_phone`, written from the real values since 0084) can contradict such a header. The input rule is the fix; the reader is the backstop.

**Historical data, measured 2026-09-24 (read-only, counts only):**
- hosted: 8 website blocks, all 8 in the strict grammar; 0 with a non-LF break in the header; 0 phone-channel leads carrying an Email line;
- hosted, the stronger check: in all 8, the header's Email and Phone lines agree with the `created` event's `has_email` / `has_phone`. So no stored production enquiry shows a forged or hidden contact line;
...
- 0114's own self-test runs inside a subtransaction that is always rolled back, so it leaves nothing, not even an event. Run against 0101's or 0106's body it aborts: `0114 aborted: a value with a line break (U+000A) was accepted`. It also plants a proposal property whose reference holds a break (`properties.reference` has no shape rule, and 0106 would have written it onto the About line). Run against 0114's proposal function with only the reference check removed, it aborts: `… a proposal reference with a line break was accepted`. So the reference check can be seen to work, and the DB test pins the same case.

**Results** (local stack at 0114, 2026-09-24):
- gates: typecheck 0, lint 0, unit 2572/2572 (199 files);
- DB: the eight enquiry and release-compat suites 103/103. The full RLS suite is 358/363; the five reds are the recorded local residue that fails on main too: RLS 41/42/57 (feed fixtures), and sweep-runs and cron on `23505` `enquiry_alert_sweep_runs_request_id_key`;
- e2e desktop: proposal-interest 11/11, enquiry-contact-suggestions 6/6, public-enquiry 9/9;
- RED before the fix: 42 unit tests failed (plus the new module's file), and 8 of the DB file's first 11. Its twelfth test, the proposal-held reference with a break, was added after the review. It fails against 0114's proposal function with only the reference check removed (installed on the local stack for the run, then restored; prosrc md5 re-checked). The e2e additions were not run against the old code; the route, DB and unit tests pinning the same behaviours were.
...
**Independent review** (one adversarial reviewer, read-only, after the first CI-green commit). No critical or important defects. It confirmed:
- the function bodies differ from 0101 and 0106 only by the insertions;
- the TS and SQL break sets match;
- every writer's header still parses;
- the rollback logic cannot hide a failure.

Its five low/info points, verified before acting:
1. the wider forgery above — documented, and measured on hosted with the event cross-check;
2. the self-test's replay checks passed vacuously on zero rows (NULL) — now `not found or … is not true or … is distinct from`;
3. the NEL claim — corrected and tested;
4. the proposal door's reference check could not be seen to work — now it can (above);
5. info only: CRLF uniformity is judged across the whole message, so a desk-typed CRLF header whose body holds a bare LF reads as ambiguous (a manual path, never a guess). No known writer mixes line endings, so it is left as is.
...
- **gnk-web** is not required for this to be correct: it forwards the CRM's sentence on a refusal. Without a change there, though, a post carrying a break gets the site's 502 "That did not send" plus one `enquiry.refused` report, the alarm meant for a real enquiry turned away. That means a script, or a visitor who pasted a rare separator. gnk-web [#12](https://github.com/KALAITSIDIS/gnk-web/pull/12) mirrors the rule (a pinned copy of `LINE_BREAK_CODE_POINTS`), so it answers its own 400 instead. It is independent of this PR's order.
...
**Landing (2026-09-24 late night).** On the operator's word ("merge #62 and verify the deploy"). Main was re-checked right before the merge (`f8641c8`, unmoved). PR #62 → main `0b0a3ad`, pinned to `3a2e6e3` (branch CI green on `4ead74f` and `3a2e6e3`). Vercel production `dpl_FinAq1CYbWnXtUYt33QrHpnULw7i` READY and aliased. CI on the merge commit green (run 36056567514: checks, rls, e2e). Hosted holds 0 reservations and 0 `reservation_status_changed` events (counts only), so production has nothing for the changed line to render and no record was created to make one; the busiest property's Reservation and Activity tabs render in the operator's Chrome with no error, on the new deployment (200), with no runtime errors and no new Sentry issue. `/login` 200 with the CSP nonce on 16 of 16 scripts; protected routes 307. Nothing on hosted. Remote branch deleted, worktree removed.

## T-media-file-name-shape — a photo's events carry its id and the digest of its bytes, never its file name (2026-09-25; no migration)

**The leak** (operator: "fix the photo file name leak next"; BACKLOG "More event payloads carry typed text by value"). Inspected at `0b0a3ad` (`origin/main` when the work began), with a read-only scouting workflow (three readers and a critic).
- `uploadPropertyMedia` (`lib/actions/media.ts:167-174`) logged `media_uploaded` as `{ media_id, file: file.name, kind, watermarked }`. The name is whatever the uploader's machine called the file ("Andreou villa front.jpg"), and an ASSIGNED AGENT uploads too, not only admins and listing managers (`property_media_insert`, 0002/0100). The importer (`scripts/import/media.mts:254-261`) wrote `{ media_id, file: name, … }` the same way.
- `deleteMediaBulk` (`media.ts:447-493`) READ the name back out of the property's old `media_uploaded` events, with the ADMIN client (the lookup the 2026-07-16 audit added so "Photo deleted" named the photo), and copied it into the new `media_deleted`. The chain fed itself typed text, and would have kept doing so from old events after the writers stopped.
- `mediaUploadedFile` / `mediaDeletedFile` printed the name on the property's Activity tab and the admin feed. The evidence report excludes these two types (`MEDIA_NOISE`).
- **Hosted, read-only, counts only:** 35 `media_uploaded` events, all with a `file`; 29 of 35 `media_deleted` with one (copied forward); 0 `media_alt_set`. They stay (the chain cannot be edited) and are no longer printed.
- **RED first**, through the real actions and the real `logEvent` (`lib/actions/media-event-payload.test.ts`: a real sharp image named with a synthetic person and phone). 7 of 9 failed on `0b0a3ad`: the four upload cases because the payload carried the name and no digest; the three delete cases on `admin.from is not a function`, because the admin stand-in holds storage only, so the old events lookup could not run. The 2 controls — a rejected row and a refused delete — log nothing.

**What the timeline can honestly say.** `property_media` never stored a name: its columns are ids, generated UUID paths, `kind`, size, `alt`, `content_sha256` (0088). Storage never received one either (`binaryBody` is a nameless Blob). So there is no "current title" to join, and a deleted photo has no row at all. The lines are the fact — "Photo uploaded" / "Photo deleted" — as `media_cover_set` and `media_reordered` have always been.
...
**Tests.**
- The writer test above: 9/9 after the fix — exact upload payloads for a photo and a floor plan, the event's digest equal to the row's, one event per file of a multi-file upload, the delete's exact payload with storage removal unchanged, the select naming `content_sha256`, a bulk delete (one photo without a digest), and the controls. Every inserted row is searched for the synthetic file name's words.
- `events.test.ts`: legacy upload, import and delete payloads WITH a file name render the bare lines; new payloads the same; malformed payloads do not throw; both go through the translator. The old test pinning "Photo deleted — images (4).jpg" now pins its absence.
- `tests/e2e/event-typed-text.spec.ts` gains a photo flow through the real Media tab: a sharp-made JPEG uploaded under a synthetic personal name, then deleted through the tile's own button. The stored payloads are exact, with the digest equal to the image's and the row's. The Activity tab shows one "Photo uploaded" and one "Photo deleted" with none of the name's words. Its teardown removes any stored objects (rows cascade with the property, files do not). The whole spec passes on desktop AND mobile (14/14).
- Mutation proofs (one exact replacement each, restored after): 8 of 8 killed. They cover the name restored or truncated under another key, the digest dropped, a null digest written, the admin events read restored, the select losing `content_sha256`, and each line printing a legacy name.
...
**Landing (2026-09-25 early morning).** On the operator's word ("merge, confirm the deploy, then check the live site"). Main had not moved (`88f0438`, the PR's base). PR #63 → main `0626680`, pinned to `39de646` (branch CI green on `021898a` and `39de646`). Vercel production `dpl_6pM9YPxLbaHVN2ZVtx4eYU25eB9Y` READY and aliased. CI on the merge commit green (run 36094716152: checks, rls, e2e). Verified read-only in the operator's Chrome: the test property with the most named photo events shows, on its Activity tab (latest 50 lines), 36 photo lines, all bare (13 "Photo uploaded", 17 "Photo deleted", 6 "Photos reordered"), none with a dash or a file name; the check reported counts and never printed a line's text. The request is on the new deployment in Vercel's logs (200), with no runtime errors and no new Sentry issue. `/login` 200 with the CSP nonce on 16 of 16 scripts; protected routes 307. Nothing on hosted. Remote branch deleted, worktree removed.

## T-escalation-recovery-reason-shape — an escalation recovery's event carries no typed reason (2026-09-25; migration 0115)

**The leak (verified against `f075f9b`).** `request_lead_escalation_recovery` (0111, the latest body; 0112 did not re-create it) wrote the admin-typed resend reason, up to 200 characters, into the `lead_escalation` / `recovery_requested` event as `'reason', v_reason`. The events table is append-only and hash-chained, so nothing written there can be erased or corrected (SEC-03). Nothing ever read it: there is no `lead_escalation` entry in `EVENT_LINES` and no lead timeline page, so the dialog's promise "Kept on the lead's timeline with your name" was never kept. The chain was the text's only home, and the TypeScript payload scan cannot see SQL. Reproduced on the local stack at 0114 by `supabase/tests/lead-escalation-recovery.test.ts`: a synthetic name, phone and address passed as `p_reason` landed in the lead's events (red), a resend without a reason was refused, and the retry event carried `reason: null`.

**Decision: drop the text, not move it.** The pattern in this series keeps descriptive text on its source record. Here there is no source record: the job row never held the reason, and nothing reads it. A new column would need its own erasure rule for text nobody reads, so the text is no longer collected at all. A required field that nothing keeps would tell the admin a falsehood. The audit fact stays whole in the event: the admin as actor, the time, `action`, `provider`, `job_id`, `key_serial`, `key_rotated`, and `previous_state` / `previous_category` / `previous_result`, which record why the worker had stopped. The friction that matters stays: the database still admits `resend` only when `retry` is unsafe, and Review & resend is a dialog that states the duplicate risk and needs an explicit confirmation.

**Migration 0115** (`escalation_recovery_reason_shape`) re-creates the function from 0111's text, generated by removal, with exactly three removals (a `diff` of the two bodies shows only them): the `'reason'` key, the refusal "A reason is required to resend under a new key.", and the `v_reason` variable with its 200-character cut. `p_reason` stays in the signature, accepted and ignored. Dropping it would change the arity, and the deployed code sends it on a resend, so it would fail CLOSED until the new code deployed. With the parameter kept, the apply is not deploy-coupled in either order. The return shape, security (SECURITY DEFINER, `search_path`) and grants are unchanged; the grants are re-stated anyway. Apply-time assertions: exactly one function by that name; the signature, SECURITY DEFINER and `search_path`; the body contains no `v_reason`, `'reason'` or `p_reason`, but still contains `recovery_requested`, `previous_result` and `key_rotated`; the ACLs; and the first refusal without a session. `database.types.ts` is unchanged (same arguments). The restore pin moves to 115.

**App.** `recoverLeadEscalation` no longer takes a `reason`; `z.object` strips the key that a browser loaded before this deploy still posts, so it never reaches the database. The Review & resend dialog loses the Reason box and says "The audit log records that you resent it, when, and why the worker stopped." There is no lead timeline, so the text does not say "timeline". Confirm is enabled unless a request is pending.

**Tests.** DB (`supabase/tests/lead-escalation-recovery.test.ts`, 15/15): the retry and resend events have no `reason` property; a resend with no reason is admitted; a `p_reason` from an old caller holding a synthetic name, phone and address appears nowhere in the lead's events or the job row. 3 of these were watched RED at 0114 and GREEN at 0115. Unit (`lib/actions/leads-recover-escalation.test.ts`): the RPC receives only `p_job_id` and `p_action`, even when a stale caller posts a reason (watched red first). E2E (`tests/e2e/lead-escalation-recovery.spec.ts`, desktop and mobile, 8/8 with setup): the dialog has no textbox and shows the audit-log sentence; opening it moves nothing; both events lack `reason`. Measured 2026-09-25: tsc 0 (`--incremental false`); eslint 0 on the whole tree; unit 2676/2676 across 206 files; `npm run build` exit 0; the full DB suite 364/370. The 6 reds are the known residue on this local stack (41/42/57, `enquiry-alert-sweep-runs` connect_error, `enquiry-alerts-cron`) plus `postgis-catalog-guard` "anon may not update", which passes alone both on untouched main and on this branch (a load flake).

**Hosted.** Read-only, counts only, 2026-09-25: hosted at 0114, **0** `recovery_requested` events, so no historical copy exists to decide about.

**Deploy order.** Either order is safe. Following the usual additive order: branch CI green → hosted 0115 (one `execute_sql` with the file's text, verified separately: prosrc digest = local, ACLs, ledger 115, advisors unchanged) → merge → deploy READY. An old app against 0115: its reason is ignored and a resend without one is now admitted. A new app against 0114: a resend is refused with the old sentence "A reason is required…" (fails CLOSED), and retry is unaffected. Rollback: revert the merge, then re-create 0111's body (the file's function text) if the old requirement must return.

**Landing (2026-09-25, late morning).** On the operator's word ("apply 0115 to hosted and merge #64"). Hosted first: its function body was 0111's (md5 `cf6f385c…`); 0115 applied in one `execute_sql` with the file's text, its assertions passed; verified separately: body md5 `bf6060c8…` = local, one overload, SECURITY DEFINER and `search_path` kept, ACL unchanged, ledger by hand at 115, advisors unchanged (2 ERR + 36 WARN, by design). Then PR #64 → main `8b3b313`, pinned to `e3ae0d3` (branch CI green: checks, rls, e2e; main unmoved). Vercel production `dpl_14cRt53tGch9WoTYL6o8yYQYcQZD` READY; CI on the merge commit green (run 36116353647: checks, rls, e2e); `/login` 200, protected routes 307, no runtime errors. Escalation stays paused, so no recovery was exercised in production; the behaviour is proven by the DB suite on CI's fresh database.

## T-viewing-feedback-shape — a viewing's feedback event carries its rating, never the buyer's words (2026-09-25; no migration)

**The leak** (operator: "fix the viewing feedback leak next"; BACKLOG "More event payloads carry typed text by value"). Inspected at `fff6f96` (`origin/main` when the work began), with a read-only mapping workflow (four readers and a completeness critic, which re-read the RLS and erasure claims against the migrations itself).
- `saveViewingFeedback` (`lib/actions/viewings.ts`) spread the whole feedback object, `{ rating, liked, disliked, comment }`, into the hash-chained `viewing_feedback` event on the PROPERTY. `liked` / `disliked` / `comment` are the buyer's words, typed by the agent, up to 2000 characters each, and they name people ("her husband Andreas wants a bigger garden"). The payload shape had not changed since T4.3 (`f82a68b`); it was the one writer the payload scan's `REVIEWED` map excused.
- The `viewing_feedback` line printed the comment (else `liked`) on three surfaces: the property's Activity tab (every role of the org), the admin dashboard's feed, and the commission evidence report when it is scoped to a property. That report lists EVERY viewing's feedback on the property, other buyers' included, and it is a dispute document made to leave the office (`docs/01_PROJECT_CONTEXT.md` 41/310). Its lines go into `reportContentHash` and into the stored PDF, which is not erasable by design.
- **A crash, found by the critic and proven RED:** the renderer is keyed by event type alone and any active org member may insert any payload (`events_insert`, 0071). `"★".repeat(rating)` with a forged `rating: 1e9` threw `RangeError: Invalid string length`, which would take the property page, the admin feed and evidence generation down for everyone.
- **Hosted, read-only, counts only:** 0 viewings, 0 `viewing_feedback` events. No historical copy exists.
- Name clash, kept apart throughout: the follow-up TASK kind `viewing_feedback` (the 48-hour nudge, SQL 0020…0078) reads `viewings.feedback` on the ROW and is unaffected.

**Verified, not assumed** (the mapping, re-checked by the critic): `viewings_select` and `properties_select` are both org-wide (0030:113-116, 150-153, not redefined since), so everyone who can see a property's Activity tab can read its viewings; the property page has NO other path to a past viewing's feedback (its viewings card lists scheduled ones only; the calendar has no property filter), so a bare line would have silently dropped the C7 acceptance; feedback is overwritten by every save and each save logs an event; erasure never touches `viewings.feedback`.

**The fix.**
- **Writer:** the payload is `{ viewing_id, reference, rating }`, built explicitly. `rating` is the form's 1–5 score (not typed text), `reference` the office's own property code, `viewing_id` the only link back to the row. The row keeps all four fields. The read, the admin-or-own guard, the completed-only rule and the proven UPDATE (zero rows → refused, nothing logged) are unchanged.
- **Renderer:** prints `Viewing feedback` plus the stars and nothing else, from ANY payload, old or new — and stars only for an integer 1–5, so a forged rating renders the bare line instead of throwing. `viewingFeedbackNote` / `viewingFeedbackStarsNote` are gone in EN/EL/RU; `{note}` left `messages.test.ts`.
- **Current context, not history.** `attachCurrentTitles` (`lib/services/event-context.ts`) also attaches `current_feedback`: the viewing's comment, else what was liked (the two the line used to print), read from `viewings.feedback` on the VIEWER's client in one batched, org-bounded, uuid-filtered query per page. It is labelled "current feedback: …" (`currentFeedback`, EN/EL/RU) and rendered after the line, never inside it. Only the NEWEST `viewing_feedback` event per viewing on the page carries it (by `occurred_at`), so an older save never shows today's wording beside it; only a `property` event; only when the viewing's `property_id` is the event's own property (a crafted event cannot pull one property's feedback onto another's timeline); never a fallback to any payload. The evidence report attaches nothing — its lines are the neutral ones.
- **Why a join here, when the lost-reason and release-reason fixes chose none:** those rows' text is shown elsewhere (the deal header, the leads inbox, "Earlier holds"); a viewing's feedback had no other surface on the property page, and C7 asks for it there. The mutable-row objection is met by labelling it current and attaching it to the newest save only. This reverses T4.3's "without the timeline query needing to join viewings": the join exists now, on the viewer's client, precisely so the words can leave the chain.
- **Payload scan:** `FORBIDDEN` gains `comment`, `liked` and `disliked` (whole `_` parts are matched, so `liked` would not have covered `disliked`); the `viewings.ts` `...feedback` exception is gone. The scan now refuses the words as literal keys too, which a spread-only rule would have let straight back in. It cannot see a renamed key, so the behavioural test below is the proof.

**Tests.** Every new test was watched RED on the old code first.
- `lib/actions/viewing-feedback-event-payload.test.ts` (new): the REAL action and the REAL `logEvent` over the fake transport. The row receives all four fields; ONE event with exactly `{ viewing_id, reference, rating }`; none of eight synthetic words (a name, a phone, an address, the opinions) anywhere in the inserted row; rating-only and admin saves; refused (0 rows) and failed updates, not-completed / not-yours / unreadable viewings and invalid input log nothing. RED at the old code: 4 of 8 (the payload carried six keys and all eight words).
- `lib/services/events.test.ts`: legacy payloads with the words (and a crafted one on a contact) render the bare line with stars; new payloads; `1e9`, `Infinity`, `2.5`, `0`, `-3`, `6`, a word and an object print no stars and never throw (`1e9` threw `RangeError` at the old code); the translator routes; `describeEventContext` labels current feedback.
- `lib/services/event-context.test.ts`: one viewings read on the viewer's client with the org filter; newest per viewing whatever the page order; no payload fallback for a missing row, null feedback or blank text; another property's viewing gets nothing; property events only; malformed ids never sent; a failed read logs a count and no words; tasks and viewings side by side.
- `lib/services/evidence.test.ts`: a property-scoped report with another buyer's legacy feedback prints only `Viewing feedback ★★★★` / `★★`, and never selects the feedback column.
- `supabase/tests/event-context.test.ts` (real database): admin, an agent who is not the viewing's agent, and a listing manager all read their own org's current feedback; the other org's admin reads only theirs; naming the other org buys nothing; an event naming another property's viewing gets nothing even for an admin.
- `tests/e2e/event-typed-text.spec.ts`: a completed viewing, an older-shape event WITH words inserted first, then the real form. The row holds all four fields, the event is exactly `{ viewing_id, reference, rating }`, the viewing page shows the words; the Activity tab shows the new line with `current feedback: <comment>` and the older line with its stars only; after "Update feedback" the newest line alone carries the NEW comment and the old one is nowhere; the admin feed shows the same, with no legacy words. RED at the old code (stash of the implementation): the stored payload carried three extra keys.

**Review** (an adversarial workflow over `6ed5c8e`: five lenses — logic, privacy, tests, i18n/UX, contract — each finding judged by three skeptics told to refute it, majority survives; then mutation proofs in a detached copy). No defect in the change itself; nine findings confirmed, all fixed here:
- **Letter case (2/3):** `z.guid()` admits an uppercase id, Postgres answers in lowercase, and the newest-per-viewing map was keyed by the raw string — a hand-made form post naming the viewing in uppercase made the OLDER save carry today's words (reproduced by two skeptics with a probe). `payloadId` now lowercases; the new test uses an id with hex LETTERS (a first version used an all-digit id, which `toUpperCase()` leaves alone, and passed on the old code too — caught by running it there).
- **Tests that could not fail (3/3 each):** the payload test's `.eq("id")` check matched the READ, not the UPDATE — now only filters chained after the update count; the evidence guard used non-uuid viewing ids, so no reader would ever have been asked — now real uuids and a scripted page of rows whose words the report would print, and wiring `attachCurrentTitles` into `assembleEvidence` was confirmed to turn it RED.
- **Stale record (3/3 each):** this entry and the BACKLOG strike did not exist yet; the `reportContentHash` docstring, `viewing-status-refused.test.ts` (its rationale and three test names said feedback was "published" to the timeline), and the caller comments in `event-timeline.tsx`, `admin-dashboard.tsx` and `entity-timeline.ts` named only a current title. All updated.
- **Erasure (2/3):** `viewings.feedback` is now the words' only home and erasure never touches it — recorded on the BACKLOG erasure-decision entry, not changed.
- **Rejected as this change's defect, recorded:** the `rescheduled` line crashes the same three surfaces on a forged `{ from: "x" }` (`formatDateTime` → `RangeError`). All three skeptics confirmed the crash and that it predates this change (`ebcfec5`) — a new BACKLOG item.
- **Mutation proofs:** 14 of 14 killed — the words back in the payload (explicit and as a spread), the renderer printing a comment, the old `isFinite && > 0` guard, attaching to every save, dropping the property match / the org filter / the property-only rule, a payload fallback, `liked` before `comment`, dropping the label, `disliked` out of FORBIDDEN, a missing Greek key, and reading viewings through the admin client.

**Measured** (the branch, local stack at 0115, 2026-09-25): tsc 0 (`--incremental false`); eslint 0 on the whole tree; unit 2710/2710 across 207 files; the database suite 370/375 — the 5 reds are the known residue on this stack (rls 41/42/57, `enquiry-alert-sweep-runs` connect_error, `enquiry-alerts-cron`), identical on `main`; `supabase/tests/event-context.test.ts` 11/11 (five new: every role of the org, the other org, naming the other org, another property's viewing); e2e 122/122 across nine specs touching viewings, feedback and the nudges (`event-typed-text`, `nudges`, `viewing-confirmation`, `viewing-reschedule`, `viewings-export`, `viewings-window`, `slip-pdf-hash`, `contact-merge`, `security`), desktop AND mobile; `npm run build` exit 0 and `check:static-routes` ok.

**Compatibility and deploy order.** No migration, nothing on hosted, not deploy-coupled: an old app renders a new payload as stars (its `comment` branch finds nothing); a new app renders an old payload as stars. A property-scoped evidence report generated BEFORE this change that included feedback words recomputes to a different `reportContentHash` (verification is by `pdf_sha256`, unaffected); hosted has 0 such events, so no such report exists. Order: branch CI green → merge → deploy READY → probes.

**Not changed here.**
- **Erasure (operator decision):** `viewings.feedback` is now the ONE home of the buyer's words, and contact erasure (`lib/actions/contact-erasure.ts`, `lib/services/erasure-run.ts`) never touches it — before and after this change. After an Article 17 erasure the words keep showing on `/viewings/[id]` and, as current feedback, on the property's Activity tab and the admin feed. Added to the BACKLOG erasure-decision entry; the policy is not changed here.
- **Evidence PDFs already generated** keep whatever lines they printed; hosted holds 0 feedback events, so none carries feedback words.
- **The same crash class in `rescheduled`:** a forged `{ from: "x", to: "y" }` makes `formatDateTime` throw `RangeError` on the same three surfaces. A different event type — on BACKLOG, not built here.

**Landing (2026-09-25, afternoon).** On the operator's word ("merge #65 and verify the deploy" — the PR number was named before it was opened; #65 was then the next number, and it is this change). Main had not moved (`fff6f96`, the PR's base). PR #65 → main `1402e9c`, pinned to `8c134a9` (branch CI green: checks, rls, e2e). Vercel production `dpl_Hp5S29Te7JrpgzTKrHyNbPNayZK7` READY; CI on the merge commit green (run 36125556523: checks, rls, e2e). Verified read-only in the operator's Chrome: the admin dashboard and the busiest property's Activity tab (latest 50 of 55 events) render with no error boundary and no feedback line — hosted holds 0 viewings and 0 feedback events, so there was nothing to render and nothing was created. `/login` 200, protected routes 307, no runtime errors. Nothing on hosted.

## T-imported-identity-shape — an imported person is named on the row, never in the chain (2026-09-25; no migration)

**The leak** (operator: "fix the importer identity leak next"; BACKLOG "More event payloads carry typed text by value" — the last TypeScript writer on that list). Inspected at `a4a1f43`, with a read-only mapping workflow (writers, consumers, test harness, precedent; a completeness critic re-read the migrations, the harness and the configs itself).
- `scripts/import/contacts.mts` logged `imported` with `{ name }` — the row's first + last name, else its company name (exactly `contacts.display_name`) — through `logImported` (`scripts/import/_shared.mts`, which spreads the caller's keys into `{ source: "csv_import", … }`, actor null).
- `scripts/import/properties.mts` logged the OWNER contact it creates with `{ name: name ?? phone, as: "owner" }`. **Correction found by the critic:** the phone fallback cannot reach a real database — `contact_has_name` (0001) refuses a nameless owner insert before the event is written; it was a latent path (the stand-in enforces no CHECK). The live leak on that path is the owner's NAME.
- The `imported` line printed `reference ?? name`: the contact's Activity tab (anyone who can open the contact), the admin feed, and an admin's un-narrowed evidence report (`events_select` hides actor-null rows from everyone else). Identity in an append-only chain that neither erasure (which never edits events and keeps identity on the row) nor an Article 16 correction can reach — the T-merged-event-ids-only rule.
- **Hosted, read-only, counts only:** 0 `imported` events. No chain copy exists.

**The fix.**
- **Writers:** a contact import logs `{ source, batch }`; the owner contact `{ source, as: "owner", batch }` (`as` is the role that created it, which the row also holds). The mandate `{ source, property, batch }` and property `{ source, reference, batch, visibility, score, published_at? }` events are unchanged — `reference` is the office's own code, not a person. The local import report keeps the name (gitignored, never committed, never in the chain).
- **Renderer:** `imported` reads no `name` from any payload, old or new, and prints `importedRef` only for a PROPERTY — so neither a legacy name nor a crafted `reference` on a contact reaches a line. No current-name join: the event's entity IS the contact, whose page header names it; the evidence header names it from the row; the admin feed names no contact for any contact event. (T-merged-event-ids-only joined only because its event names a DIFFERENT contact; joins were declined wherever the text shows elsewhere.)
- **No runtime allowlist in `logImported`.** The critic showed why: it runs AFTER its row is committed, and a guard that throws there lands in the per-row catch — rows with no `imported` event (a public property among them) and a report that says "error". `scripts/**` is outside tsc and eslint, so a type narrowing would enforce nothing. The proof is behavioural.

**Tests.** Every new test was watched RED first.
- `scripts/import/entry-points.test.ts` (the REAL scripts, run as the operator runs them, against a request-recording PostgREST stand-in): a person row, a company-only row, and a property row with a named owner. Positive controls: the name and the E.164 number reach the `contacts` INSERT. The `imported` bodies are pinned exactly — contact `{ source, batch }`, owner `{ source, as: "owner", batch }`, mandate `{ source, property, batch }`, property keys `{ batch, reference, score, source, visibility }` — and no event body carries the name, the number (spaced or not), the e-mail or the company. Case-SENSITIVE distinctive words: the kept `as: "owner"`, the `zz-entry-…` batch and the ZZ references would trip a case-folded check. RED at the old code: 3 of 3.
- `lib/services/events.test.ts`: legacy contact / company / owner / owner-with-phone payloads read "Imported from CSV"; new shapes; a property prints its reference (even beside a legacy name); a property without one, a mandate, and a crafted `reference` on a contact read bare; no line carries the name or number; the translator routes. RED: 8 of 13.
- `lib/services/evidence.test.ts`: an admin's un-narrowed report over three imported events (legacy name, legacy owner phone, new) prints three bare lines. RED at the old code.

**Review** (an adversarial workflow over `07e7a8c`: writers-completeness, renderer-consumers and tests lenses, three skeptics per finding, majority survives; mutation proofs in a detached copy). No defect in the change. Two confirmed findings, both records, fixed here: this entry and the BACKLOG strike did not exist yet, and the `events.ts` header index did not name the rule or `imported`'s entity gate. Rejected 0/3 as pre-existing and already recorded: the default `batch` label is the CSV's file name and goes into every `imported` event — kept as a BACKLOG NOTE for the operator's call. **Mutation proofs: 8 of 8 killed** — the name back (contact, owner), a company key, a phone key, an extra key added inside `logImported` (killed by the exact-shape pins), the old `reference ?? name` line, the entity gate dropped, and `importedRef` dropped.

**Measured** (the branch, 2026-09-25): tsc 0 (`--incremental false`); eslint 0 on the whole tree; unit 2727/2727 across 207 files (the importer harness included: vitest runs `scripts/**`). No browser flow imports a CSV (the importers are CLI scripts), and no page's behaviour changed except the `imported` line, pinned by the renderer tests.

**Compatibility and deploy order.** No migration, nothing on hosted, not deploy-coupled. The scripts run from a checkout, so the operator's next import after merge writes the new shapes. An old app renders a new payload as "Imported from CSV" for a contact (as it would any payload without a name); a new app renders an old one the same. An evidence report regenerated over a legacy `imported` line with a name recomputes to a different `reportContentHash` (verification is by `pdf_sha256`); hosted has 0 such events.

**Not changed here** (each on BACKLOG): `record_key_movement` (0013) writes a key's typed HOLDER — an owner's name on a transfer — into key events (hosted: 2 events carry one); `archive-records.mts` writes the operator's typed `--reason` into `archived` (hosted: 7); a phone-only owner row's dry run promises an owner the live run refuses, after creating a missing area; the default `batch` label is the CSV's file name (a NOTE, the operator's call).

**Landing (2026-09-25, evening).** On the operator's word ("merge #66 and verify the deploy"). Main had not moved (`a4a1f43`, the PR's base). PR #66 → main `22287d9`, pinned to `fd06051` (branch CI green: checks, rls, e2e). Vercel production `dpl_HUhUxxKYWGQjsp1h3s6Ws2VqNAYN` READY; CI on the merge commit green (run 36134760083: checks, rls, e2e). Verified read-only in the operator's Chrome: the admin dashboard and the busiest contact's Activity tab (13 events) render with no error boundary and no "Imported" line — hosted holds 0 `imported` events, so there was nothing to render and nothing was created. `/login` 200, protected routes 307, no runtime errors. Nothing on hosted.

## T-key-holder-shape — a key's movement event names the movement, never the holder (2026-09-25; migration 0116)

**The leak** (operator: "fix the key holder leak next"; BACKLOG, found by T-imported-identity-shape's scouting critic). Inspected at `0510047`, with a read-only mapping workflow (the SQL writer, consumers, callers and policy, precedent; a completeness critic re-read the migrations and the UI itself).
- `record_key_movement` (0013, never re-created; SECURITY DEFINER, `returns void`, authenticated-callable) logged every movement as `{ key_code, holder }`. `holder` was the TYPED name of an external holder on a checkout (a lawyer, a cleaner — nothing limits it to non-clients), the property OWNER's typed name on a transfer ("Owner name (optional)"), the same cached name copied again on a return or a loss, or a staff member's `full_name` by value. The pre-0013 app-side writer (T4.6, `78fd4c9`) wrote the same shape.
- The key lines printed it — "Key K12 checked out to …", "handed to owner (…)", "last with …" — on the property Activity tab (read as the system) and the admin feed.
- **Hosted, read-only, counts only:** 1 `key_checkout` and 1 `key_return` carry a holder; neither equals any staff member's name (typed); both written by 0013's RPC. The hosted function body is 0013's text exactly, with CRLF line endings (md5 `bf29a5e9…` = 0013's body with \r\n; LF md5 `15a8c83e…`).

**Verified, not assumed — why no new table.** The critic answered the one question that could have made this more than a removal: is the event the only custody record? No. `key_movements` gets one row per movement, written in the same transaction with the same name (0013's movement insert), and EVERY custody surface reads rows: the per-key History dialog (every role, on the register and the property page) and the /keys "Recent movements" list read `key_movements`; the register, the CSV export and the property page read `property_keys.current_holder_name`. The chain copy fed only a suffix on two event lines. Phase-1 spec (doc 02 §T4.6: every movement an event; holder and time recorded; the property page shows the holder) is still met: every movement still writes an event, the holder and time stay on the movement row, and the property page's holder comes from the cache.

**The fix.**
- **Migration 0116** re-creates the function from 0013's text with three changes and nothing else (a `diff` of the two bodies shows exactly them): a `v_movement_id` variable, `returning id into v_movement_id` on the movement insert, and the event payload `{ key_code, movement_id }` — the id is the fact, the row carries the name (the T-merged-event-ids-only idiom). Signature, defaults, `returns void`, SECURITY DEFINER, `search_path` and grants unchanged; re-stated. Apply-time assertions: one function by name; the signature, the void return, definer and search_path; no `'holder'` in the body, and the movement id both captured and logged; the ACLs; the first refusal without a session. Its header does not quote the removed line, so the BACKLOG VERIFY cannot hit a comment (the trap 0115's header set for its own VERIFY — fixed here by excluding SQL comment lines).
- **Renderer:** the four key lines read the code only — no `holder` from any payload, old, new or inserted by hand (`events_insert`, 0071, lets a member write any payload, which no body change can close). The six `*Holder` messages are gone in EN/EL/RU; `holder` left the parity test's sample values.
- **Kept byte-for-byte, recorded on BACKLOG:** no second-factor check in the function; a staff id matching no active profile NULLs the typed name (non-STRICT SELECT INTO); direct INSERT into `key_movements` and UPDATE of the holder cache bypass the RPC.

**Tests.** Watched RED first.
- `supabase/tests/key-movement-event-shape.test.ts` (new; the real RPC on real sessions): a checkout to an external holder, a return, a transfer to the owner then a loss, and a staff checkout. Positive controls: the typed name reaches the movement row and the holder cache (and survives a return onto the row, and a loss "for accountability"). Each event's payload is exactly `{ key_code, movement_id }` with `movement_id` = its row; a refused movement writes no row and no event; none of the typed names is in any of the run's key events. RED at 0115: 5 of 6 (the refusal control held).
- `lib/services/events.test.ts`: legacy payloads with a typed holder, the owner, a cached holder, a staff name, and a holder without a code read without a name; new payloads read the code. RED: 6 of 14.

**Review** (an adversarial workflow over `2a441c1`: SQL, consumers-and-UI and tests lenses, three skeptics per finding; no database touched). The SQL and tests lenses found no defect; one finding, confirmed 3/3 and fixed here: the `events.ts` header index of fields never printed did not name the key holder. **Mutation proofs** (the function re-created in a broken form on the LOCAL stack, the DB test run, 0116's exact text restored — prosrc digest `8d9dc7ba…` before and after): 4 of 4 killed — the holder back in the event (5 red), a `movement_id` naming another row (4), the row losing the holder's name (4), no movement id (4); the renderer's holder branch is pinned by the 6 RED renderer cases.

**Measured** (the branch, local stack at 0116, 2026-09-25): tsc 0 (`--incremental false`); eslint 0 on the whole tree; unit 2737/2737 across 207 files; the database suite 378/381 — the 3 reds are the known feed residue (rls 41/42/57), identical on `main`; `key-movement-event-shape.test.ts` 6/6 and RLS tests 13 and 18 (key history append-only; key events) green; e2e `keys-export` + `event-typed-text` 20/20 on desktop and mobile (no spec moves a key: the renderer tests carry the lines).

**Compatibility and deploy order.** Not deploy-coupled: same arity, `returns void`. An old app renders a new payload as "Key K12 checked out" (its holder branch finds nothing); a new app renders an old one the same. Hosted 0116 FIRST all the same — it stops new names reaching the chain soonest — then merge, deploy READY, probes. The evidence report has no key family, so no report hash moves.

**Not changed here** (BACKLOG): the three kept-as-is SQL behaviours above; the erasure decision now also covers `key_movements.holder_name` and `property_keys.current_holder_name` — a key's typed holder's only home, unlinked to any contact — and records backups as the real horizon; found on the way: four list exports write the raw URL query string into `exported` events, and a signed viewing slip's `signer_name` is in its event (hosted: 1, while its slip row is gone).

**Landing (2026-09-25, late evening).** On the operator's word ("apply 0116 to hosted and merge #68"). Hosted FIRST: its function body was 0013's with CRLF line endings (md5 `bf29a5e9…`); 0116 applied in one `execute_sql` with the file's text, its assertions passed; verified separately: body md5 `8d9dc7ba…` = local, one overload, definer / `search_path` / identity arguments / `void` / ACL unchanged, ledger by hand at 116, advisors unchanged (2 ERR + 36 WARN). Then PR #68 → main `5cebdfc`, pinned to `5906690` (branch CI green: checks, rls, e2e; main unmoved). Vercel production `dpl_DCCFCr3FRy3jQZcGij1qXtrL9AiJ` READY; CI on the merge commit green (run 36146101249: checks, rls, e2e). Verified in the operator's Chrome, the property holding both hosted key movement events renders its Activity tab (latest 50) with no error boundary; its two key lines read bare — the LEGACY checkout (which still carries a typed name in the chain) as "Key … checked out" and the return as "returned to office"; 0 lines with "checked out to", "handed to owner (" or "last with" (counts only, no text read). `/login` 200, `/leads` and `/keys` 307; no runtime errors after the deploy.

## T-rescheduled-line-crash — no crafted event can take a timeline page down (2026-09-25; no migration)

**The defect** (BACKLOG "A crafted `rescheduled` event crashes three pages", found by T-viewing-feedback-shape's review). Inspected at `a4a1f43`. Any active org member may insert any event — any type, any payload, any `occurred_at` (`events_insert`, 0071, checks only org and actor) — and the hash chain can never drop the row. `describeEvent` renders the property Activity tab, the admin dashboard feed and the commission evidence report, and it was unguarded. One event that breaks its line breaks those pages for everyone, for good. **Hosted, read-only, counts only:** 0 `rescheduled` events — nothing in production is broken today.

**What the sweep and the review found** (the first sweep, then an adversarial review — three lenses, three skeptics each, all nine findings confirmed):
- `rescheduled` (WF-1, `ebcfec5`) passed any non-blank `from` / `to` to `formatDateTime`: `Intl.DateTimeFormat#format(new Date("x"))` throws `RangeError`.
- **Not one line — a class.** The first sweep's battery held only primitives and plain objects, and on it `rescheduled` was the only offender. The review added what jsonb can also carry: an object with its OWN `toString` / `valueOf` (`{"toString":0}` — JSON.parse makes it an own data property, and `Number()` / `String()` then throw `TypeError`) and a deeply nested array (`String` recursion overflows the stack). Those broke **26 more lines** — every `Number(p.x)` count line, `asMoney`, `portalName`'s `String(p.portal)`, and `viewing_feedback`'s `Number(p.rating)` (so T-viewing-feedback-shape's "never throws" held only for its own battery). My first claim, "rescheduled was the only offender", was true only of the values I had chosen.
- **A prototype-named type** (`event_type: "constructor"`) resolved `EVENT_LINES["constructor"]` to `Object`, whose "line" is an OBJECT — no throw, so the new guard never saw it, and React failed rendering it. `__proto__`, `hasOwnProperty` and `valueOf` too; an `entity_type` of that name leaked a function's source into the prefix.
- **A crafted far-future `occurred_at`** (year 200000; Postgres allows it, and PostgREST returns text JavaScript cannot parse) threw in `formatDateTime` OUTSIDE `describeEvent` — the timeline, the key-movement list, the evidence preview and PDF.

**The fix.**
- `rescheduled` formats only a value that parses (`asDateText`), else "Viewing rescheduled".
- `asNumber` coerces only a number or a string; every `Number(p.x)` in `EVENT_LINES` (20 sites) and `asMoney` go through it; `portalName` takes a string or nothing. A crafted value reads as missing, which every line already handled.
- `describeEvent` renders only a type the registry OWNS (`Object.hasOwn`, the same for the entity-prefix map), requires the line to be a string, and a line that throws or returns anything else reads as the bare event type. The console line and a Sentry `captureMessage` carry tags only — the type if registered, else `"unregistered"` (an unregistered type is text anyone typed), and the error's NAME (a message can quote the payload). The guard is a last resort: the sweep fails for any registered line that reaches it.
- `formatDateTime` / `formatDate` read an unbuildable date as "—", as `formatNumber` / `formatArea` already read a non-finite number. Every place an event's time is shown goes through them.

**Tests.**
- `lib/services/event-lines-crafted-payload.test.ts` (new): every registered type (read from the source; a broken reader fails loudly) × every entity type × EN/EL/RU through the real translators, fed a Proxy payload whose every key reads one crafted jsonb value — 30 of them: words, an impossible date, "NaN"/"Infinity", a 100k string, extreme doubles, fractions, booleans, null, objects, arrays, the own-`toString` objects, a 5000-deep array, an absent value, and the strings "constructor" / "__proto__" / "toString" / "hasOwnProperty" — then 30 mixed rotations across keys, the discriminators (`kind` / `reason` / `action` … answer every literal the source compares them with, every other key a crafted value — the branches a uniform value never reaches), and an empty, null, array and string payload. It fails on a throw, a non-string, or a guard hit. Prototype-named event and entity types must read as a string with nothing of `Object.prototype` in it.
- `lib/services/events.test.ts`: `rescheduled` with words, a date beside a word, an impossible date, "NaN"/"Infinity", numbers and objects reads bare; two real dates still format; a translator that throws on one key makes that line read bare, leaves the next alone, and logs and reports `{ event_type, error }` with none of the payload; an unregistered type is reported as `"unregistered"`. The old `not.toThrow` tolerance test now also fails on a guard hit (the guard had made it unfalsifiable).
- `lib/utils/format.test.ts`: a word, an impossible date, "200000-01-01T00:00:00+00:00" (as PostgREST returns it) and an invalid Date read "—" in both formatters; a real timestamp still formats.
- RED at the old code throughout (14 at the first sweep; then 61 cases and 27 offending types on the extended sweep, and the formatter cases). **Mutation proofs: 8 of 8 killed** — the own-key lookup (4 red), the prefix own-key lookup (survived at first: the translator does not throw on `Object`, it prints "events.function Object() { [native code] }" — the prototype test now forbids that text, then 5 red), `asNumber` back to `Number` (57), `portalName` back to `String()` (34), the date check (6), the formatter check (4), the Sentry report (2), the raw type in the report (1).

**Measured** (the branch, 2026-09-25): tsc 0 (`--incremental false`); eslint 0 on the whole tree; unit 2855/2855 across 208 files (after rebasing onto `5cebdfc` — T-imported-identity-shape's and T-key-holder-shape's tests included, and the sweep over the new key lines); e2e 18/18 for `viewing-reschedule` + `event-typed-text` on desktop and mobile; a wider set of nine specs (those two, `keys-export`, `modules`, `csp`, `enquiry-contact-suggestions`, `mandate-lifecycle`, `performance`, `security`) 192/194 on desktop and mobile — the 2 reds are `modules` "Leads / Properties — loads clean" on MOBILE, a horizontal overflow (469 and 607 px in a 390 px viewport) that fails identically on untouched main `22287d9` (the known local-residue header overflow); `npm run build` exit 0 and `check:static-routes` ok.

**Compatibility and deploy order.** No migration, nothing on hosted, not deploy-coupled. Real payloads render exactly as before (a number or a numeric string is coerced as `Number` did; a boolean count, which no writer produces, now reads as missing rather than 1). A line that could not render now reads bare instead of failing the page; a date that could not be built reads "—". An evidence report could not be generated at all over such an event, so no stored report hash moves. Order: branch CI green → merge → deploy READY → probes.

**Not changed here** (on BACKLOG): the database still accepts a crafted `occurred_at` (the row now renders "—" but sorts first in "latest" lists — a CHECK or a stamping trigger needs a migration); `attachNotes` sends payload `note_id`s unchecked, so one malformed id blanks every note on a timeline (a degradation, not a crash).

**Landing (2026-09-25, night).** On the operator's word ("merge #67 and verify the deploy"). Main had moved twice since the branch was cut (the #66 landing docs, then #68 — which changed `events.ts`, its tests, the locales, BACKLOG and DECISIONS): rebased onto `5cebdfc`; the code applied cleanly, the two docs conflicts were both-sides appends and were kept in order; re-verified on the rebased tree (tsc 0, eslint 0, unit 2855/2855 — the sweep now covering #68's key lines); branch CI green on `a4241c6`. PR #67 → main `48a31cd`, pinned to `a4241c6`. Vercel production `dpl_Apxi5hkh3ueQDZLEMT4wkCsaoNRY` READY; CI on the merge commit green (run 36149107129: checks, rls, e2e). Verified in the operator's Chrome (counts only, no text read): the admin dashboard, the busiest property's Activity tab (50 lines: 36 photo, 2 key — as before), the busiest contact's Activity tab (13) and /keys render with no error boundary; every event time formats (the dashboard 10, the property 50, the contact 13; 0 read "—"); 0 lines fell into the guard (none reads as a bare lowercase type); the /keys em dashes are empty Holder cells of keys in the office. No runtime errors; no new Sentry issue in the two hours after the deploy (so no "[timeline] line failed" report). Hosted holds 0 rescheduled events — nothing crafted was created to exercise the guard.

## T-atomic-deal-close — closing a deal is one transaction; a competing or repeated close changes nothing (2026-09-25; migration 0117)

**The defect** (BACKLOG "`markDealLost` and `markDealWon` do not fold the open-status check into their UPDATE"; the brief asked for independent verification before any build). Inspected at `6366ef8` — the commit the previous audit examined; `main` had not moved. Mapped read-only first (six readers: the deals model and its policies, the events chain, every writer of closing state and any reopen path, the follow-ups and the UI, the test harnesses, the written rules). Both actions read `status = 'open'` on the caller's client, UPDATEd the row `.eq("id")` only, then wrote their events as separate requests through `logEvent`, which THROWS — after the update had committed. Nothing in the database stopped any of it: no CHECK ties status to its timestamps, `deals_update` (0100) restricts rows not columns, and no trigger guards a transition. No reopen or correction workflow for deals exists anywhere.

**Reproduced before anything changed — the real actions against the real stack.** `supabase/tests/deal-close-actions.test.ts` (new) runs the REAL `markDealWon` / `markDealLost` with supabase-js clients signed in at aal2, so every read and write goes through PostgREST, RLS, the triggers and the hash chain; only `revalidatePath` is stubbed. Interleaving is held by a gate on each client's fetch that HOLDS the action's first write until the test releases it — against `6366ef8` that write came after the status read, so "overlapping" meant exactly "both read the deal as open first". Never a sleep. Failure injection is real: a BEFORE INSERT trigger on `events` refusing one event type for one entity of the throwaway organisation, removed in `finally` (`trg_events_hash` untouched). At `6366ef8`, 10 of 11 red, each with the database's own evidence:
- two overlapping Lost requests: both "saved", **two `lost` events**;
- overlapping Won then Lost: the deal ended **lost, still carrying `won_at` and `final_value`**, one event of each kind; Lost then Won: **won, carrying `lost_at` and the Lost reason**;
- two overlapping override Wins: **two `won_override` and two `won`**, the later price standing;
- the `lost` event refused: the deal **stayed lost with no event** and the action threw; `won_override` or `won` refused: **won with no event**, or with `won_override` alone;
- two overlapping Wins on a listing with a live hold: **two `listing_status_check` prompts**;
- a follow-up event refused after the close: the action threw — an error page for a deal that WAS won;
- a sequential Won then Lost WAS refused ("Deal is already won") — the gap was the read-then-write, as BACKLOG said.
And the UI: the same outcome spec run against `6366ef8`'s app (the new spec copied into the main checkout for the run, then removed) was 4 of 4 red — among them, **the success toast never showed**: the page renders the Won/Lost dialogs only while the deal is open, the close revalidates it, and the effect that toasted ran in a component the refresh had already unmounted.
Classification: the action behaviour is confirmed with the real code on the real database; the database behaviour — no safeguard — against the actual schema; production occurrence is unknown (hosted holds test data only, and no production count was taken for this).

**The design was critiqued before it was built** (three adversarial lenses — PostgreSQL/RLS semantics, security and privacy, compatibility/UX/follow-ups — with rolled-back SQL experiments). What it changed: `FOR UPDATE` conflicts with the `FOR KEY SHARE` of every foreign-key check, so a close would block task and offer inserts on its deal and could deadlock the nightly nudge sweep through one — **`FOR NO KEY UPDATE`**; a guard freezing the reason for every role would pre-empt the open erasure decision — **it binds user sessions only**; letting a user session reopen let two PATCHes flip a closed deal with no event — **refused**; `numeric` admits NaN and it passes `>= 0` — **refused, typed or from the offer**; PostgreSQL's `\s` strips NEL and keeps the BOM, the opposite of JavaScript — **an explicit class, compared with `trim()` on every BMP code point: 0 mismatches**; an answer with no error code may have committed — **reported as unknown, never "nothing changed"**; `raiseLiveHoldCheck` swallowed its failures into a `0` that read as "no hold" — **a discriminated outcome**.

**The fix.**
- **Migration 0117 — `close_deal(p_deal_id, p_outcome, p_final_value, p_lost_reason, p_override) returns jsonb`**, SECURITY INVOKER, modelled on `move_deal_to_stage` (0067): `deals_update` / `deals_select` / `offers_select` / `events_insert` and `require_aal2` apply unchanged to a DIRECT call. Identity from the session only (`auth.uid()`, `mfa_satisfied()`, `current_org_id()`, `current_role_gnk()`); inputs validated as the form validates them, NULLs refusing; the role checked before any deal is read (no existence oracle: another agent's deal, another organisation's and a missing one all read "Deal not found"); the row locked `FOR NO KEY UPDATE` and its status checked UNDER the lock; both UPDATEs also conditional on `status = 'open'`; every event of the close in the same transaction, so a refused event rolls back the status, stage, timestamps, price, reason and the nudge supersession the status change fires. A deal already closed is untouched and answered `already_closed` (same outcome) or `conflict` (the other) — keyed on the row's status, never on "a terminal event exists": idempotency per open lifecycle. Rules kept: an accepted offer, or an admin's explicit override (`won_override` before `won`); `final_value` typed, else the offer's amount, else null; the type's is_won / is_lost stage (lowest `sort_order` — the schema allows several), else the stage kept; the reason on the row, never in an event. Additions: `won` names the justifying `offer_id` (an id — SEC-03 allows it); the offer is read in the deal's own organisation, newest first as the dialog prefills, `FOR SHARE`; a close clears the other outcome's columns. EXECUTE: authenticated only.
- **`deals_closed_guard`** (BEFORE UPDATE on `deals`): for a USER SESSION on a row ALREADY won or lost, refuses the other outcome, reopening, and any change to `won_at`, `lost_at`, `lost_reason`, `final_value`, `stage_id`, `stage_entered_at`. That covers the hosted-first window: the deployed actions' competing second UPDATE now fails before their `logEvent`. It does not bind service_role, postgres or definer bodies — the operator's correction path and a future reopen RPC's. Contact erasure writes on the admin's USER session (only notes and storage use the service role), so the pending decision on blanking `lost_reason` must put that step on the admin client or a definer function — pinned by a test, recorded on the erasure entry.
- **The actions** validate the form and call `close_deal`; they never throw. `closed` runs the Won follow-ups (the listing-status prompt, the live-hold ask, the health recompute) — only there, so a double submit, a retry or a competing close cannot raise them twice; each follow-up that fails AFTER the commit becomes a caveat on a success that says what is known (the reminder may not have been created — check by hand; or it was, and only its timeline line failed — tell an admin, the NOT_RECORDED_NOTICE convention), paged to Sentry by shape. `already_closed` is a saved state flagged `alreadyClosed`; `conflict` is an error naming the outcome that committed; a P0001 shows its own sentence; an error that proves a rollback (a SQLSTATE outside class 08, PGRST1xx–3xx, PGRST002/003) says nothing changed; anything else — no code, a gateway body, class 08, an unreadable answer — is UNKNOWN: the page is refreshed and the user is told, in the one text `unconfirmedCloseText` (`lib/validators/deals.ts`) shared with the dialog, to check the listing and any live hold if the deal now reads Won.
- **The dialogs** submit through a transition and toast from it (the review-and-link pattern), so a message outlives the refresh that unmounts them; everything that arrives with a refresh (`pageRefreshed`: a conflict, a repeat, an unknown answer) or with no answer at all stays until dismissed; `onSubmit`, not a form action, so a refusal does not wipe a typed reason; a refusal is cleared when the dialog closes.
- **Readers:** the deal page, the entity timeline and the admin feed break `occurred_at` ties by id — `won_override` and `won` now share one transaction time.

**Tests.** Watched RED first where a test could exist before the code.
- `supabase/tests/deal-close-actions.test.ts` (11) — the real actions on the real stack: the reproduction above, now green (the before/after evidence lines are in the PR). Its header says what the gate does and does not control against the RPC actions.
- `supabase/tests/deal-close.test.ts` (51) — the database: ordered races on two impersonated `pg` sessions (A closes and holds; B OBSERVED blocked on the row lock through `pg_stat_activity`; A commits or rolls back; B returns) for Lost/Lost, Won/Won with and without an offer, Won-then-Lost, Lost-then-Won and a rolled-back first close; a PostgREST race; repeats; the refused-event rollback for `lost`, `won_override` and `won`, nudge included; the rules by direct call (another deal's offer, another organisation's planted offer, a NaN amount, a typed price, zero); the reason trimmed as JavaScript trims and measured in UTF-16 units; the exact payloads, and not one word of the reason in ANY event the close wrote in ANY organisation, on the Lost path and on a Won after a maintenance reopen; the roles (listing manager, another agent, another organisation, aal1, deactivated, anon, service_role); the guard against the deployed `6366ef8` actions' exact requests, the user session refused (blanking the reason included), the maintenance path, a reopened deal's second lifecycle; the chain verifying.
- Unit: `deal-won-close.test.ts` (18, new), `deal-lost-event-payload.test.ts` (7, rewritten to the rpc-mock pattern — the reason leaves the action once, as `p_lost_reason`, and the action writes nothing itself), `followup-tasks.test.ts` (the discriminated outcomes), `entity-timeline.test.ts` (the tie-break).
- `tests/e2e/deal-close-outcomes.spec.ts` (5, new): the success toast, a colleague's close that committed first, a repeat, a follow-up failure after the commit (the caveat outlives sonner's 4 s and is dismissed by its button), and a Won whose request never comes back.
- **Mutation proofs:** SQL 11 of 12 killed — no status check or fold (13 red), no row lock (6), an event failure swallowed (2), the reason in the `lost` event (2), NaN from the offer (1), the other outcome's columns kept (1), locale trimming (1), the role check skipped (1), the guard allowing reopen (1), binding the maintenance roles (2), swapping outcomes (2); the one survivor — the offer read without its organisation predicate — is equivalent under RLS (`offers_select` already hides a foreign offer from an invoker call; the predicate is belt and braces). App 10 of 10 killed. Every mutant confirmed installed, every function restored to its digest.
- The apply-time SEC-03 tripwire read two of the three event inserts (a semicolon in a comment ended its match); it now strips comments and must read all three — fed the reviewers' leak (the row's earlier reason in the `won` payload) it refuses the migration, where the old regex stayed silent.

**Review** (an adversarial workflow over the full diff: SQL, actions, UI, tests, docs/compatibility lenses, two independent refuters per finding). 18 findings: 15 confirmed by both, 2 split, 1 refuted. All acted on: the dialog's own "could not confirm" text lacked the Won caveat in the one case the docs said it covered (a function killed after the commit), and the must-act toasts faded after 4 s — one shared text, persistent toasts, an e2e case; a refusal survived reopening the dialog (its clear-on-open branch could never run); the persistence assertion could not fail; the SEC-03 tripwire above; three written claims were false ("cannot deadlock" — a rarer task/chain-lock cycle predates 0117 and remains, now recorded; "erasure runs on the admin client" — it runs on the user session; the offers entry's mechanism); two VERIFY greps could never report fixed; the action-test header overclaimed what its gate controls. The refuted one (database text in console breadcrumbs) was applied as hardening to the new line only.

**Measured** (the branch, local stack at 0117, 2026-09-25): tsc 0 (`--incremental false`); eslint 0 on the whole tree; unit 2876/2876 across 209 files; the database suite 440/443 — the 3 reds are rls 41/42/57, identical on untouched `6366ef8` against the same database (the known local feed residue); `deal-close.test.ts` + `deal-close-actions.test.ts` 62/62; e2e desktop 15/15 (`deal-close`, `deal-close-outcomes`, `event-typed-text`, setup); `npm run build` exit 0 and `check:static-routes` ok. On MOBILE the specs whose deal has an offer fail at the dialog's button — exactly as `deal-close.spec` fails on untouched `6366ef8`: the deal page overflows a phone once it lists an offer (BACKLOG, new; CI runs desktop only).

**Compatibility and deploy order.** Additive: a function and a trigger; no enum, no column, no changed signature. **Hosted 0117 FIRST, then merge** (the working agreement). In the window the DEPLOYED app still closes an open deal exactly as before (its PATCH is legal — pinned with its exact requests), while its race cases are refused by the guard (the loser's dialog shows "Deal is already won — it cannot be marked lost"; nothing overwritten, no second terminal event); what it keeps until the deploy is the non-atomic event write. The NEW app against a database WITHOUT 0117 fails closed: PGRST202 → "Could not close the deal — nothing was changed", paged. No `release-compat-contracts` entry: that harness is the enquiry door's, and 0117 changes no function the deployed app calls.
**Rollback.** Revert the app first (merge a revert, confirm the deploy) — safe on its own: the old actions work against 0117 and the guard keeps their races refused; only atomicity returns to the old shape. Only then, if ever, drop the objects — through a FORWARD migration that also removes the two `verify-restore.sql` grants rows and moves the migrations pin, never by hand. Dropping them while the new app is deployed makes every close answer PGRST202 (fails closed: no deal can be closed). Dropping them reopens the original race for the old actions. No data migration either way: rows and payloads written by `close_deal` have the old shapes (plus `offer_id`).

**Not changed here** (BACKLOG): a direct write can still close an OPEN deal without `close_deal` (deploy-coupled, code first); the Won reminders are at-most-once; NaN is a legal price in three columns; `move_deal_to_stage` still takes FOR UPDATE; the rare task/chain-lock cycle between a close and the nudge sweep; the deal-nudge supersession writes across organisations; offers stay editable on a closed deal; the dashboard's "won this month" lost `final_value` in 0093; on a phone a deal with an offer cannot be closed from its page; a superseded deal nudge renders the generic line.

**Landing (2026-09-25, late night).** On the operator's word ("go ahead", then "merge pr 69"). Branch CI green first (run 36169435034: checks, rls, e2e — the three local rls reds were residue, as expected). Hosted FIRST: 0117 applied in one `execute_sql` with the file's text, its assertions passed; verified separately: `close_deal` and `trg_deals_closed_guard` bodies md5 = local (`cb70a918…`, `7ee172ae…`), invoker with `search_path=public`, one overload each, `close_deal` executable by authenticated only, the guard body by nobody, the trigger bound BEFORE UPDATE FOR EACH ROW; ledger by hand at 117; aal2 gaps 0, partition health 0, the chain verifies; advisors 2 ERR + 36 WARN unchanged; an anon PostgREST call answers 42501. The first `gh pr merge` was refused by the session's auto-mode classifier; the operator then asked for the merge explicitly. PR #69 → main `aa01ef8`, pinned to `9c98512`. Vercel production `dpl_7BFFpRnoN9cDzuk2XquP7A9Yn7FL` READY, aliased to gnk-crm.vercel.app; no runtime errors after it. CI on the merge commit (run 36176646388): checks and rls green; e2e was still running when this record was written (read it with `gh run view 36176646388 --json jobs`) — it completed green (checks, rls, e2e all success; read 2026-09-25 by T-deal-close-db-boundary). Verified in the operator's Chrome (counts only): the one hosted deal (lost) renders with its banner and no close buttons, and the admin dashboard renders. No deal was closed in production.

## T-deal-close-db-boundary — a deal reaches won or lost only through `close_deal` (2026-09-26; migration 0118)

**The gap** (BACKLOG "A direct write can still close an OPEN deal without `close_deal`", left open by 0117 on purpose because refusing it was deploy-coupled; the brief asked for independent verification first). Inspected: gnk-crm `main` = `3a72498` (= the deployed `aa01ef8` plus docs), gnk-web `main` = `b158f37` (it has no writer of deals; its only hit is a test asserting it holds no service key). No open pull request on either repository; the three stale remote branches on gnk-crm do not touch deals. Mapped read-only first (five readers: every app and script writer of `deals`, the database layer — policies, grants, triggers, every SQL writer —, the test harness, the written rules and pins; then a completeness critic).

**Reproduced before anything changed** — the local stack at 0117, real PostgREST, an aal2 agent and an aal2 admin on a throwaway organisation (script outside the repository, cleaned up):
| attempt | result at 0117 |
|---|---|
| PATCH an open deal with no accepted offer to `{status: won, won_at}` | 200 — won, no event; `close_deal` then answers `already_closed` and the 0117 guard refuses any user-session correction |
| PATCH to `{status: lost}` with no reason / with `lost_reason: 'x'` | 200 — lost, no reason (or one letter), no event |
| PATCH closing details onto a deal that stays open (`won_at`, `lost_reason`, `final_value`) or into its Won / Lost stage | 200 each (the kanban's RPC refuses the stage move; a PATCH did not) |
| POST a deal born won or lost, or open with closing details, or open in the Won stage | 201 each |
| UPSERT (`on_conflict=id`, merge) an existing open deal to won; a new id born won | 200 / 201 |
| controls: `close_deal` Won with an accepted offer, Lost with a reason; an agent's Won without an offer | closed / closed / refused |
| a deal already won: PATCH to lost or open, upsert to open or lost | refused (0117's guard) |
Two adjacent facts from the same run, recorded rather than fixed here: the same session can POST hand-written `won` / `lost` events (`events_insert` checks only organisation and actor), and `deals_insert` does not bind `agent_id` / `created_by` to the caller.

**The dependency, confirmed and not assumed.** Production `gnk-crm.vercel.app` is `dpl_7BFFpRnoN9cDzuk2XquP7A9Yn7FL` at `aa01ef8` (Vercel API, alias read 2026-09-25); `aa01ef8:lib/actions/deals.ts` closes only through `rpc("close_deal")` (its only other deal write after a Won is the health recompute); the merge commit's CI was fully green (run 36176646388: checks, rls, e2e). Hosted (read-only `execute_sql`): ledger at 0117 (117 rows, non-filename versions 0), `close_deal` md5 `cb70a918…` and the guard `7ee172ae…` as recorded, owner `postgres` (BYPASSRLS, not superuser), pg_graphql not installed, `authenticated` cannot create in `public`; ONE deal, lost, with `lost_at`, a reason, its Lost stage and its `lost` event; no open deal with a closing detail or a terminal stage, no closed deal without its event, no Won without an offer or an override. Nothing needs repair and nothing was repaired.

**Options compared.**
| option | benefit | effort | permission-regression risk | verdict |
|---|---|---|---|---|
| a transaction-local flag `close_deal` sets and the guard reads (BACKLOG's first suggestion) | keeps the invoker body | small | `set_config` is executable by PUBLIC; a custom setting outlives the statement (and reads `''`, not NULL, on a pooled connection afterwards — measured) — a freely settable flag is not authorisation | rejected |
| column privileges (revoke UPDATE / INSERT on status and the closing columns) + a definer `close_deal` | engine-enforced | medium: the TABLE grant must go first, every later column needs a grant, hosted's default grants differ from local (HANDOFF §4.2) | high; and a privilege cannot say "a non-terminal stage of your own organisation", so a trigger is needed anyway | rejected |
| a deferred constraint trigger checking at commit that a terminal event exists in the transaction | keeps the invoker body | medium | low, but the rules live in two places and "close_deal ran" becomes an inference from side effects (and events are user-insertable) | rejected |
| **`close_deal` SECURITY DEFINER + the guard keyed on the ROLE the statement runs as** | the role comes from the signed JWT; the maintenance path and definer bodies stay unbound, as 0117 and the erasure decision require | small: one function re-created, one trigger function widened | the definer loses RLS — mitigated by restating `deals_update` explicitly, re-reading the caller under the lock, a differential test and apply-time assertions | **chosen** |

**The fix — migration 0118.**
- **`close_deal` becomes SECURITY DEFINER** (owner postgres, `search_path = public, pg_temp`, every relation schema-qualified; EXECUTE authenticated only, as before). Its body is 0117's with what RLS used to supply restated: the lock query carries `deals_update`'s USING clause (an admin of the organisation, or an agent who is the deal's `agent_id` or `created_by`; its WITH CHECK holds whenever USING does — a close changes neither organisation nor owner); **step 4b re-reads the caller under the lock** — aal2, organisation, role, ownership — because 0117's RLS re-read the profile at every statement after the lock, so an admin demoted (or an agent deactivated) while the close waited must still be refused (measured: without 4b the demoted admin's override Won committed); 0117's "defensive" `exists (...)` probe is removed, because as postgres it would have told another agent's deal apart from a missing id — every deal the caller may not close reads "Deal not found", and the two unreachable "You do not have permission to close this deal" sentences read the same. Rules, payloads, answers, grants and the signature are unchanged, so no `release-compat-contracts` entry and no type regeneration.
- **`deals_closed_guard`** fires BEFORE INSERT OR UPDATE (`search_path = public, pg_temp`, `public.deal_stages` qualified) and, for a user session (`current_user` authenticated / anon), refuses on an OPEN row any change of `status`, any change to `won_at` / `lost_at` / `lost_reason` / `final_value` (a CHANGE, compared with OLD — a maintenance reopen leaves the previous lifecycle's values on the row, and an ordinary edit carries them), and a move to a stage that is not a non-terminal stage of the deal's own organisation; on INSERT a deal that is not open, carries a closing detail, or starts in such a stage. The closed-row rules are 0117's text. service_role, postgres and definer bodies are not bound — the correction path, a maintenance reopen, and now `close_deal`.
- **Apply-time assertions:** definer + owner + search_path; the restated predicate in the lock query; the re-read under the lock; no existence oracle; both UPDATEs folded on `status = 'open'`; the SEC-03 tripwire (no event insert names the reason); grants; the guard an invoker with `pg_temp` last; the trigger bound BEFORE INSERT OR UPDATE FOR EACH ROW; **no SECURITY DEFINER function other than `close_deal` writes `deals`** (the guard exempts definer bodies, so any other would be a second door); a no-session probe. Measured by the mutation runs: they refuse every `close_deal` mutant tried (M7–M11, M16–M19b, M22).

**Tests.** Watched RED first: with the final files, 54 of the 123 tests in the two database files fail against 0117's exact bodies (installed from 0117's own text, md5 = hosted) — every one a 0118 test or the definer flag; the differential persona test PASSES there, which is itself the evidence that the restatement matches 0117's RLS. (A first draft's "cannot switch to a maintenance role" test was a harness mistake and was removed: `SET ROLE` checks the login role, and the harness logs in as postgres.) `supabase/tests/deal-close.test.ts`:
- the KNOWN GAP test flipped into a regression (refused; row and chain byte-identical); the pre-`aa01ef8` app's exact PATCHes (Won and Lost) refused with nothing written;
- a PATCH matrix, agent and admin (to won with a full payload, to lost with no / a one-letter reason, each closing detail alone, into the Won / Lost stage, into another organisation's Won and open stages), a bulk PATCH refused as a whole;
- an INSERT matrix, agent and admin (born won / lost, open with each closing detail, open in a terminal or foreign stage) and the ordinary insert (convertLead's shape);
- UPSERT: onto an open deal (the INSERT arm refuses before the conflict is resolved), onto a closed deal (stage move refused; an explicit reopen refused), a new id born won, an ordinary merge;
- forged context: `request.jwt.claims` claiming service_role, three made-up session flags, `set_config` over REST (PGRST202), and — labelled as the assumption the guard stands on — a token the project did not sign (401 before any SQL);
- a direct PATCH queued behind a held close (commit: meets the closed row; rollback: the open-row rule);
- a DIFFERENTIAL persona test: `close_deal` succeeds exactly where a PostgREST PATCH of an ordinary column succeeds, each refusal pinned to its rule — the deal's agent created by an admin (the `agent_id` arm alone), the creator who is not the agent (the `created_by` arm alone), an admin; not another agent ("Deal not found"), a listing manager ("You do not have permission to close deals."), another organisation's admin, another organisation's user named as the deal's agent, an aal1 session ("Second factor required."), anon (42501), a deactivated account ("Account deactivated.");
- the caller re-read under the lock: an admin demoted, and an agent deactivated, while their close waits — refused, nothing written, then the control succeeds;
- the definer's owner and search_path; the catalogue tripwire (only `public.close_deal` among definer functions writes deals); the maintenance path.
Every 0117 test (races in both orders, repeats, the refused-event rollback incl. the nudge, the rules, inputs, roles) now runs against the definer body unchanged. `deal-close-actions.test.ts` adds the REAL `convertLead`, `updateDealSection`, `logDealContact` and `moveDealToStage` — the app's user-session writers of deals — still succeeding, and the kanban still refusing a Won stage.

**Mutation proofs** (a harness that refuses to install unless the database matches a recorded fingerprint, installs by exact string edit, confirms the mutant installed, runs the two files, and restores to the fingerprint in `finally`). First round, 16: 15 killed; M5 (the guard's stage lookup without its organisation predicate) survived and is EQUIVALENT — the guard runs as the caller, `deal_stages_select` already hides other organisations' stages, and a rolled-back probe refused the same writes. Second round, after the review fixes: M17 (step 4b removed), M18 (the `agent_id` arm dropped), M19a (a precedence slip in the lock query alone — EQUIVALENT: step 4b's organisation check still refuses), M19b (the slip plus 4b's organisation check removed), M20 (the INSERT arm's `lost_at` check removed — the survivor of the first round's extra M12b, now covered), M22 (4b keeps the stale role): M17 (step 4b removed — killed by the two lock-wait tests), M18 (the `agent_id` arm dropped — killed by the differential test), M19b (the slip plus 4b's organisation check removed — killed by "another organisation's user named as the deal's agent"), M20 (the INSERT arm's `lost_at` check removed — the first round's extra survivor M12b, now killed) and M22 (4b keeping the stale role — killed by the demotion test) killed; M19a (the precedence slip in the lock query alone) survived and is EQUIVALENT: step 4b's organisation check refuses what the slipped query admits. The migration's own DO block refused every `close_deal` mutant of both rounds (M7–M11, M16–M19b, M22); it does not read the guard's body, and the tests are that proof (M1–M6, M12–M15, M20 killed by them). A planted SECURITY DEFINER function writing deals is refused by both the tripwire test and the DO block.. Every mutant was confirmed installed and every restore returned to the fingerprint.

**Review** (an adversarial workflow: four lenses — the definer restatement, a bypass hunt, rollout and written claims, test quality — with two independent refuters per finding). Acted on: the profile re-read under the lock (the mechanism was reproduced by both refuters; they judged it "not a defect", this record judges it a parity gap the brief names and closes it); `pg_temp` last and the guard's unqualified `deal_stages` (a raw SQL session could shadow it — no PostgREST caller can); the differential test never isolated the `agent_id` arm and pinned no refusal reason; no INSERT carried `lost_at` alone and no INSERT ran as admin; no upsert reopened; a direct PATCH queued behind a close was untested; three written claims overstated ("messages unchanged", "every refusal after the role check", "no other deal write"); the advisor +1 was unpredicted; the rollback recipe and the application floor were unwritten. Confirmed and recorded, not changed here (pre-existing): `created_at` / `stage_entered_at` / `deal_type` are writable by a direct PATCH (reports read them); an admin may flip a stage's `is_won` / `is_lost` with open deals in it; a chosen-id POST tells whether another organisation's deal id exists (the generic RLS + ON CONFLICT answer). Refuted: nothing reached the goal — no path to won / lost without `close_deal` was found.

**Measured** (the branch, local stack at 0118): tsc 0 (`--incremental false`); eslint 0 on the whole tree; unit 2876/2876 across 209 files; the database suite 501/504 — the 3 reds are rls 41/42/57, the public-feed residue on this machine's database (the feed's first 50 rows are residue listings), identical when `main`'s own test file runs against the same database; `deal-close.test.ts` + `deal-close-actions.test.ts` 123/123 (54 of them red against 0117's exact bodies); Playwright desktop 28/28 (`deal-close`, `deal-close-outcomes`, `event-typed-text`, `nudges`, `happy-path`, `deals-export`, setup); `npm run build` exit 0 and `check:static-routes` ok; the restore pack's `close_deal` grants row (true/false/true/false), the guard's (all false) and migrations 118 pass on the local stack. Branch pushed and PR [KALAITSIDIS/gnk-crm#70](https://github.com/KALAITSIDIS/gnk-crm/pull/70) opened; its CI is the fresh-database arbiter.

**Compatibility and deploy order.** Additive for the deployed application: no signature, return shape or enum changes. `aa01ef8` against 0118: closes through `close_deal` exactly as before (same answers, same messages the dialogs show), every other writer of deals it has is untouched (the real actions above). **Deploy order: hosted 0118 is a database-only change — apply it with the app at `aa01ef8` or later, then merge this branch (it carries no application code).** Visible to a DIRECT PostgREST caller only: a refused deal INSERT answers 400 / P0001 from the guard (it runs before the RLS WITH CHECK) where 0117 answered 403 / 42501; nothing in the app keys on either. Advisors: `close_deal` joins the by-design `authenticated_security_definer_function_executable` WARNs — expect 2 ERR + 37 WARN after the apply (36 at 0117).
- **The application rollback floor moves:** against a database at 0118, **no application older than `aa01ef8` can close a deal** — its `markDealWon` / `markDealLost` PATCH is refused before their event writes, so nothing is written and the dialog shows the database's sentence ("Deal is open — it is marked won only from the deal page (close_deal)"). A Vercel instant rollback past `aa01ef8` therefore fails CLOSED for closing and leaves everything else working. Stale browser tabs run no old code: a server action executes on the deployment that serves it.
- **Hosted apply recipe:** ONE `execute_sql` with the file's exact text (the DO block must run with it; `create or replace` keeps ACLs, the file restates them), then, separately: both bodies' md5 against local (`md5(replace(prosrc, chr(13), ''))`), `prosecdef` / owner / `proconfig`, ACLs (`close_deal` authenticated only; the guard body nobody), the trigger BEFORE INSERT OR UPDATE, the ledger row 0118 by hand, `non_filename_versions` 0, aal2 gaps 0, partition health 0, the chain verifies, advisors 2 ERR + 37 WARN, and an anon PostgREST call to `close_deal` → 42501. The DO block needs the functions owned by `postgres` — measured on hosted for 0117's two.
- **Existing data:** 0118 adds no constraint and validates no existing row (the guard judges a write, NEW against OLD). Nothing to check before applying beyond the invariants above.

**Rollback.** Through a FORWARD migration, never by hand: re-create 0117's `close_deal` (invoker) AND 0117's guard body AND the BEFORE-UPDATE-only trigger **in one transaction** (0117's file text is a complete body for it), move the `verify-restore.sql` pins (`close_deal` secdef false; migrations +1), and revert the 0118 tests. **Never make `close_deal` an invoker while 0118's guard body is installed**: its own UPDATE would run as `authenticated` and every close would be refused. Guarantees lost on rollback: a user session can again PATCH / POST / UPSERT an open deal to won or lost with no rule and no event, park closing details or a terminal / foreign stage on an open deal, and insert a deal born closed. Kept: 0117's atomic close, its answers, the closed-row guard, the SEC-03 payloads. No data migration either way: every row closed at 0118 is valid at 0117.

**Not changed here** (BACKLOG): the accepted offer `close_deal` requires is whatever `offers.status` says, and offers have no database boundary (a user session may POST an offer already accepted or re-point an accepted offer's `deal_id` within the organisation — no new authority over the Won rule, since an agent may record and accept an offer on their own deal in the app, but the offer's own event and single-accepted check are skipped); hand-written terminal events; `deals_insert` does not bind `agent_id` / `created_by`; `deal_type` / `created_at` / `stage_entered_at` writable, and a closed deal's `expected_value` / `agent_id` editable (a product decision — reports read them); an admin may flip a stage's terminal flags; `verify-restore.sql` does not pin `forbid_srs_api_writes` (0099). No offer, reservation or listing status is changed by a close; the Won reminders stay app-side.

**Landing (2026-09-26).** On the operator's word ("applying 0118 to hosted, then merging go a head"). Branch CI green first on both code commits (runs 36192011940 and 36192189997: checks, rls, e2e — on CI's fresh database the three local rls reds are green). Hosted FIRST. Pre-flight, read-only: ledger at 0117 (117 rows, non-filename versions 0), bodies `cb70a918…` / `7ee172ae…` as 0117 recorded, the trigger BEFORE UPDATE, no definer writer of deals, one deal (lost), advisors 2 ERR + 36 WARN. Applied in one `execute_sql` with the file's exact text; its assertions passed. Verified separately: `close_deal` md5 `777737d8…` and the guard `258b2b3e…` equal local; `close_deal` SECURITY DEFINER, owner postgres, `search_path=public, pg_temp`, one overload, ACL `postgres` + `authenticated` (anon and service_role cannot execute); the guard an invoker, `search_path=public, pg_temp`, callable by nobody; the trigger BEFORE INSERT OR UPDATE FOR EACH ROW; the ledger row 0118 inserted by hand (118 rows, non-filename versions 0); `rls_aal2_coverage` 0, `events_partition_health` 0, the chain verifies; advisors 2 ERR + 37 WARN — the one new finding is `close_deal` under `authenticated_security_definer_function_executable`, as predicted. Anon PostgREST: `close_deal`, PATCH and POST `deals` all answer 42501. A behavioural probe as an impersonated aal2 admin session, inside savepoints that roll back: a deal born won and an open deal with a `lost_reason` were refused with the guard's own sentences; no row left, the one deal untouched. Then PR #70 → main `365c44d` (pinned to `b7c8486`; main unmoved at `3a72498`). Vercel production `dpl_2AJpweLSE4A7FQPcuPjkCR6n1BSQ` READY and aliased to gnk-crm.vercel.app (the application code is byte-identical to `aa01ef8`); no runtime errors; `/login` 200, protected routes 307. CI on the merge commit (run 36218353658): green (checks, rls, e2e all success). Remote branch deleted, worktree removed, local branch kept. No deal was closed in production.

## T-task-deal-org-isolation — a task belongs to the organisation of the deal it names, and a deal's reminder supersession stays inside that organisation (2026-09-26; migration 0119)

**The gap** (BACKLOG "The deal nudge supersession writes across organisations", found by T-atomic-deal-close's design critique; the brief asked for independent verification first). Inspected: gnk-crm `main` = `79f3b3e` (PR #70's landing docs on top of `365c44d`), gnk-web `main` = `b158f37` (it has no writer of tasks or deals). No open pull request on either repository. Mapped read-only first (six readers: every writer of `tasks` in the app, the scripts and the SQL sweeps; every PostgREST embed between tasks and deals; every SQL join of tasks to deals; the test harness; the pins and records; the delete and maintenance paths — then the design). What they established: `tasks.deal_id` (0001) references `deals(id)` alone and `tasks_insert` / `tasks_update` (0030 / 0032) check only the caller's organisation, so nothing ties a task to its deal's tenant; `trg_supersede_deal_nudges` (0025, SECURITY DEFINER) matches the deal's open `deal_no_contact` tasks by `deal_id` alone and writes each `superseded` event with the TASK's `org_id` and `auth.uid()` as actor; the nightly sweep's step 3 (`create_followup_nudges`, 0078) has the same join; every legitimate writer that sets `deal_id` (quickAddTask after an RLS read of the deal, the Won follow-ups from `close_deal`'s answer, the sweep's own insert) takes `org_id` from the same row it read the deal from — except `transitionReservation`, which copies `reservations.deal_id`, itself accepted from the form without an RLS re-read (BACKLOG); nothing in the app, the scripts or the SQL ever changes a task's or a deal's `org_id`; no PostgREST select embeds deals from tasks or tasks from deals, and nothing names `tasks_deal_id_fkey` but the generated types.

**Reproduced before anything changed** — the local stack at 0118 (ledger 118, trigger md5 `081ca764…` = hosted), real PostgREST, aal2 sessions of two throwaway organisations A and B, the new test file run RED (10 of 16):
| attempt at 0118 | result |
|---|---|
| B's agent / B's admin INSERT a task of B naming A's deal (a reminder, an ordinary task) | 201 — the row exists |
| B PATCHes one of B's tasks (with or without a deal of its own) onto A's deal; B's admin the same | 200 — re-pointed |
| B UPSERTs onto an existing task of B, and as a new id | 200 / 201 |
| the service role moves a task's `org_id` to B while it names A's deal | 200 — `tasks.org_id` and `deals.org_id` no longer agree |
| B INSERTs naming a REAL A deal id vs a MISSING id | 201 vs 23503 — an existence oracle on deal ids, through tasks |
| A's agent closes the deal through `close_deal` | B's task marked done, `done_at` set (the `superseded` event goes into B's chain with A's user as actor — the trigger writes `t.org_id`) |
| A's agent logs contact (`last_contact_at`, the trigger's other branch) | the same |
| controls: B cannot read A's deal; A's own reminders (the agent's and a colleague's) complete with the right event and actor; a deal-less task; deletion NO ACTION | as expected, unchanged |
Hosted (read-only `execute_sql`, 2026-09-26): ledger 0118 (118 rows), **0 tasks**, 1 deal, 1 organisation, 0 mismatches, `tasks_deal_id_fkey` = 0001's, the trigger's body = 0025's. Nothing to repair, nothing repaired, no mutation run against production.

**Options compared.**
| option | benefit | cost / risk | verdict |
|---|---|---|---|
| the organisation predicate in the trigger alone (BACKLOG's first line) | one function re-created | the invalid RELATIONSHIP is still creatable (INSERT/PATCH/UPSERT still 2xx), the oracle stays, the sweep's step 3 still completes it nightly, every future reader of `tasks.deal_id` must remember the predicate | rejected on its own |
| a `tasks_insert` / `tasks_update` policy clause (`exists (select 1 from deals where id = deal_id and org_id = current_org_id())`) | no schema change | binds user sessions only — service_role, definer bodies and pg_cron are not held by RLS; two subselects on every task write; a policy cannot follow a later change of the DEAL's org | rejected |
| a BEFORE INSERT OR UPDATE trigger on tasks checking the deal's organisation | catches every writer | a second mechanism for what the engine has first-class; judged at write time only; not visible to PostgREST or the types | rejected |
| the composite key ADDED beside the single-column one (0101's shape) | no DROP | two relationships between tasks and deals — the first `deals(...)` embed from tasks answers PGRST201 (measured by mutant M3) | rejected |
| **the predicate PLUS `(org_id, deal_id) → deals (org_id, id)` REPLACING the single-column key (0088's shape), NO ACTION kept** | the relationship is a fact the database checks for every writer, the oracle through tasks closes, one relationship, the pair subsumes the single column, the trigger's contract is legible in its text | a unique key on deals and one index; the types regenerate; a NOT VALID path is needed only if mismatches exist (none do) | **chosen** |

**The fix — migration 0119.**
- **Preflight** counts tasks whose `org_id` differs from their deal's and ABORTS the whole file (one transaction) naming the count and the listing query — nothing deleted, reassigned or repaired, and this file never adds the constraint NOT VALID. If a hosted database ever holds such rows the operator decides: repair through the app, or a separate migration adding the constraint NOT VALID and validating it after the repair — and until it is validated, the sweep's step 3 needs the same predicate (BACKLOG NOTE).
- **`deals_org_id_id_key` UNIQUE (org_id, id)**, the referenced side (implied by the primary key; exists so a referencing table can name the pair — offers, viewings, reservations and `leads.converted_deal_id` can use it later).
- **`tasks_org_deal_fkey` FOREIGN KEY (org_id, deal_id) REFERENCES deals (org_id, id)**, replacing `tasks_deal_id_fkey`; ON DELETE and ON UPDATE NO ACTION exactly as 0001's key (a deal with tasks cannot be deleted — the one app delete, `convertLead`'s compensating rollback, deletes a seconds-old deal no task can name; 0088's CASCADE was NOT copied); MATCH SIMPLE, so a task with no deal is not checked; `org_id` is NOT NULL, so every task with a deal is. `tasks_org_deal_idx` (org_id, deal_id) WHERE deal_id IS NOT NULL is the referencing side (the referential check on a deal's delete or key change, and the trigger's own lookup — 0020's partial nudge index serves only `kind = 'deal_no_contact'`). `set local lock_timeout = '5s'` (0113's lesson) first in the file, ahead of the preflight and the ALTERs: ACCESS EXCLUSIVE on deals and tasks, give up and re-apply rather than queue behind a close or a sweep. Run twice by mistake, the file aborts at `deals_org_id_id_key` (42P07) and changes nothing.
- **`trg_supersede_deal_nudges`** re-created from 0025's text with one predicate, `and t.org_id = new.org_id`, in the UPDATE's WHERE; header (`security definer set search_path = public`), body, ACL (the revoke restated — HANDOFF §4.3), the trigger and its WHEN clause otherwise 0025's. It completes the DEAL's organisation's reminders and writes into THAT organisation's chain, attributed to the closer — whom `close_deal` (0118) and `deals_update` (0100) have already placed in that organisation. A same-organisation reminder assigned to ANOTHER staff member completes exactly as before: the predicate reads organisations, never assignees. With the key validated the predicate cannot fail to hold; it is defence in depth for the row the key could not have stopped (planted before 0119 and constrained NOT VALID, or written past the constraint with replication-role privileges), and the test proves it does that.
- **Apply-time assertions:** exactly one foreign key from tasks to deals, named, over (org_id, deal_id) → (org_id, id), VALIDATED, NO ACTION both ways, MATCH SIMPLE; `tasks_deal_id_fkey` gone; the unique key and the index present; the trigger body — comments stripped — scopes its UPDATE to the deal's organisation, is a definer with 0025's search_path, callable by neither anon nor authenticated; the trigger still AFTER UPDATE FOR EACH ROW WITH a WHEN clause; and the constraint EXERCISED once (0088's probe): two organisations, A's stage and deal, A's task on it and a deal-less task accepted, B's task naming A's deal refused — the foreign-key violation is what unwinds the sub-block's own inserts. On hosted the probe runs as `postgres` (not superuser there): `deals_closed_guard` exempts it, and every column it writes is one the tables require. Measured by the mutation runs: the block refuses every mutant tried (below).

**Tests.** Watched RED first: 10 of the 16 tests in `supabase/tests/task-deal-org-isolation.test.ts` fail against 0118's exact catalogue (the table above is that run), 16/16 at 0119. Two kinds of caller, as in `deal-close.test.ts`: supabase-js through PostgREST — how the app writes tasks and closes deals — and one `pg` session as postgres for fixtures, verification and cleanup, which also PLANTS the row the constraint now refuses (`session_replication_role = replica` for that one insert, which bypasses the referential check the way a NOT VALID constraint or a restore would; postgres is superuser on the local stack, in CI too). Two throwaway organisations, five users enrolled sequentially, everything deleted at the end as postgres — every task of BOTH organisations before any deal, because the planted rows name the other organisation's deal:
- the premise: B's admin and agent read nothing of A's deal; A's agent reads it;
- **cross-organisation writes refused without partial changes (23503):** INSERT as B's agent (a reminder) and B's admin (an ordinary task), and no row landed; UPDATE of a deal-less task, of a task on B's own deal, as agent and admin — the row keeps its `deal_id`; UPSERT onto an existing task (merge) and as a new id — nothing written; a task's `org_id` moved away from its deal's — by a user session (42501, `tasks_update`'s WITH CHECK) and by the SERVICE ROLE (23503: a key binds the maintenance role, unlike 0118's role-keyed guard); a real A deal id and a missing one answer the same code and message — the oracle through tasks is closed (`reservations.deal_id` keeps one — BACKLOG);
- **same-organisation links, deal-less tasks, embeds, maintenance:** A's agent and admin link and re-point tasks within A, clear a link, insert without a deal; PostgREST resolves `deals(...)` from tasks and `tasks(...)` from deals (no PGRST201 — the one-relationship claim, measured); the nightly sweep still mints a reminder in the deal's own organisation (its insert meets the new key); a deal with tasks still cannot be deleted (NO ACTION) and a task can, then the deal;
- **closing or contacting A's deal leaves B untouched:** with A's reminder for the deal's agent, one for ANOTHER staff member, an ordinary task on the deal, B's PLANTED reminder naming A's deal and B's own reminder on B's deal — `close_deal` completes A's two reminders with one `superseded` event each (`entity_type` task, actor = the closer, payload `{ kind, deal_id, reason: 'deal_closed' }`), leaves the ordinary task open, B's planted task and B's own reminder untouched (`is_done` false, `done_at` null), writes nothing into B's chain and nothing outside A, both chains verifying; `last_contact_at` (what `logDealContact` writes) does the same with `reason: 'deal_contacted'`;
- **a refused event during the close rolls the reminders back too** (a BEFORE INSERT trigger on events refuses the `lost` event): the close answers the injected failure, the deal stays open, A's reminders stay open, no event survives, B untouched — and the retry closes and completes them, B still untouched;
- **the deal-close protections stand:** B's admin and agent get "Deal not found" from `close_deal`; A's agent's direct PATCH to won is refused by 0118's guard; the planted row gives B no read of the deal;
- **the catalogue:** the key's columns, validation, actions; `tasks_deal_id_fkey` absent; `deals_org_id_id_key`; the index; the trigger body's predicate, definer flag, search_path, ACL; the trigger definition byte-for-byte 0025's.
Every existing test that plants a reminder does so in the deal's own organisation (rls test 27, `deal-close.test.ts`), so the database suite runs unchanged.

**Mutation proofs** (a harness that refuses to start unless the database carries 0119's fingerprint — trigger md5 `2020a0ee…`, the composite key present, the old one absent — installs each mutant by exact catalogue change, confirms it installed, runs the file, then runs the migration's own DO block against the mutant, and restores in `finally` to the fingerprint). M1 0025's trigger body reinstalled (no predicate): 4 tests fail — the close, the contact branch, the refused-event retry, the catalogue — and the DO block aborts "does not scope its UPDATE". M2 the single-column key reinstalled instead of the composite: 6 fail — INSERT, UPDATE, UPSERT, the org move, the oracle, the catalogue — DO block aborts. M3 the single-column key KEPT beside the composite (0101's shape): the embed test fails with PGRST201 and the catalogue test; DO block aborts "found 2". M4 ON DELETE CASCADE (0088's clause): the deletion test and the catalogue test fail; DO block aborts. Control: the DO block passes on the restored catalogue; the fingerprint matched before and after every mutant.

**Review.** An adversarial workflow over the first commit (four lenses — migration correctness and hosted-apply safety, a bypass hunt, test quality, written claims and rollout — with two independent refuters per finding; 36 agents): 16 raw findings, 3 confirmed, 13 refuted. **No defect in the change and no bypass:** every writer of `tasks.deal_id` was walked and none is refused by the key; no path to a cross-organisation task, a cross-organisation supersession or a cross-organisation chain write was found after 0119. Confirmed and fixed in `3031f18`: the rollback recipe said the restore pin moves *back* — a forward rollback is one more ledger row, so it moves forward (0118's record said the same); the three "nothing else was written" scans read the whole `events` table, which the local stack's own crons (lead-sla every ten minutes, the nightly sweeps) write into on their own schedule — now scoped to the two throwaway organisations; the BACKLOG NOTE on the sweep named only a NOT VALID constraint, not a replica-mode restore of a backup that held a mismatch, and only step 3, not step 1's idempotence join. Refuted but adopted anyway as wording: `lock_timeout` first in the file (0113's placement); the cron window in pg_cron's clock (UTC); the test header's exact RED count (ten: the five "B cannot", the first three "leaves B untouched", the two catalogue tests — the protections test pins 0118 and was green); what is proven about the local postgres role; "no existence oracle" scoped to tasks (`reservations.deal_id` keeps one — BACKLOG); the two BACKLOG VERIFY lines that could not discriminate; the performance advisor's INFO rows; a replay's 42P07. Refuted and left: nothing. Also acted on from the review: a restore-pack invariant row that counts task/deal organisation mismatches, because a replica-mode restore loads rows past every constraint.

**Measured** (the branch, local stack at 0119): `tsc --noEmit --incremental false` 0; eslint 0; unit 2874/2876 across 209 files with the two misses being 5 s timeouts under the concurrent database run (`due-at-is-a-cyprus-day`, `env-names-documented`) — 15/15 when run alone; the database suite 517/520 across 34 files — the 3 reds are rls 41/42/57, the public-feed residue on this machine's database (the feed's first 50 rows are residue listings), the same three the last two records name, green on CI's fresh database; the new file 16/16 (10 red at 0118); `verify-restore.test.ts` 4/4 after the pin moved (118 → 119); `npm run build` exit 0 and `check:static-routes` ok; Playwright desktop 12/12 through the worktree's own dev server (`deal-close`, `deal-close-outcomes`, `nudges`, `entity-tasks`, plus the setup login — the one "won follow-up failed" server line is `deal-close-outcomes`' own injected event refusal), 3.5 min. Branch pushed and PR [KALAITSIDIS/gnk-crm#71](https://github.com/KALAITSIDIS/gnk-crm/pull/71) opened; branch CI on the first commit `af6b348` GREEN (run 36223983365: checks, rls, e2e — the fresh-database apply of 0119 and the new file on it); the review follow-up `3031f18` GREEN too (run 36224931437: checks, rls, e2e — the fresh-database apply of the final migration text); the restore-pack invariant row and these records follow it.

**Compatibility and deploy order.** ADDITIVE for the deployed application (`79f3b3e`'s code = `aa01ef8`): every task it writes takes `org_id` from the row it read the deal from, so no request it sends is refused by the new key; no function signature, return shape, grant or enum changes — no `release-compat-contracts` entry (HANDOFF §3's rule is about functions the app calls). `database.types.ts` regenerates: the tasks Relationships entry `tasks_deal_id_fkey` over `deal_id` becomes `tasks_org_deal_fkey` over `(org_id, deal_id)`; nothing in TypeScript names either, nothing embeds across the relationship. **Deploy order: hosted 0119 first, then merge** — the branch carries no application code beyond the generated types. Visible to a DIRECT PostgREST caller only: a task naming a deal of another organisation now answers 409 / 23503 instead of 2xx, and a missing deal id the same; `transitionReservation` of a hold whose `deal_id` was posted as a foreign deal logs the refused prompt and raises none (BACKLOG). Advisors: no ACL moves, so the security advisor's 2 ERR + 37 WARN is expected unchanged after the apply; the performance advisor's INFO rows will move (+1 unused index for `tasks_org_deal_idx` and `deals_org_id_id_key`'s; a partial index may read as an unindexed foreign key) — record the INFO counts before and after.
- **Hosted apply recipe:** ONE `execute_sql` with the file's exact text (its preflight, DO block and probe must run with it; `set local lock_timeout = '5s'` is inside — apply outside 03:00–04:00 UTC, pg_cron's clock (06:00–07:00 Cyprus in summer), and simply re-run on a timeout), then separately: `pg_get_constraintdef` of `tasks_org_deal_fkey` = `FOREIGN KEY (org_id, deal_id) REFERENCES deals(org_id, id)` with `convalidated`, `deals_org_id_id_key`, `tasks_org_deal_idx`, `tasks_deal_id_fkey` gone; the trigger body md5 `2020a0ee…` = local (`md5(replace(prosrc, chr(13), ''))`), `prosecdef`, `search_path=public`, ACL as before (nobody over PostgREST); `pg_get_triggerdef` of `deals_supersede_nudges` unchanged; the ledger row 0119 by hand, `non_filename_versions` 0; `rls_aal2_coverage` 0, the chain verifies; advisors 2 ERR + 37 WARN; an anon PostgREST POST of a task → 42501 as before.
- **Existing data:** the preflight is the check. Hosted holds 0 tasks today, so the constraint validates trivially; if that changes before the apply, the file's abort names the count and the listing query, and the staged path above applies.
- **Rollback floor:** none moves — the application does not depend on 0119.

**Rollback.** Through a FORWARD migration, never by hand: drop `tasks_org_deal_fkey` and `tasks_org_deal_idx`, re-add `tasks_deal_id_fkey foreign key (deal_id) references deals(id)`, drop `deals_org_id_id_key`, re-create `trg_supersede_deal_nudges` from 0025's text and reset its `comment on function`; regenerate the types, move the `verify-restore.sql` migrations pin FORWARD again (a rollback is one more ledger row — 0118's record says the same), revert this test's catalogue block, the docs/04 note and the BACKLOG strike. No data moves either way: every row valid at 0119 is valid at 0118. Guarantees lost on rollback: a cross-organisation task becomes creatable again, the oracle re-opens, and a close or a contact completes such a row and writes into the other organisation's chain.

**Not changed here** (BACKLOG): the OFFERS half of the original entry (`offers.deal_id` is org-blind; `deal-close.test.ts` plants that row as a fixture and `close_deal` reads offers in the deal's own organisation, so it grants nothing); every other `tasks.*` link and every sweep's parent join are org-blind — `viewing_id` and `trg_supersede_viewing_nudges` are the exact twin; `createReservation` copies a form `deal_id` onto the hold without an RLS re-read; the sweep's steps 1 and 3 keep their joins (a NOTE says what to do if the constraint is ever NOT VALID or a replica-mode restore loads rows past it); `reservations.deal_id`, `viewings.deal_id` and `leads.converted_deal_id` can take the same key later; the trigger keeps `search_path = public` (0118 left the other definers out of scope); the close/sweep deadlock NOTE is unaffected — the predicate narrows the rows, not the lock order. No offer, reservation, listing status or commission rule changes; the atomic close and 0118's guard are untouched — every test of both runs unchanged.

**Landing (2026-09-26).** On the operator's word ("PROCEED WITH NESSESARY", then "you do it you have the access"). Branch CI green first on `af6b348` (run 36223983365), `3031f18` (36224931437) and `81fb725` (checks, rls, e2e). Hosted FIRST, ~07:50Z: pre-flight read-only at 07:46Z — ledger 0118 (118 rows), 0 tasks, 1 deal, 0 mismatches, trigger md5 `081ca764…` = 0025's, `tasks_deal_id_fkey` as 0001's, 0 other active backends, aal2 gaps 0, partition health 0, the chain verifies, security advisors 2 ERR + 37 WARN; applied in one `execute_sql` with the file's exact text (md5 `91db073e…` = `81fb725`'s), its preflight, assertions and probe passed; verified separately — `tasks_org_deal_fkey` = `FOREIGN KEY (org_id, deal_id) REFERENCES deals(org_id, id)` validated, `deals_org_id_id_key` = `UNIQUE (org_id, id)`, `tasks_org_deal_idx` present, the old key gone, the trigger body md5 `2020a0ee…` = local, SECURITY DEFINER, owner postgres, `search_path=public`, one overload, anon and authenticated cannot execute it (service_role true, the known cloud delta, unchanged), the trigger definition byte-identical to 0025's, the probe left nothing; the ledger row 0119 by hand in a separate call (119 rows, non-filename versions 0); anon has no privilege on `tasks`, RLS on, 5 policies. The post-apply advisor reads, the publishable-key read for an anon PostgREST probe and the first `gh pr merge` were refused by the session's auto-mode classifier (as PR #69's merge was); on the operator's explicit word the same merge command went through: PR #71 → main `a1b846e`, pinned to `81fb725`, main unmoved at `79f3b3e`; CI on the merge commit green (run 36228644417: checks, rls, e2e). Vercel production `dpl_F5ZSQkWGNsKhyVjtC1L5bXW6dbHG` READY and aliased to gnk-crm.vercel.app (fra1); no runtime errors in the two hours around the apply and the deploy; `/login` 200, `/tasks` `/deals` `/dashboard` 307 signed out. Hosted holds 0 tasks, so nothing in production exercised the key beyond the migration's own probe; no record was created to make one. Advisors read after the landing (08:42Z, on the operator's word): security 2 ERR + 37 WARN, unchanged; performance INFO only — 56 unindexed foreign keys (`tasks_org_deal_fkey` is not among them: the partial index covers it), 25 unused indexes including the new `tasks_org_deal_idx` (0 tasks on hosted), 0 WARN. Remote branch deleted, worktree removed, local branch kept.

## T-task-viewing-org-isolation — a task belongs to the organisation of the viewing it names, and a viewing's feedback-reminder supersession stays inside that organisation (2026-09-26; migration 0120)

**The gap** (BACKLOG "Every other `tasks.*` link is org-blind", `viewing_id` first — the exact twin of T-task-deal-org-isolation, found by that change's scouting). Inspected: gnk-crm `main` = `f645274` (0119's landing records on top of the merge `a1b846e`), no open pull request. `tasks.viewing_id` (0020) references `viewings(id)` alone; `tasks_insert` / `tasks_update` check only the caller's organisation; `viewings_select` is organisation-scoped, so B cannot read A's viewing; `trg_supersede_viewing_nudges` (0020, SECURITY DEFINER, never redefined — md5 `57d304f3…` on both local and hosted) matches the viewing's open `viewing_feedback` tasks by `viewing_id` alone and writes each `superseded` event with the TASK's `org_id` and `auth.uid()`; its trigger fires AFTER UPDATE when `feedback` or `status` changes — `saveViewingFeedback` (`update({ feedback })`) and `updateViewingStatus` (`update({ status })`, a compare-and-set on `scheduled`), both on the user session. The only legitimate writers of `tasks.viewing_id` are the nightly sweep's arms 2 and 2b (0078), which take `org_id` and `viewing_id` from the same viewings row; the application writes no task with a viewing. No role can delete a viewing (no DELETE grant, no policy). Mapped read-only first by four readers (every writer of `tasks.viewing_id` and of `viewings.feedback` / `status`; every embed and join between tasks and viewings; every fixture and the test harness; every delete path, pin and record): no PostgREST select embeds across the relationship, nothing names `tasks_viewing_id_fkey` but the generated types, and no fixture links a task to another organisation's viewing. They also found the sweep's reminder-creation guards (arms 2 / 2b, `not exists … t.viewing_id = d.id`) organisation-blind — a planted row would suppress a legitimate one-shot reminder — which the validated key closes too (BACKLOG NOTE), and an inferred twin gap on `viewing_slips` (BACKLOG).

**Reproduced before anything changed** — the local stack at 0119, real PostgREST, aal2 sessions of two throwaway organisations, the new test file run RED: exactly the ten tests marked "RED at 0119" failed and the six controls passed.
| attempt at 0119 | result |
|---|---|
| B's agent / admin INSERT a task of B naming A's viewing (a reminder, an ordinary task) | accepted |
| B PATCHes a task of B (with or without a viewing of its own) onto A's viewing; B's admin the same | accepted |
| B UPSERTs onto an existing task, and as a new id | accepted |
| the service role moves a task's `org_id` to B while it names A's viewing | accepted |
| B names a REAL A viewing id vs a MISSING one | accepted vs 23503 — an existence oracle on viewing ids, through tasks |
| A's agent saves feedback (the action's exact write) | B's planted reminder marked done; the `superseded` event goes into B's chain with A's agent as actor |
| A's agent cancels a scheduled viewing (the action's exact write) | the same, through the trigger's status branch |
Hosted (read-only `execute_sql`, 2026-09-26): ledger 0119 (119 rows), **0 viewings, 0 tasks**, 0 mismatches, the trigger body = 0020's, `tasks_viewing_id_fkey` = 0020's. Nothing to repair, nothing repaired, no mutation run against production.

**The fix — migration 0120**, 0119's decision applied to viewings (the options were compared there: the predicate alone, an RLS clause, a tasks trigger, a second key beside the first — all rejected for the same reasons):
- **Preflight** aborts the whole file over any task whose organisation differs from its viewing's, naming the count and the listing query; never NOT VALID by this file.
- **`viewings_org_id_id_key` UNIQUE (org_id, id)**, the referenced side; **`tasks_org_viewing_fkey` (org_id, viewing_id) → viewings (org_id, id)** REPLACING `tasks_viewing_id_fkey` — one relationship, so PostgREST embeds stay unambiguous; NO ACTION both ways as 0020's key; MATCH SIMPLE, so a task without a viewing is unchanged; **`tasks_org_viewing_idx`** the referencing side. `set local lock_timeout = '5s'` first.
- **`trg_supersede_viewing_nudges`** re-created from 0020's text with `and t.org_id = new.org_id` in the UPDATE's WHERE; header, body, ACL (revoke restated), trigger and WHEN clause otherwise 0020's. Same-organisation reminders assigned to another staff member complete exactly as before.
- **Apply-time assertions:** exactly one key from tasks to viewings — named, columns, validated, NO ACTION, MATCH SIMPLE; the old key gone; the unique key and the index; **0119's `tasks_org_deal_fkey` still validated**; the trigger body scoped (comments stripped), definer, `search_path=public`, callable by neither anon nor authenticated; the trigger still 0020's AFTER UPDATE FOR EACH ROW WITH a WHEN clause; and the constraint EXERCISED once in a sub-block (two organisations, A's property, contact and viewing, A's own link and a viewing-less task accepted, B's cross-organisation task refused). The probe names an existing profile as the viewing's agent and skips with a notice on a database that has none (never the case on hosted or CI).
- **The restore pack** gains `INTEGRITY: no task names a viewing of another organisation (0120)` beside 0119's row (a replica-mode restore loads rows past every constraint), and its migrations pin moves 119 → 120.

**Tests** — `supabase/tests/task-viewing-org-isolation.test.ts`, 16, the structure of 0119's file: supabase-js through PostgREST for every write the app sends, one `pg` session as postgres for fixtures and verification, which also PLANTS B's reminder naming A's viewing with `session_replication_role = replica` (the only way to prove the trigger's own predicate once the key forbids the row). Two throwaway organisations with a property and a contact each, five users enrolled sequentially, everything deleted at the end — every task of BOTH organisations before any viewing. It covers: the premise (B reads nothing of A's viewing); cross-organisation INSERT, UPDATE, UPSERT and org-move refused with 23503 (the org move 42501 on a user session) without partial changes; the oracle closed; same-organisation links, re-pointing, clearing, viewing-less tasks; PostgREST embeds `viewings(...)` from tasks and `tasks(...)` from viewings (no PGRST201); the nightly sweep still minting a feedback reminder in the viewing's own organisation, assigned to its agent; NO ACTION deletion (and no user session can delete a viewing at all); feedback saved by A's agent completing A's two reminders — the colleague's too — with one `superseded` event each (actor the agent, payload `{ kind, viewing_id, reason }`), the ordinary task left open, B's planted reminder and B's own reminder untouched, nothing written into B's chain, both chains verifying; the same through the status branch (scheduled → cancelled); a refused `superseded` event rolling the feedback write back with the reminders (the trigger runs inside the UPDATE's transaction), then the retry succeeding; B's feedback write on A's viewing matching zero rows; the catalogue, including 0119's deal key untouched. Every existing fixture that links a task to a viewing is same-organisation, so the rest of the suite runs unchanged.

**Mutation proofs** (the 0119 harness, fingerprinted — trigger md5 `7d5f95b6…`, the composite key present, the old one absent — restored in `finally`, each mutant also run through the migration's own DO block): M1 0020's trigger body reinstalled → 4 fail (feedback, status branch, refused-event retry, catalogue), DO block aborts "does not scope its UPDATE"; M2 the single-column key instead of the composite → 6 fail (INSERT, UPDATE, UPSERT, org move, oracle, catalogue), DO block aborts; M3 the single-column key kept beside it → the embed test (PGRST201) and the catalogue, DO block aborts "found 2"; M4 ON DELETE CASCADE → the deletion test and the catalogue, DO block aborts. Control: the DO block passes on the restored catalogue; the fingerprint matched before and after every mutant.

**Review.** An adversarial workflow over the first commit (three lenses — migration correctness and hosted-apply safety, a bypass hunt with compatibility, test quality and written claims — two independent refuters per finding; 31 agents): 14 raw findings, 1 confirmed, 13 refuted — most of them because the refuters found the follow-up already in the working tree. **No defect in the change and no bypass.** Acted on in `cca6ccb`: the probe's verdict tied to THIS key (`get stacked diagnostics … constraint_name`; the borrowed profile held FOR KEY SHARE; an abort names what the probe met); the header's "no role can delete a viewing" corrected to "no user session"; fixture viewings default to 1 h old so the local nightly sweep (every organisation) mints nothing for them; the sweep test reads the global `viewing_feedback_hours` instead of assuming 48 h; the whole-table "elsewhere" event scan, which could not fail once B's own scan passed, removed here and in 0119's file. Records: the HANDOFF line no longer calls the probe "the same" (it skips, with a NOTICE only, where no profile exists) and the hosted line carries the profile count; the BACKLOG VERIFY counts only `add constraint` lines. The one confirmed finding: `mandate_id` is not "lower severity" — `raise_key_recall_tasks` runs at edit time from `setMandateStatus` with the admin as `p_actor`, supersedes by `mandate_id` alone into the task's organisation, and its creation guard can be blocked by a planted row — so it is BACKLOG's NEXT twin. Two further defects outside this change, recorded not built: a viewing's own `property_id` / `contact_id` / `agent_id` are organisation-blind (the sweep then copies another organisation's property into a reminder), and the contact merge repoints every referencing table on the admin client without an organisation filter.

**Measured** (the branch, local stack at 0120): the new file 16/16 (10 red at 0119); `tsc --noEmit --incremental false` 0; eslint 0; the full database suite 533/536 across 35 files — the 3 reds are rls 41/42/57, the public-feed residue on this machine's database (identical at 0119's landing), green on CI; unit 2876/2876 across 209 files; `verify-restore.test.ts` 4/4 and the restore pack itself green on the local stack (130 rows, both invariant rows 0); `npm run build` exit 0 and `check:static-routes` ok; Playwright desktop 11/11 through the worktree's own dev server (`nudges`, `viewing-confirmation`, `viewing-reschedule`, `slip-pdf-hash`, `entity-tasks`, `viewings-export` + setup); after the review follow-ups both isolation files 32/32, tsc 0, eslint 0, and the FINAL migration text replayed on a 0119-shaped catalogue in a rolled-back transaction (profiles present, as on hosted — preflight, DDL, assertions and probe passed, nothing left). Branch CI green on `3d1ac24` (run 36231730913: checks, rls, e2e) and on the review follow-up `cca6ccb` (run 36232724250: checks, rls, e2e — the fresh-database apply of the final text, where the probe is skipped because no profile exists yet and the test file proves the key).

**Compatibility and deploy order.** ADDITIVE: the application writes no task with a viewing, and the sweep takes both columns from the same row, so nothing deployed is refused; no function signature, return shape, grant or enum changes — no release-compat entry; `database.types.ts` regenerated (the tasks Relationships entry only). **Hosted 0120 first, then merge.** Apply outside 03:00–04:00 UTC with one `execute_sql` of the file's exact text, then verify separately (the key, the unique key, the index, the old key gone, the trigger body md5 `7d5f95b6…` = local, definer, `search_path=public`, ACL, the trigger definition 0020's, the probe left nothing), the ledger row 0120 by hand, advisors (security expected unchanged at 2 ERR + 37 WARN; performance INFO +1 unused index). No application rollback floor moves.

**Rollback.** A FORWARD migration: drop `tasks_org_viewing_fkey` and `tasks_org_viewing_idx`, re-add `tasks_viewing_id_fkey foreign key (viewing_id) references viewings(id)`, drop `viewings_org_id_id_key`, re-create `trg_supersede_viewing_nudges` from 0020's text and reset its comment; regenerate the types, move the restore migrations pin FORWARD (one more ledger row), remove the 0120 invariant row, revert the test's catalogue block and the docs. No data moves either way.

**Not changed here** (BACKLOG): the sweep's arms 2 / 2b / 4 / 4b keep their joins (0119's NOTE covers a NOT VALID constraint or a replica-mode restore); `viewing_no_show` reminders have no edit-time trigger (sweep-only by 0075's design); the remaining `tasks.*` links — `mandate_id`, `reservation_id`, `installment_id`, `lead_id`, `contact_id`, `property_id` — and their sweeps (all actor-null: no trigger attributes their supersession to a user); the other deal-side keys and `createReservation`'s form `deal_id`.

**Landing (2026-09-26).** On the operator's word ("GO", under the standing instruction to land what is built). Branch CI green on `3d1ac24` (36231730913), `cca6ccb` (36232724250) and `19ad688` (checks, rls, e2e). Hosted FIRST, ~09:42Z: pre-flight read-only at 09:41Z — ledger 0119 (119 rows), 0 viewings, 0 tasks, 0 mismatches, 2 profiles, trigger md5 `57d304f3…` = 0020's, `tasks_viewing_id_fkey` as 0020's, 0119's key validated, 0 other active backends, aal2 gaps 0, the chain verifies; applied in one `execute_sql` with the committed file's exact text (md5 `ff928ed7…`), its preflight, assertions and probe passed — the probe RAN (2 profiles); verified separately — `tasks_org_viewing_fkey` = `FOREIGN KEY (org_id, viewing_id) REFERENCES viewings(org_id, id)` validated, `viewings_org_id_id_key` = `UNIQUE (org_id, id)`, `tasks_org_viewing_idx`, the old key gone, 0119's key validated, trigger body md5 `7d5f95b6…` = local, SECURITY DEFINER, owner postgres, `search_path=public`, one overload, anon and authenticated cannot execute it (service_role true, the known cloud delta), the trigger definition 0020's, the probe left nothing, partition health 0; the ledger row 0120 by hand in a separate call (120 rows, non-filename versions 0); security advisors 2 ERR + 37 WARN, unchanged. PR #72 → main `d9e5c3e`, pinned to `19ad688`, main unmoved at `f645274`; CI on the merge commit green (run 36234141761: checks, rls, e2e). Vercel production `dpl_BFz1aS1ne1pxRVkPxwqRR8Ctv9CE` READY and aliased to gnk-crm.vercel.app (fra1); no runtime errors; `/login` 200, protected routes 307. Hosted holds 0 viewings and 0 tasks, so nothing in production exercised the key beyond the migration's own probe; no record was created to make one. Remote branch deleted, worktree removed, local branch kept.
