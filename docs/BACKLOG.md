...
- ~~**Proposal page "I'm interested" button, S.**~~ **BUILT 2026-09-21 on branch `fix/audit-2026-09-21-key-lifetime-sweep-health-interest` (migration 0106 + `POST /api/public/proposals/interest` + `InterestForm` on every property card, EN/EL/RU), on hosted and MERGED the same evening (PR #30 → `e4de93e`) — DECISIONS `T-audit-2026-09-21-evening`.** Not as sketched below: the website door needs an org SLUG and binds PUBLIC listings only, while a proposal may hold a private one, so a dedicated `submit_proposal_interest` resolves the org, the proposal and the property from the token digest; and a forwarded link proves nothing about who is typing, so the link's contact is never assumed — a small name + e-mail/phone form on every card, the enquirer's own details. VERIFY: `grep -n "InterestForm" components/features/share-links/proposal.tsx` — a hit. The original note: The audit's DA-07 / Sprint A step A9, deferred: a buyer reading a proposal (`/p/[token]`) sees the agent's e-mail and phone as links, so the open is tracked and the interest is not. One button per property posting to the public enquiry door (with the link's contact name and the reference, `source_page = proposal`) would close the loop through the existing pipe — but `resolve_share_link` returns the org NAME, not its slug, and a link with no contact needs a small name + e-mail/phone form. VERIFY: `grep -n "interest" components/features/share-links/proposal.tsx` — nothing.
- ~~**Lead SLA e-mail escalation — NEEDS AN OPERATOR DECISION.**~~ **BUILT 2026-09-22, SHIPPED DISABLED — ON HOSTED (0107) AND MERGED THE SAME DAY (PR #33 → main `c4a5c85`, deployed; audit 2026-09-22 finding 3; DECISIONS `T-audit-2026-09-22-interest-i18n-lead-escalation`).** A `lead_escalation` kind on the desk-alert outbox: `raise_lead_escalations()` (cron `lead-escalation`, every five minutes, the twelfth job) mints one job per website lead still open, unanswered, not redacted and received within `max_age_hours` once its WORKING-TIME wait (`lead_escalation_due_at`, Asia/Nicosia, DST-safe) has passed; the alert worker re-reads the policy, the lead and the recipients at send time and e-mails the named active admins/agents of the lead's org — never its assignee — under the outbox's own claim, lease, retry schedule and provider key. Policy row `cyprus_config.lead_escalation` is seeded `enabled: false` with PLACEHOLDER values (15 min, 48 h, Mon–Fri 09:00–18:00, no recipients) — the 2026-09-15 audit PROPOSED fifteen minutes and "the other principal"; neither is approved. **To activate (operator):** Settings → Lead escalation → tick Escalate, choose the wait, tick the recipient(s), confirm the hours, Save. Nothing else: `RESEND_API_KEY` is already set; `ENQUIRY_ALERT_FROM` unset means it leaves from the onboarding sender like the desk alert. Tolerance: the wait plus up to seven minutes. **Still a decision — the daily digest.** No approved recipients, content or hour, and `notification_jobs` wants a lead per row; a digest is a different shape (an org/recipient/local-date dedup key) and is NOT built. (original) 0098's `lead-sla` sweep raises a `lead_unanswered` task after an hour; the audit's trigger T1 also wants an e-mail to the OTHER principal at fifteen minutes and a daily digest. Vercel Hobby's cron is once a day, so the hop from the database is `pg_net` (INSTALLED on the hosted project since 2026-09-21 — 0103 uses it for the desk-alert sweep, with the bearer in Vault as `cron_secret`, so the hop exists) posting to a CRM route behind `CRON_SECRET`. What remains is the escalation itself — the route, the fifteen-minute e-mail to the other principal, the digest — and that is still the operator's call. **2026-09-22 afternoon (fourth brief, branch `fix/audit-2026-09-22-chain-order-escalation-payload-due-cutoff` — ON HOSTED (0109–0110) AND MERGED THE SAME DAY, PR #35 → main `6db0ae8`, deployed; DECISIONS `T-audit-2026-09-22-chain-order-escalation-payload-due-cutoff`):** the escalation's provider payload is now stable for the life of a key (the wait counts to the job's first attempt; recipients sorted), and **0110** makes `max_age_hours` count from the END of the working-time wait, so a Friday-night or weekend enquiry is escalated after Monday's opening instead of being cut off by the closed days — same seeded value, stated meaning on the settings page. The value itself (48 h, like the 15 min, the hours and the recipients) is still a placeholder awaiting the operator. VERIFY: `grep -c "p_now" supabase/migrations/0110_lead_escalation_due_cutoff.sql` → more than 0, and on the stack `select pg_get_function_arguments('public.raise_lead_escalations'::regproc)` names `p_now`. **2026-09-22 evening (fifth brief, branch `feat/audit-2026-09-22-escalation-visibility-recovery`, ON HOSTED (0111) AND MERGED THE SAME EVENING — PR #37 → main `463d63e`, deployed; DECISIONS `T-audit-2026-09-22-escalation-visibility-recovery`):** the inbox row now shows the escalation's state beside the desk alert's, and an admin may recover a stopped one by its job id — `retry` under the same provider key while it is safe, `resend` under a new key only when it is not, with a reason on the timeline (`request_lead_escalation_recovery`, 0111); the claim takes `p_job_id` so a staff action never sends the other kind. The values and the activation were decided 2026-09-22 17:08Z (ON — 15 min working time, Mon–Fri 09:00–18:00 Asia/Nicosia, cutoff 48 h, both admins as recipients; DECISIONS `T-audit-2026-09-22-escalation-preview`, Activation). What stays a decision: the digest, and whether a queued row should ever be sent early by hand (deliberately not built). VERIFY: `grep -c "p_job_id" supabase/migrations/0111_lead_escalation_recovery.sql` → more than 0.
...
- **A contact profile edit writes the old and new identifiers into the chain, S/M.**
  ...
  `changed` (T-merged-event-ids-only records why no key scan can) — so it wants a behavioural test like
  `lib/actions/merge-contacts-event-payload.test.ts`. The same `changed` idiom is written by deals,
  mandates, properties and party defaults: check each for client fields. The `updated` timeline line
  does not read `changed` (`lib/services/events.ts`). Existing events cannot be edited. Found by
  T-merged-event-ids-only. **VERIFY:** `grep -n "changed\[key\] = { from: prev" lib/actions/contacts.ts` —
...
- ~~**A secret token IN THE PATH still reaches Sentry, S.**~~ **BUILT 2026-09-23 on branch
  `fix/sentry-path-token-redaction` — not merged (the operator approves).** DECISIONS `T-sentry-path-token-redaction`. `redactPathTokens` turns the segment after
