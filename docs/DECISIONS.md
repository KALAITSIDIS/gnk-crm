...
**Measured 2026-09-23 on the branch.** First commit: `npm run typecheck` exit 0; `npm run lint` exit 0; `npx vitest run` 2404/2404 across 193 files, exit 0; branch CI green (run 35905447137: checks, rls, e2e). After merging main (#50) and the review fixes: typecheck exit 0; lint exit 0; `npx vitest run` 2434/2434 across 195 files, exit 0. One full run taken while the review agents were running vitest in the same worktree timed out six PDF and ignored-build-step tests at 5 s; those three files re-run alone passed 19/19, and the quiet full run above is the one that counts. There is no DB or e2e change, so CI's rls and e2e jobs are the only further arbiter.
