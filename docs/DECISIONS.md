...
**Not done here, recorded in BACKLOG.** Other events still carry typed text by value: a task's title (`tasks.ts`: `created`, `completed`, `reopened`), a contact or property document's title (`contact-documents.ts`, `property-documents.ts`: `document_uploaded`, `document_deleted`) and a mandate file's NAME (`mandates.ts` `document_uploaded`). The timeline renders those titles from the payload (`events.ts`), so the fix is a row join like 0094's for notes, not a payload edit — and a deleted document has no row left to join. The branch with `T-merged-event-ids-only` is still unmerged in its own worktree; it and this branch both touch BACKLOG around the same entries, so whichever lands second resolves the conflict there.
