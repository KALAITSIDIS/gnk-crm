...
**Built on PR #52.** The branch started from `fix/sentry-incoming-request-scrub` (`aad675d`) while #52 was open. #52 landed while this was being built (main `eb9d12d`), and so did #49. `origin/main` was merged in (DECISIONS and BACKLOG append conflicts only; `scrub-event.ts`, its test and both instrumentation files are identical between `aad675d` and that main), and the PR targets `main`.
