...
**2026-09-26, latest (deal close at the database boundary): a deal reaches won or lost only through `close_deal` — BUILT, NOT LANDED. Branch `fix/deal-close-db-boundary`, PR [KALAITSIDIS/gnk-crm#70](https://github.com/KALAITSIDIS/gnk-crm/pull/70). HOSTED IS STILL AT 0117; the local stack is at 0118.**
...
- **Dependency confirmed (2026-09-25/26, read-only):** production serves `aa01ef8` (`dpl_7BFFpRnoN9cDzuk2XquP7A9Yn7FL`), which closes only through `close_deal`; hosted 0117 bodies md5 as recorded; hosted holds one deal, lost and consistent — nothing to repair.
- **Landing needs the operator's word, in this order:** hosted 0118 FIRST (one `execute_sql` with the file's text; verification recipe in DECISIONS; expect advisors 2 ERR + 37 WARN), then merge (docs and tests only besides the migration — no application code changes). **Rollback floor:** once 0118 is on hosted, no application older than `aa01ef8` can close a deal (it fails closed). A database rollback is a forward migration restoring 0117's two bodies and the UPDATE-only trigger in ONE transaction — never `close_deal` back to an invoker while 0118's guard is installed.
- **If the PR is abandoned:** the shared LOCAL stack has 0118 applied (ledger row 0118; `close_deal` definer `777737d8…`, guard `258b2b3e…`). Re-run `supabase/migrations/0117_atomic_deal_close.sql` against it and delete the 0118 ledger row (or reset).
