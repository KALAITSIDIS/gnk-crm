...
- Final review (`4ce0799`): `portal_supplement` withholds the point of an approximate listing in the SQL itself — null lat/lng with the flag still returned — because 0054's flag means "never publish this as the property's location", nothing in the schema stops an import or a direct write from flagging a surveyed point, and the function is reachable by anyone holding the token over PostgREST, so a renderer's check alone was not the gate; the RLS test proves it and the renderers keep their double-check (`approx-guard.test.ts`).
