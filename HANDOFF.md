...
**2026-09-26, latest (task / viewing organisation isolation): a task belongs to the organisation of the viewing it names, and a viewing's feedback-reminder supersession stays inside that organisation — LANDED on the operator's word ("GO"). HOSTED IS AT 0120.**
- **What.** BACKLOG's "Every other `tasks.*` link is org-blind", `viewing_id` first — the exact twin of 0119. Verified against `f645274` on the local stack at 0119 through PostgREST with two throwaway organisations (the new test file run RED, 10 of 16): B's INSERT, PATCH and UPSERT of a task onto A's viewing accepted, the service role moving a task's org away from its viewing's accepted, a real-vs-missing A viewing id an oracle; A's `saveViewingFeedback` write, and A's `updateViewingStatus` write, then completed B's planted reminder through `trg_supersede_viewing_nudges` (0020) and wrote into B's chain with A's agent as actor. Migration **0120**: `tasks (org_id, viewing_id) → viewings (org_id, id)` (`tasks_org_viewing_fkey`, NO ACTION as before, replacing the single-column key; `viewings_org_id_id_key`; `tasks_org_viewing_idx`; the same abort-over-mismatch preflight as 0119, and the same apply-time probe except that it needs an existing profile as the viewing's agent and is SKIPPED, with a NOTICE only, where none exists — as in CI's and `db reset`'s migration run, where the test file covers the key) and the trigger re-created from 0020's text with `and t.org_id = new.org_id`. The restore pack gains the viewing twin of 0119's invariant row. DECISIONS `T-task-viewing-org-isolation`.
- **Hosted, read-only (before anything was written):** ledger 0119 (119 rows), **0 viewings, 0 tasks**, 0 mismatches, 2 profiles (so the probe runs there), the trigger body md5 `57d304f3…` = 0020's, `tasks_viewing_id_fkey` = 0020's. **Order: ADDITIVE — hosted 0120 first, then merge** (the app writes no task with a viewing; the sweep copies both columns from the same row).
- **Hosted 0120 FIRST (2026-09-26 ~09:42Z).** Pre-flight, read-only at 09:41Z: ledger 0119 (119 rows), 0 viewings, 0 tasks, 0 mismatches, 2 profiles, trigger md5 `57d304f3…` (0020's), 0119's key validated, 0 other active backends, aal2 gaps 0, the chain verifies. Applied in ONE `execute_sql` with the committed file's exact text (md5 `ff928ed7…`); its preflight, assertions and probe passed — the probe RAN, 2 profiles. Verified separately: `tasks_org_viewing_fkey` = `FOREIGN KEY (org_id, viewing_id) REFERENCES viewings(org_id, id)` validated; `viewings_org_id_id_key` = `UNIQUE (org_id, id)`; `tasks_org_viewing_idx`; the old key gone; 0119's key validated; trigger body md5 `7d5f95b6…` = local, SECURITY DEFINER, owner postgres, `search_path=public`, one overload, anon and authenticated cannot execute it (service_role true — the known cloud delta); the trigger definition 0020's; the probe left nothing (1 organisation, 0 viewings, 0 tasks); partition health 0. Ledger row 0120 by hand, re-read: 120 rows, non-filename versions 0. Security advisors 2 ERR + 37 WARN, unchanged.
- **Merge.** PR #72 → main `d9e5c3e` (pinned to `19ad688`; main unmoved at `f645274`; branch CI green on `3d1ac24`, `cca6ccb`, `19ad688`). Vercel production `dpl_BFz1aS1ne1pxRVkPxwqRR8Ctv9CE` READY and aliased to gnk-crm.vercel.app (the app code changes only in `database.types.ts`). CI on the merge commit green (run 36234141761: checks, rls, e2e). No runtime errors; `/login` 200, protected routes 307. Hosted holds 0 viewings and 0 tasks, so nothing in production exercised the key beyond the migration's own probe. Remote branch deleted, worktree removed, local branch kept.
- **Rollback:** a forward migration restoring 0020's key and trigger body, types regenerated, the restore pin FORWARD (one more ledger row), the 0120 invariant row removed. No data moves either way.
- **Measured on the branch:** the new test file 10/16 RED at 0119, 16/16 at 0120; mutation proofs 4/4 killed (predicate removed, composite key removed, second key beside it, ON DELETE CASCADE — each also refused by the migration's own DO block); database suite 533/536 (the three known public-feed residue reds); unit 2876/2876; tsc 0; eslint 0; build 0; Playwright desktop 11/11; the final text replayed on a 0119-shaped catalogue in a rolled-back transaction; branch CI green on `3d1ac24` and `cca6ccb`. Adversarial review: no defect, no bypass; follow-ups in `cca6ccb`; `mandate_id` is BACKLOG's next twin.

**2026-09-26 (task / deal organisation isolation): a task belongs to the organisation of the deal it names, and a deal's reminder supersession stays inside that organisation — LANDED on the operator's word ("PROCEED WITH NESSESARY", then "you do it you have the access"). HOSTED IS AT 0119.**
- **What.** BACKLOG's "The deal nudge supersession writes across organisations" (T-atomic-deal-close's critique), verified first against `79f3b3e` (gnk-web `b158f37` has no writer of tasks or deals; no open PR on either repository) on the local stack at 0118 through PostgREST with two throwaway organisations: B's agent and admin INSERTed a task of B naming A's deal (201), PATCHed one of B's tasks onto it and UPSERTed the same (200), the service role could move a task's org away from its deal's, and a real-vs-missing A deal id answered 201 vs 23503 — an existence oracle through tasks. A's `close_deal` then completed B's task (and A's `logDealContact` did the same through the trigger's other branch), writing into B's chain with A's user as actor. Migration **0119**: `tasks (org_id, deal_id) → deals (org_id, id)` (`tasks_org_deal_fkey`, NO ACTION as before, replacing the single-column key so PostgREST keeps ONE relationship; `deals_org_id_id_key` the referenced side; `tasks_org_deal_idx` the referencing side; a preflight that ABORTS the whole file over any existing mismatch — nothing deleted, reassigned or repaired, never NOT VALID; an apply-time probe that inserts the cross-organisation row inside a sub-block and expects 23503) and `trg_supersede_deal_nudges` re-created from 0025's text with `and t.org_id = new.org_id`. DECISIONS `T-task-deal-org-isolation`.
- **Hosted 0119 FIRST (2026-09-26 ~07:50Z).** Pre-flight, read-only at 07:46Z (outside the cron window): ledger 0118 (118 rows, non-filename versions 0), **0 tasks**, 1 deal, 1 organisation, 0 mismatches, the trigger's md5 `081ca764…` = 0025's text, `tasks_deal_id_fkey` = 0001's, 0 other active backends, aal2 gaps 0, partition health 0, the chain verifies, security advisors 2 ERR + 37 WARN. Applied in ONE `execute_sql` with the file's exact text (md5 `91db073e…` = the committed file); its preflight, assertions and cross-organisation probe passed. Verified in a separate call: `tasks_org_deal_fkey` = `FOREIGN KEY (org_id, deal_id) REFERENCES deals(org_id, id)`, validated; `deals_org_id_id_key` = `UNIQUE (org_id, id)`; `tasks_org_deal_idx` present; `tasks_deal_id_fkey` gone; the trigger body md5 `2020a0ee…` = local, SECURITY DEFINER, owner postgres, `search_path=public`, one overload, anon and authenticated cannot execute it (service_role true = the known cloud delta, unchanged); the trigger definition byte-identical to 0025's; the probe left nothing (1 organisation, 0 probe rows, 0 tasks). Ledger row 0119 by hand in a separate call, re-read: 119 rows, non-filename versions 0. Anon posture from the catalogue: no INSERT or SELECT privilege on `tasks`, RLS on, 5 policies — unchanged. **Not executed:** the post-apply `get_advisors` reads and the anon PostgREST probe were refused by the session's auto-mode classifier (0119 moved no grant, policy, view or function ACL); read after the landing at 08:42Z on the operator's word: **security 2 ERR + 37 WARN, unchanged; performance INFO only — 56 unindexed foreign keys (`tasks_org_deal_fkey` is NOT among them: the partial index covers it), 25 unused indexes including the new `tasks_org_deal_idx` (0 tasks on hosted), 0 WARN.**
- **Merge.** The first `gh pr merge` was refused by the session's auto-mode classifier (as on PR #69); on the operator's explicit word it went through: PR #71 → main `a1b846e` (pinned to `81fb725`; main unmoved at `79f3b3e`). Vercel production `dpl_F5ZSQkWGNsKhyVjtC1L5bXW6dbHG` READY and aliased to gnk-crm.vercel.app (fra1; the app code changes only in `database.types.ts`). CI on the merge commit green (run 36228644417: checks, rls, e2e). Remote branch deleted, worktree removed, local branch kept.
- **Verified in production, read-only:** no runtime errors in the two hours around the apply and the deploy; `/login` 200 (X-Vercel-Cache MISS, fra1), `/tasks` `/deals` `/dashboard` 307 signed out. Hosted holds 0 tasks, so nothing in production exercised the new key beyond the migration's own probe; no record was created to make one.
- **Rollback:** a forward migration (drop `tasks_org_deal_fkey` + `tasks_org_deal_idx`, re-add `tasks_deal_id_fkey (deal_id) → deals(id)`, drop `deals_org_id_id_key`, re-create the trigger from 0025's text, reset its comment), types regenerated, the restore migrations pin FORWARD again (one more ledger row), the test's catalogue block and the docs reverted. No data moves either way.
- **Measured on the branch:** the new `supabase/tests/task-deal-org-isolation.test.ts` 10 of 16 RED at 0118, 16/16 at 0119; the database suite 517/520 (the three reds are rls 41/42/57, the public-feed residue on this machine's database, identical on main); unit 2874/2876 with both misses 5 s timeouts under the concurrent database run, 15/15 alone; `tsc --incremental false` 0; eslint 0; `verify-restore` pins moved (118 → 119). Mutation proofs 4/4 killed (the predicate removed, the composite key removed, a second key kept beside it, ON DELETE CASCADE — each also refused by the migration's own DO block); Playwright desktop 12/12 (`deal-close`, `deal-close-outcomes`, `nudges`, `entity-tasks` + setup) through the worktree's own dev server; `npm run build` exit 0, `check:static-routes` ok; branch CI GREEN on `af6b348` (run 36223983365) and on the review follow-up `3031f18` (run 36224931437) — checks, rls, e2e, the fresh-database apply of 0119; an adversarial review (4 lenses × 2 refuters, 36 agents) confirmed 3 wording items, no defect and no bypass; the restore pack gained an invariant row counting task/deal organisation mismatches (a replica-mode restore loads rows past every constraint). DECISIONS `T-task-deal-org-isolation` has the detail.
- **Open, on BACKLOG (new):** the OFFERS half of the entry (`offers.deal_id` still org-blind — `deal-close.test.ts` plants that row as a fixture); every other `tasks.*` link and every sweep's parent join are org-blind (`viewing_id` first); `createReservation` copies a form `deal_id` onto the hold without an RLS re-read (after 0119 the converted hold's prompt is refused 23503 and logged, not raised); a NOTE on the sweep's step 3 for a future NOT VALID constraint.

**2026-09-26 (deal close at the database boundary): a deal reaches won or lost only through `close_deal` — LANDED on the operator's word ("applying 0118 to hosted, then merging go a head"). HOSTED IS AT 0118.**
- **What.** BACKLOG's "a direct write can still close an OPEN deal", verified first at 0117 through PostgREST (aal2 agent and admin, throwaway org): PATCH, POST and UPSERT each made an open deal won or lost with no accepted offer, no reason and no event, and closing details or a Won/Lost stage could be parked on an open deal. Migration 0118: `close_deal` becomes SECURITY DEFINER (`search_path = public, pg_temp`) with `deals_update` restated in its lock query and the caller re-read under the lock (0117's RLS re-read the profile at every statement); `deals_closed_guard` binds INSERT and OPEN rows for user sessions (no status change, no closing detail, no terminal or foreign stage). Maintenance roles and definer bodies stay unbound. DECISIONS `T-deal-close-db-boundary`.
- **Hosted 0118 FIRST** (branch CI green on both code commits first: runs 36192011940 and 36192189997, checks + rls + e2e). Pre-flight, read-only: ledger 0117 (117 rows, non-filename versions 0), bodies `cb70a918…` / `7ee172ae…` as recorded, no definer writer of deals, one deal (lost). One `execute_sql` with the file's exact text — its assertions passed. Verified separately: `close_deal` md5 `777737d8…` and the guard `258b2b3e…` = local; `close_deal` definer, owner postgres, `search_path=public, pg_temp`, one overload, executable by authenticated only; the guard an invoker callable by nobody; the trigger BEFORE INSERT OR UPDATE FOR EACH ROW; ledger row 0118 by hand (118 rows, non-filename versions 0); aal2 gaps 0, partition health 0, the chain verifies; advisors **2 ERR + 37 WARN** (the +1 is `close_deal`, predicted); anon PostgREST → 42501 on `close_deal`, PATCH and POST `deals`. A behavioural probe as an impersonated aal2 admin session, in savepoints that roll back: a deal born won and an open deal with a `lost_reason` were both refused with the guard's sentences; 0 rows left, the one deal untouched.
- **Merge.** PR #70 → main `365c44d` (pinned to `b7c8486`; main unmoved at `3a72498`). Vercel production `dpl_2AJpweLSE4A7FQPcuPjkCR6n1BSQ` READY and aliased to gnk-crm.vercel.app — app code byte-identical to `aa01ef8`. CI on the merge commit (run 36218353658): green (checks, rls, e2e all success). Remote branch deleted, worktree removed, local branch kept.
- **Verified in production, read-only:** no runtime errors in the two hours around the deploy; `/login` 200, `/deals` `/pipeline` `/dashboard` 307 signed out. No deal was closed in production (hosted holds one deal, already lost).
- **Rollback floor:** no application older than `aa01ef8` can close a deal now (it fails closed). A database rollback is a forward migration restoring 0117's two bodies and the UPDATE-only trigger in ONE transaction — never `close_deal` back to an invoker while 0118's guard is installed.
- **Open, on BACKLOG (new):** an offer can be born accepted or re-pointed by a direct write (measured: `close_deal` then counts it); hand-written terminal events; `deals_insert` does not bind `created_by`; deal fields around a close that nothing freezes (a product decision); the restore pack does not pin `forbid_srs_api_writes` (0099); NOTE — refused deal INSERTs now answer 400/P0001.

**2026-09-25 (late night, deal close): closing a deal is one transaction — LANDED on the operator's word ("go ahead", then "merge pr 69"). HOSTED IS AT 0117.**
- **What.** BACKLOG's open-status fold, verified first: at `6366ef8` two overlapping Lost requests wrote two `lost` events, overlapping Won and Lost left a lost deal carrying `won_at`/`final_value`, override Wins wrote everything twice, and a refused event left a closed deal with none (reproduced with the real actions on the real stack). Migration 0117: `close_deal` (SECURITY INVOKER, row lock, status checked under it, UPDATE and every event in one transaction, `already_closed` / `conflict` answers) and `deals_closed_guard` (a user session cannot swap, reopen or re-detail a closed deal). The actions never throw; Won follow-ups run only for the request that committed; the dialogs' messages outlive the refresh. DECISIONS `T-atomic-deal-close`.
- **Hosted 0117 FIRST:** one `execute_sql` with the file's text, its assertions passed; verified separately — bodies md5 = local (`close_deal` `cb70a918…`, `trg_deals_closed_guard` `7ee172ae…`), invoker + `search_path`, one overload each, ACLs (`close_deal` authenticated only; the trigger body callable by nobody), the trigger bound BEFORE UPDATE; ledger by hand at 117 (non-filename versions 0); aal2 gaps 0, partition health 0, the chain verifies; advisors 2 ERR + 36 WARN, unchanged; an anon PostgREST call to `close_deal` answers 42501.
- **Merge.** PR #69 → main `aa01ef8` (pinned to `9c98512`; branch CI green, run 36169435034: checks, rls, e2e). Vercel production `dpl_7BFFpRnoN9cDzuk2XquP7A9Yn7FL` READY and aliased to gnk-crm.vercel.app. CI on the merge commit (run 36176646388): checks and rls green; e2e was still running when this record was written (read it with `gh run view 36176646388 --json jobs`) — it completed green (checks, rls, e2e all success; read 2026-09-25 by T-deal-close-db-boundary).
- **Verified in production, read-only:** no runtime errors after the deploy; `/login` 200, `/deals` and `/pipeline` 307 signed out; in the operator's Chrome (counts only, no text read) the one hosted deal (lost) renders with no error boundary, its Lost banner, no Won/Lost buttons and one "Marked lost" line, and the admin dashboard renders. No deal was closed in production.
- **Open, on BACKLOG (new):** a direct write can still close an OPEN deal (deploy-coupled follow-up — the app is now deployed, so it can be built); Won reminders are at-most-once; NaN is a legal price; `move_deal_to_stage` takes FOR UPDATE; a rare close/nudge-sweep deadlock through the nudge tasks; cross-org nudge supersession; offers editable on a closed deal; the dashboard's won-this-month lost `final_value` (0093); on a phone a deal with an offer cannot be closed from its page.

**2026-09-25 (night, crafted events): no crafted event can take a timeline page down — LANDED on the operator's word ("merge #67 and verify the deploy"). No migration, nothing on hosted.**
- **What.** Any org member may insert any event (0071) and the chain can never drop it, so one line that breaks takes its timeline, the admin feed and the evidence report down for good. Not one line but a class: `rescheduled` formats only parseable dates; every line's numbers go through `asNumber` (a jsonb `{"toString":0}` or a deep array broke 26 more lines); `describeEvent` renders only a type the registry OWNS (`event_type: "constructor"` returned `Object`) and insists on a string; a line that throws reads as its bare type with a tags-only Sentry report; `formatDateTime` / `formatDate` read an unbuildable date as "—" (a crafted far-future `occurred_at`). The sweep `event-lines-crafted-payload.test.ts` fails on a throw, a non-string or a guard hit. DECISIONS `T-rescheduled-line-crash`.
- **Merge.** PR #67 → main `48a31cd` (rebased onto `5cebdfc` — #68 and the #66 landing — docs conflicts resolved by keeping both appends; pinned to `a4241c6`; branch CI green on it). Vercel production `dpl_Apxi5hkh3ueQDZLEMT4wkCsaoNRY` READY. CI on the merge commit green (run 36149107129: checks, rls, e2e).
- **Verified in production, read-only:** in the operator's Chrome (counts only, no text read): the admin dashboard, the busiest property's Activity tab (50 lines: 36 photo, 2 key — as before), the busiest contact's Activity tab (13) and /keys render with no error boundary; every event time formats (the dashboard 10, the property 50, the contact 13; 0 read "—"); 0 lines fell into the guard (none reads as a bare lowercase type); the /keys em dashes are empty Holder cells of keys in the office. No runtime errors; no new Sentry issue in the two hours after the deploy (so no "[timeline] line failed" report). Hosted holds 0 rescheduled events — nothing crafted was created to exercise the guard.
- **Open, on BACKLOG (new):** a crafted `occurred_at` still sorts first (a database bound needs a migration); one crafted `note_id` blanks every note on a timeline.

**2026-09-25, late evening (key holder): a key's movement event names the movement, never the holder — LANDED on the operator's word ("apply 0116 to hosted and merge #68"). HOSTED IS AT 0116.**
- **What.** Migration 0116 re-creates `record_key_movement` from 0013's text with exactly three changes — the event is `{ key_code, movement_id }` (was `{ key_code, holder }`: a typed external holder, the OWNER's typed name on a transfer, the cached name again on return / lost, or a staff full name). `key_movements` already keeps the holder of every movement and every custody screen reads it, so nothing moved to a new table. The key lines read no holder from any payload. DECISIONS `T-key-holder-shape`.
- **Hosted 0116 first.** Before: prosrc md5 `bf29a5e9…` = 0013's body with CRLF line endings (no drift). One `execute_sql` with the file's exact text (its assertions passed). Verified separately: prosrc md5 `8d9dc7ba…` = local 0116, one overload, SECURITY DEFINER + `search_path=public`, same identity arguments and `void`, ACL `authenticated` + `service_role` as before, no `'holder'` in the body; ledger by hand at 116, `non_filename_versions` 0. Security advisors 2 ERR + 36 WARN, unchanged.
- **Merge.** PR #68 → main `5cebdfc` (pinned to `5906690`; branch CI green on it; main unmoved at `0510047`). Vercel production `dpl_DCCFCr3FRy3jQZcGij1qXtrL9AiJ` READY. CI on the merge commit green (run 36146101249: checks, rls, e2e).
- **Verified in production, read-only:** in the operator's Chrome, the property holding both hosted key movement events renders its Activity tab (latest 50) with no error boundary; its two key lines read bare — the LEGACY checkout (which still carries a typed name in the chain) as "Key … checked out" and the return as "returned to office"; 0 lines with "checked out to", "handed to owner (" or "last with" (counts only, no text read). `/login` 200, `/leads` and `/keys` 307; no runtime errors after the deploy.
- **Open, on BACKLOG (new from this fix):** `record_key_movement` has no second-factor check; an unmatched staff id discards a typed holder; key movements can bypass the RPC; **four list exports write the raw URL query string (search terms included) into `exported` events**; **a signed viewing slip's `signer_name` is in its event** (hosted 1, slip row gone). The erasure decision now covers the key-holder columns and records backups as the horizon.

**2026-09-25, evening (importer identity): an imported person is named on the row, never in the chain — LANDED on the operator's word ("merge #66 and verify the deploy"). No migration, nothing on hosted.**
- **What.** `scripts/import/contacts.mts` logs `imported` as `{ source, batch }` (was `{ name }`), and the owner contact `properties.mts` creates as `{ source, as: "owner", batch }` (was `{ name: name ?? phone }`; the phone fallback was latent — `contact_has_name` refuses a nameless owner first). The `imported` line reads no name from any payload and prints a reference on a PROPERTY only. The scripts run from a checkout, so the next import writes the new shapes. DECISIONS `T-imported-identity-shape`. This closes the TypeScript writers on the SEC-03 BACKLOG list.
- **Merge.** PR #66 → main `22287d9` (pinned to `fd06051`; branch CI green on it; main unmoved at `a4a1f43`). Vercel production `dpl_HUhUxxKYWGQjsp1h3s6Ws2VqNAYN` READY. CI on the merge commit green (run 36134760083: checks, rls, e2e).
- **Verified in production, read-only, in the operator's Chrome:** the admin dashboard and the busiest contact's Activity tab (all 13 of its events) render with no error boundary and no "Imported" line — hosted holds 0 `imported` events, so there was nothing to render and nothing was created. `/login` 200, protected routes 307; no runtime errors in the hour after the deploy.
- **Review.** Adversarial workflow (3 lenses × 3 skeptics): no defect in the change; 2 record findings fixed in `fd06051`; 8/8 mutation proofs killed.
- **Open, on BACKLOG (new):** a key's typed HOLDER (an owner's name on a transfer) goes into key events via `record_key_movement` (0013, SQL; hosted 2 such events); `archive-records.mts` writes the typed `--reason` into `archived` (hosted 7); a phone-only owner row's dry run promises what the live run refuses; a NOTE on the batch label defaulting to the CSV's file name. Still open: the erasure decision; photo alt text (NOTE). The rescheduled-line crash fix is on branch `fix/rescheduled-line-crash` (not yet a PR).

**2026-09-25, afternoon (viewing feedback): a viewing's feedback event carries its rating, never the buyer's words — LANDED on the operator's word ("merge #65 and verify the deploy"). No migration, nothing on hosted.**
- **What.** `saveViewingFeedback` logs `viewing_feedback` as `{ viewing_id, reference, rating }`; `viewings.feedback` keeps `liked` / `disliked` / `comment`. The line prints stars only (an integer 1–5 — a forged `rating: 1e9` used to throw `RangeError` and take the property page, the admin feed and evidence generation down) from any payload. The property Activity tab and the admin feed show the viewing's CURRENT comment, labelled "current feedback", on the newest save per viewing only, read on the VIEWER's client (`attachCurrentTitles`). The evidence report prints neutral lines. The payload scan forbids `comment` / `liked` / `disliked`. DECISIONS `T-viewing-feedback-shape` (it records the reversal of T4.3's "no join").
- **Merge.** PR #65 → main `1402e9c` (pinned to `8c134a9`; branch CI green on it; main unmoved at `fff6f96`). Vercel production `dpl_Hp5S29Te7JrpgzTKrHyNbPNayZK7` READY. CI on the merge commit green (run 36125556523: checks, rls, e2e).
- **Verified in production, read-only, in the operator's Chrome (Browser 1):** the admin dashboard renders with no error boundary (0 feedback lines, 0 "current feedback" labels — hosted holds none); the busiest property (55 events) Activity tab renders its latest 50 lines, 36 of them the bare photo lines #63 left, 0 feedback lines, no error. `/login` 200, protected routes 307; no runtime errors in the hour after the deploy. Hosted counts (read-only): 0 viewings, 0 `viewing_feedback` events — nothing in production for the changed line to render, and no record was created to make one.
- **Review.** Adversarial workflow (5 lenses × 3 skeptics): no defect in the change; 9 confirmed findings fixed in `8c134a9` (uuid letter case, two tests that could not fail, stale docs/comments, the erasure note); 14/14 mutation proofs killed.
- **Open, on BACKLOG:** the erasure decision now also covers `viewings.feedback` (the words' only home; erasure never touches it); a crafted `rescheduled` event crashes the same three surfaces (pre-existing, `ebcfec5` — a follow-up task was offered); both row importers' `imported` identity; photo alt text (NOTE).

**2026-09-25, late morning (escalation recovery reason): an escalation recovery's event carries no typed reason. LANDED on the operator's word ("apply 0115 to hosted and merge #64"). HOSTED IS AT 0115.**
- **What.** Migration 0115 re-creates `request_lead_escalation_recovery` from 0111's text minus the `'reason'` key, the reason requirement and `v_reason`. `p_reason` stays in the signature and is ignored (no arity change, so the deploy order did not matter). The Review & resend dialog has no Reason box and says what the audit log records. The event keeps actor, action, job_id, key_serial, key_rotated and the previous state/category/result. DECISIONS `T-escalation-recovery-reason-shape`.
- **Hosted 0115 first.** Before: prosrc md5 `cf6f385c…` = 0111's body. One `execute_sql` with the file's exact text (its assertions passed). Verified separately: prosrc md5 `bf6060c8…` = local 0115, one overload, SECURITY DEFINER + `search_path=public`, ACL `authenticated` + `service_role` as before, no "reason" in the body; ledger by hand at 115, `non_filename_versions` 0. Security advisors: 2 ERR + 36 WARN, the known by-design residual (0115 adds nothing: the function was already counted).
- **Merge.** PR #64 → main `8b3b313` (pinned to `e3ae0d3`; branch CI green on it; main unmoved at `f075f9b`). Vercel production `dpl_14cRt53tGch9WoTYL6o8yYQYcQZD` READY. CI on the merge commit green (run 36116353647: checks, rls, e2e). Probes: `/login` 200, `/leads` and `/dashboard` 307 to login; no runtime errors in the hour after the deploy.
- **Hosted counts (read-only):** 0 `recovery_requested` events existed, so no historical copy is left in the chain. Escalation is still PAUSED; nothing was sent or recovered to test this in production.
- **Open, on BACKLOG:** viewing feedback (`viewing_feedback` payload), both row importers' `imported` identity, photo alt text (NOTE, kept on purpose); the erasure decision on `leads/deals.lost_reason`. A later migration may drop the ignored `p_reason` once no deployed caller sends it.

**2026-09-25, early morning (photo file names): a photo's events carry its id and the digest of its bytes, never its file name — LANDED on the operator's word ("merge, confirm the deploy, then check the live site"). No migration, nothing on hosted.**
...
| `SECURITY DEFINER` functions callable by `authenticated` (WARN ×9 when measured 2026-08-20; 20 by 2026-09-21, the growth being each later migration's deliberate app RPCs — 0101's `request_enquiry_alert_retry`, the staff retry, is the latest: it checks the org, the lead rule, a live lease and redaction itself; **0118's `close_deal` joins them** — the one path by which a user session makes a deal won or lost, restating `deals_update` and re-reading the caller under its row lock — so expect **+1 WARN (2 ERR + 37 WARN)** once 0118 is on hosted) | **Intentional.** `current_org_id`, `current_role_gnk` and `mfa_satisfied` are the RLS helpers; 0029 grants EXECUTE to `authenticated` deliberately and revokes it from `anon`, which was verified when it was applied. |
