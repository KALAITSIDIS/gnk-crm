...
- The UPDATE is conditional on `erased_at is null` as well as `is_archived = true`. Erase is offered on an archived contact, so an erasure could land between the read and the write; the condition means it is never undone. Zero rows back is followed by a re-read that says "erased" when that was the reason, and the existing permission sentence otherwise.
- The page asks one rule, `contactArchiveAction` (`lib/validators/contacts.ts`, beside `mayEditBuyerRequirements`): Archive on an active contact the viewer may update; Unarchive on an archived one unless it was merged or erased; nothing otherwise. An erased contact that is still ACTIVE — unarchived before this change — keeps Archive, because that is the way back to the state erasure left it in.
...
**Tests.** RED first — 9 failing: the four action tests below (no erased refusal, `erased_at` not read, no conditional write, the race answered "permission") and the five rule tests (`contactArchiveAction` did not exist). Unit: `lib/actions/contacts-unarchive-erased.test.ts` (12, the `fakeClient` pattern) — the refusal with no UPDATE, event or revalidation; `erased_at` in the read; erased-and-merged; the UPDATE's `is_archived = true` and `erased_at is null` conditions; one `unarchived` event for an unarchive that happened; zero rows classified as erased or permission; the old refusals (malformed id, unknown, active, merged, a slot taken meanwhile as a sentence, not a raw 23505) pinned. `lib/validators/contacts.test.ts` (+5) — the rule's five cases. E2E `tests/e2e/contact-unarchive-erased.spec.ts` (3, local only, service-client fixtures): the CONTROL — an archived contact that was not erased still unarchives through the button, one event, Archive back on the page (so the other two cannot pass by the button vanishing for everyone, and the new condition runs against real PostgREST); an erased contact's page shows the erasure banner and neither button; an erasure landing after the page was drawn is refused with the sentence in a toast, the row stays archived, no `unarchived` event.
