...
**Progress.** Everything below shipped on 2026-09-06/07. This table carries
MERGE SHAS rather than prose, because the paragraph that was here duplicated
what git and CI already hold and had gone stale inside a day — naming branches
for items already merged (T-review-drift (unnumbered), finding 10). A sha does
not go stale; `git show` is the detail.

| # | Item | gnk-crm | gnk-web | Note |
|---|---|---|---|---|
| Now 1 | Borrowed photographs removed | — | — | through the CRM, evented, feed at 0 images on both |
| Now 2 | Enquiry door (0087) | `8a149f0` | — | route deployed first, THEN applied hosted (the 0055/0057 order) |
| Now 3 | One `<JsonLd>` sink | — | `cc909c7` | |
| Now 4 | Erasure that means it | `ef05293` | — | + `redactLead` for what erasure could not reach |
| Now 5 | Every feed page; whole-or-absent sitemap | — | `80d9195` | |
| Now 6 | `--text-3` contrast + palette parity | `5d8fc58` | — | 4.97:1 / 4.68:1 |
| Now 7 | ETag digests the bytes sent | `0e67338` | — | DECISIONS `T-etag-from-body` |
| Now 8 | `npm audit fix` + a CI audit step | `a48f47b`, `d8dcb44` | `3add9af` | the workflow file went through the upload page (no `workflow` scope) |
| Next 1 | The site proves it is the forwarder | `cb56df0` | `467dabd` | `T-forwarder-proof`; three secrets set in Vercel |
| Next 2 | One pricing / bedrooms / currency; JSON-LD once | — | `8657901` | |
| Next 3 | Grouped price bracket (A09) | `e537961` | — | |
| Next 4 | **Migration 0088** | `8944ac4` | `34587d6` | applied hosted BEFORE the merge (additive); 88/88 zero drift; `T-feed-reference` |
| Next 5 | Paged, loud reads (A08a) | `80f208b` | — | |
| Next 6 | Optimistic predicate on section saves | `a76e105` | — | **merged on a red e2e**; locator fixed in `7af529e` — see `T-optimistic-save` |
| Next 7 | Floor plans to the private bucket | `7af529e` | — | production had zero non-photo rows |
| Next 8 | Four small site fixes | — | `65d4513` | |
| Next 9 | The measured freshness, stated | — | `65d4513` | README + `lib/freshness.test.ts` |
| — | `T-review-drift`: eleven findings of a twelve-lens review | `fix/review-drift` | `fix/review-drift` | the only item not yet merged |

**The Next phase is complete** except F3's computed container columns, which
stay in §5 Later until PAF0002's units are ready to publish.
...
| 7 | **ETag derived from the body** (A10): the route already runs `public_listings` on every 200 — hash the serialised listings and answer 304 on match. A validator computed from the body cannot drift. *(The round-trip drop this row also promised did NOT happen and was never going to: `public_listings_etag` stays, because gnk-web compares its segment across pages — see `T-etag-from-body`.)* Discharges the "next migration must carry the 0087 area-rename segment" note in DECISIONS/HANDOFF without SQL; `public_listings_etag` and RLS tests 43/56 stay until a later migration retires it. | 0.5 | no | route test: ETag moves on an alt edit and on an area rename, stable otherwise |
...
**The six borrowed photographs removed from PAF0003 and PAF0004 on 2026-09-06 are still on PAF0001, which is live and public** — same six files, uploaded to all four listings within one hour on 2026-09-01; found by 0088's hash backfill on 2026-09-07 and confirmed against the event log. See HANDOFF's operator row; the CRM now warns on the worklist and on both listings. This is the first thing to decide. Real photographs for PAF0003 and PAF0004 (≥1600 px wide); PAF0002's title and description before it publishes; two principal names, roles, bios and portraits; the lawyer's view on Law 71(I)/2010 wording; the domain cut-over (`SITE_URL`); whether floor plans are held under confidentiality; the enquiry retention period.
