...
| A10 | ~~**Turn on Supabase leaked-password protection**~~ — ❌ **ASSESSED 2026-08-28: NOT A GAP HERE, do not buy Pro for it.** It is Pro-gated and this org is free, but the decisive point is that **there is nothing for it to check**: `inviteUser` mints every password as `randomBytes(9)` (72 bits of entropy) and there is no forgot-password link, no change-password UI and no SMTP — no human has ever chosen a password in this system, and HIBP matches human-chosen passwords. The advisor lint's PRESENCE is expected, not a regression. Re-open the day a password-change or self-signup flow lands; the free fix then is the HIBP range API (k-anonymity, no key, no plan). The free half — minimum length and required character classes, NOT Pro-gated — was set on 2026-08-28. Original entry below. | ⚠️ **NOT a toggle — corrected 2026-08-04.** Gated to **Supabase Pro** on this plan, so it is a spend decision, not 5 minutes. Until the plan changes the standing `auth_leaked_password_protection` advisor finding is **accepted, not unnoticed**. Not agent-reachable either (no auth-config tool on the connector; platform config, not DB state). | plan upgrade | operator |
...
### C2. Two-factor authentication — ✅ **DONE, AND MANDATORY SINCE 2026-08-28.** Opt-in enrolment 2026-07-24; DB-level enforcement for enrolled users 2026-08-11 (0029); **mandatory for everyone 2026-08-28** — `MFA_REQUIRED = true` gates the browser and migration **0059** removed the opt-in arm from `mfa_satisfied()`, so a session without a second factor reads NOTHING through any client. The two halves are coupled by a test that fails if either ships alone. Both operators confirmed sign-in afterwards. What it cost was the TEST HARNESS, not the switch: fixtures now enrol a TOTP factor so the RLS suite passes under either rule, and the E2E setup enrols a real factor and answers a real challenge every run. See DECISIONS `T-mfa-mandatory`.
...
- **Hard delete anywhere** — the append-only hash-chained `events` spine *is* the commission evidence. Retire states (archived / withdrawn / spam / lost) are the correct answer, and the GDPR erasure path already handles the one case that legally requires more.
  ...
  **RESOLVED 2026-09-02 — the rule now carves out what the practice already proved.** Production was hard-deleted twice (nineteen properties 2026-08-22; PAF0002/0003/0005 + a viewing with its signed slip 2026-08-28), both times operator-instructed, both times TEST records. Rather than keep a rule the project breaks, the carve-out is now explicit:

  **Test-record exception — valid only while production holds zero real client data.** Hard-deleting an operator-created test record is permitted when ALL of: (1) the operator instructs it by name; (2) the rows are test data, not evidence of any real transaction; (3) the deleting session records what was removed and accepts the orphaned events (they are the audit trail OF the deletion — 28 `property` events already reference deleted rows, and the chain verifies because events are never touched); (4) `verify_events_chain()` is run afterwards and comes back true. **The exception dies the day the first real client record lands** — from that point the original rule is absolute: retire states only, erasure path for the one legal exception. The dead reference in HANDOFF that pointed at hard-deleted PAF0002 (“still wants archiving”) is gone with this ruling.
