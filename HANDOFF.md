...
| # | Item | Who | Note |
|---|---|---|---|
| 0 | **Arm the dead-man's switch** | operator | Create a healthchecks.io check (Period 1 day, Grace 2h) and paste its URL as `HEALTHCHECK_URL=` in `C:\Users\user\.gnk-crm\backup.env`. The plumbing shipped 2026-08-29 and logs a SKIPPED line nightly until armed; without it, a powered-off machine still means silent missed backups. ~2 minutes. |
| 1 | **Store the USB offsite** | operator | It is verified and restore-proven but only helps if it leaves the building. Downgraded from "highest-value item" on 2026-08-29 — the automated OneDrive leg now ships a verified archive off-machine nightly (§3.3) — but the USB remains the offline copy that sync cannot corrupt. |
