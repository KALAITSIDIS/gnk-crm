...
| ⚠ Operator | **PAF0001 and PAF0002 hold the SAME SIX PHOTOGRAPHS, byte for byte** — found 2026-09-07 the moment 0088's `content_sha256` backfill ran on production (12 rows, 6 distinct images, every one on both listings; almost certainly the create-from-duplicate path copying media). PAF0001 is a **live public mandate**; PAF0002 is the unpublished six-villa development. One of the two is wrong, and if PAF0002 publishes as it stands it shows one existing villa's photographs as a six-villa development — the same class as the borrowed photographs removed from PAF0003/PAF0004 on 2026-09-06. **Nothing was changed: whose photographs these are is the operator's call.** The CRM now says so itself — an amber section on `/properties/worklist` and an amber line in each listing's quality-ring tooltip (`T-feed-reference` (3); a warning, never a score change, because a development's units legitimately share exteriors). |
