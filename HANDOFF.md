...
| 1e | **Arm the desk-alert outbox** (0101, PR #22 → `ba54f18`) | operator | **(a)–(c) DONE 2026-09-21** — pushed (CI 35601803676 green), 0101 applied to hosted and verified, merged, production `dpl_26ieXHH9H8PfufJ5epP33GP12Xyo` READY and aliased. **REMAINING:** (d) Vercel, gnk-crm, Production: add `CRON_SECRET` (a long random string), redeploy — the daily Vercel cron in `vercel.json` then sweeps once a day. (e) Decide `pg_net` (the same decision as 1d's third point): yes → Vault secrets `cron_secret` + `crm_url`, move `supabase/activation/0103_enquiry_alerts_cron.sql` into migrations, bump the five cron pins, apply — a retry then lands within two minutes. (f, added by the 2026-09-21 review) approve, apply and merge migration 0102 first — the key window, the budget and the honest sweep verdict — see the latest dated line. Until (d) the sweep answers 503 and the inbox shows a missed alert as "Desk alert queued"; the accelerator sends at once regardless. |
