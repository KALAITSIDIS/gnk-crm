...
**Stacked on PR #52.** #52 is not merged, so this branch starts from `fix/sentry-incoming-request-scrub` (`aad675d`) and its PR targets that branch. The diff is this change only. GitHub retargets it to `main` when #52 merges and its branch is deleted. **Merge #52 first.**
