...
- Refused: a blank header name (a trailing separator makes one) or a name appearing twice after the existing trim — case is not folded, so `Bedrooms` stays an unknown column for LST-10's check; a row with more or fewer cells than the header, giving both counts; a quote never closed (reported at the line that opened it), a quote inside an unquoted value, text after a closing quote.
...
- Unchanged: numeric interpretation (`parseNumberCell`), the measurement rules, the delimiter choice (`delimiterOf`), BOM, CRLF/LF, escaped quotes, quoted line breaks and separators, explicitly empty cells (`,,` and `""` are empty, not missing), optional columns left out of the header, publication, contact matching, permissions, the feed.
