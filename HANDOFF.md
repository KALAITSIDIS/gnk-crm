...
| Hosted DB | `yjgirvzgoiywdojnpkpd` — **37 migrations, latest `0037` (applied 2026-08-21)**, `non_filename_versions` = **0**, **79 events**, 3 properties (1 with an owner, backfilled by 0034 from its active mandate), 1 mandate — all MEASURED 2026-08-21 in calls separate from the ones that applied the migrations. **`verify_events_chain` checked BEFORE and AFTER each apply**, true both sides every time; a chain that verifies only afterwards proves nothing about what the migration did to it. `get_advisors` run after each: no new lint, and 0034/0037 created no database objects at all. **0037 closed a privilege-escalation path** — `mandates_safe` gave `anon` and `authenticated` `arwd`, and the view is auto-updatable with a `rolbypassrls` owner, so writes through it skipped `mandates` RLS. Now `authenticated=r`, `anon` nothing, byte-identical to local. **DB-level 2FA is LIVE** — `require_aal2` on all 29 RLS tables, IMPROVEMENTS C2 |
