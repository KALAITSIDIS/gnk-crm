 ...
 * identity, generator + scope disclosure, chronological event table,
 * signed-slip thumbnails + hashes, deals with manual commission notes,
 * chain-check + report-hash footer. Text set in Noto Sans (see pdf-fonts.ts)
 * so Greek/Cyrillic names survive; Courier only carries hex digests.
 */

/**
 * Noto Sans LGC has no arrows block — U+2192 (used by the from→to event
 * lines) would print as a tofu quote. Render-time substitution only: the
 * report hash is computed from the raw row strings before this runs.
 */
const glyphSafe = (s: string) => s.replace(/→/g, "->");

const styles = StyleSheet.create({
  page: { padding: 40, paddingBottom: 64, fontSize: 9, color: "#1a1a1a", fontFamily: PDF_FONT },
  org: { fontSize: 15, fontFamily: PDF_FONT, fontWeight: 700 },
  title: { fontSize: 11, marginTop: 2, marginBottom: 12, color: "#555" },
  identity: { marginBottom: 4, flexDirection: "row", gap: 6 },
  idLabel: { width: 70, color: "#777", fontFamily: PDF_FONT, fontWeight: 700 },
  scope: { marginTop: 6, fontSize: 8, color: "#777" },
  chain: {
    marginTop: 8,
    marginBottom: 12,
    padding: 6,
  ...
  cProp: { width: 90 },
  cActor: { width: 90 },
  section: { fontSize: 11, fontFamily: PDF_FONT, fontWeight: 700, marginTop: 16, marginBottom: 6 },
      ...
      <Page size="A4" style={styles.page}>
        ...
        {d.filter.from || d.filter.to ? (
          <View style={styles.identity}>
            ...
            <Text>
              {d.filter.from ?? "…"} — {d.filter.to ?? "…"} (Cyprus local days, inclusive)
            </Text>
          </View>
        ) : null}
        <Text style={styles.scope}>
          Generated by {d.generatedBy.name} ({d.generatedBy.role.replace(/_/g, " ")}).
          {d.generatedBy.role === "admin"
            ? " Scope: full organization record."
            : " Scope: events visible to this user — activity by other staff or by the system may be absent."}
        </Text>

        <Text style={[styles.chain, d.chain === "verified" ? styles.chainOk : styles.chainBad]}>
          Event hash chain verified: {d.chain === "verified" ? "TRUE" : "FALSE"}{" "}
          (verify_events_chain at generation time)
        </Text>
        ...
        {d.rows.map((r, i) => (
          <View key={i} style={styles.tr} wrap={false}>
            ...
            <Text style={styles.cEvent}>{glyphSafe(r.line)}</Text>
...
export async function renderEvidencePdf(d: EvidenceData, generatedAt: string): Promise<Buffer> {
  if (d.chain === "skipped") {
    throw new Error("Evidence PDF requires a chain verification result (verifyChain: true)");
  }
  registerPdfFonts();
