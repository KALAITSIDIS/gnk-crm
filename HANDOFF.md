...
| | |
|---|---|
| `main` | **in sync with `origin/main` as of 2026-08-23** — `fix/report-phase-a` merged (`b15066c`, `--no-ff`, 4 commits: T-A1 · T-A2 · T-A3 + the plan) and PUSHED. **CI green on `b15066c` for that SHA — `checks` · `e2e` · `rls`** — and green on the branch head `b9bc0dd` first, which is why main never risked going red: `ci.yml` is `on: push:` with no branch filter, so a branch push is a free rehearsal. Production READY from `b15066c` (`/login` 200, `/dashboard` 307, 0 runtime errors in a live 2h window). **THE "COMMIT, DON'T PUSH" AGREEMENT IS SUPERSEDED** — on 2026-08-23 the operator asked why work was left unpushed and said to navigate and decide rather than park it. Treat push-and-deploy as expected unless told otherwise. **`fix/report-phase-a` still exists local and remote — DELETE IT**, per the standing rule that a stale branch is a claim someone will read. Phase A of `IMPROVEMENTS_EXECUTION.md` shipped; **Phases B and C are specified and NOT started.** `git branch -vv` and `git branch -r` are the answer, not this cell. |
...
## 0a. NEXT UP — Phases B and C of `IMPROVEMENTS_EXECUTION.md` (2026-08-23)

**Read `IMPROVEMENTS_EXECUTION.md`, not this section.** It holds the 13-task
plan that came out of an outside consultant report on 2026-08-23, and this
section deliberately does not restate it — that is the bug §0 keeps having.

| | |
|---|---|
| shipped | **Phase A** — T-A1 (share-link `opened` rendered by kind), T-A2 (median/p90 first response, **0042**, local + hosted), T-A3 (`formatResponseMinutes`, so a pre-0042 hosted cannot paint `NaNh NaNm`). Merged `b15066c`, CI green for that SHA, production READY |
| queued | **Phase B** — buyer requirements as a table (**0043**), then bidirectional matching. **Phase C** — reservations (**0044**) with a partial unique index for one-live-per-property and a pg_cron expiry |
| the caveat that matters | Phases B and C are **NEW SCOPE** on a system **no client has used** — production is still operator test data (§0). The plan says to re-read them after one real week of desk use, because that week is worth more than the plan's guesses. The operator directed the build anyway on 2026-08-23; that is their call, recorded rather than re-argued |
| how specified | **unevenly, on purpose, and the plan says so in its own "How deeply each task is specified" section.** Tasks 3 and 10 carry complete DDL; tasks 5, 6, 8, 9 and 12 carry behaviour and constraints but not invented code, because their target files were never read at plan time. **Re-specify each against the real file before starting it.** |

**Two things worth carrying rather than looking up.** T-A2's plan specified a
standalone `lead_response_percentiles` function; that was wrong and was changed
during the build, because 0018 exists to collapse round trips and the `leads_7`
CTE already held the rows. **The plan's step text was left as written rather
than back-edited**, so the deviation stays visible.

And the one only the rendered page could find, again: the first attempt to seed
dashboard data as `postgres` wrote to the **RLS fixture org** (`aaaaaaaa-…`)
rather than the admin's (`00000000-…-0001`), so the dashboard correctly showed
zeros — `admin_dashboard_stats` is SECURITY INVOKER. Restored byte-identical and
re-proven with `npm run test:rls` (49/49, first run), per A8.
