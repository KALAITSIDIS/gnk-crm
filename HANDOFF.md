...
**2026-09-25, latest (late night, deal close): closing a deal is one transaction — BUILT on branch `fix/atomic-deal-close`, PR open, NOT LANDED. HOSTED IS STILL AT 0116. The LOCAL stack is at 0117 (applied from the branch; `main` has no 0117 file yet).**
- **What.** BACKLOG's open-status fold, verified first: at `6366ef8` two overlapping Lost requests wrote two `lost` events, overlapping Won and Lost left a lost deal carrying `won_at`/`final_value`, override Wins wrote everything twice, and a refused event left a closed deal with none (reproduced with the real actions on the real stack). Migration 0117: `close_deal` (SECURITY INVOKER, row lock, status checked under it, UPDATE and every event in one transaction, `already_closed` / `conflict` answers) and `deals_closed_guard` (user sessions cannot rewrite, swap or reopen a closed deal — which also covers the deployed app in the hosted-first window). The actions never throw; Won follow-ups run only for the request that committed. DECISIONS `T-atomic-deal-close`.
- **To land (operator's word needed — the brief authorised branch + PR only):** hosted 0117 FIRST (additive; the deployed app keeps working and its races are refused), then merge, deploy READY, probes. Rollback order: revert the app first; drop the objects only through a forward migration.
- **If the PR is abandoned:** the local stack still carries `close_deal`, `trg_deals_closed_guard` and the ledger row 0117 — drop the trigger and both functions and delete the ledger row, or `supabase db reset`.
- **Open, on BACKLOG (new):** a direct write can still close an OPEN deal (deploy-coupled follow-up); Won reminders are at-most-once; NaN is a legal price; `move_deal_to_stage` takes FOR UPDATE; a rare close/nudge-sweep deadlock through the nudge tasks; cross-org nudge supersession; offers editable on a closed deal; the dashboard's won-this-month lost `final_value` (0093); on a phone a deal with an offer cannot be closed from its page.
