...
|---|---|---|---|
| ~~1~~ | ~~**Copy the offsite backup off this machine**~~ **DONE 2026-08-26 — copied to a USB stick (`E:`, Transcend, FAT32) at `E:\gnk-offsite-2026-08-26\`** | — | **The CRM and its backups no longer share one disk.** 873 nightly files SHA256-compared source vs USB: 0 missing, 0 extra, 0 mismatches. Both archives verified against their sidecars and `gzip -t`. **STILL NEEDS A HUMAN: the stick must be STORED ELSEWHERE** — left in this machine it is the same fire/theft/flood as the original. **KEEP BOTH ARCHIVES — they are NOT duplicates:** the nightly job prunes, so 2026-08-07…08-10 survive ONLY in the 08-23 archive (gone from disk), while 08-24…08-27 are only in the new one; 22 distinct sets across the two. **No restore test has ever been done** — this proves the files copied, not that they rebuild a database. `README.txt` on the stick records all of it. |
