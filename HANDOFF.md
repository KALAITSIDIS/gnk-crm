...
| 2 | **Verify `default_mandate_terms`** | operator | **Narrower than it first looks — measured 2026-08-26.** 6 of 8 rows are unverified, but the calculators read ONLY `transfer_fees` and `stamp_duty` (`calculators/page.tsx` names both in an `.in()`), and those two are the verified, test-pinned pair. **`default_mandate_terms` is the only unverified row anything reads** — `getPartyDefaults` prefills every new mandate from it, so its placeholder **3% / open / 6 months** lands on real contracts. `vat_property`, `capital_gains_tax`, `other_property_taxes` and `company_details` have **zero code references**: unverified but inert, and they must be verified BEFORE anything is built on them (`vat_property` blocks item 6). |
