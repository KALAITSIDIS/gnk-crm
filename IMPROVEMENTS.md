...
**Remaining lists** (properties, leads, deals, viewings, keys, tasks) each repeat the pattern: extract the list's filter parse/apply into `lib/queries/<list>.ts`, add an export route that calls `logListExport`, define columns. ~0.5 day each.
